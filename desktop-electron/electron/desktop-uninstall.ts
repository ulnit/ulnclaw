 ...
 * the three user-facing uninstall modes to the `ulnclaw uninstall` CLI flags,
...
/**
 * Map an uninstall mode to the `python -m ulnclaw_cli.uninstall` argv (after the
 * python executable). Uses the dedicated lightweight module entrypoint (not
 * `ulnclaw_cli.main`) so it can run under a system Python OUTSIDE the venv that
...
function uninstallArgsForMode(mode) {
  ...
  return ['-m', 'ulnclaw_cli.uninstall', '--mode', mode]
}
 ...
 *
 *   macOS:   …/ulnclaw.app/Contents/MacOS/ulnclaw  → …/ulnclaw.app
 *   Windows: …\ulnclaw\ulnclaw.exe                 → …\ulnclaw  (install dir)
...
function resolveRemovableAppPath(execPath, platform, env: any = {}) {
  ...
  if (platform === 'darwin') {
    // …/ulnclaw.app/Contents/MacOS/ulnclaw → strip 3 segments to the .app
    const macOsDir = p.dirname(exe) // …/Contents/MacOS
    const contents = p.dirname(macOsDir) // …/Contents
    const appBundle = p.dirname(contents) // …/ulnclaw.app
  ...
  if (platform === 'win32') {
    // NSIS per-user installs ulnclaw.exe directly in the install dir.
    const dir = p.dirname(exe)

    if (/[\\/]ulnclaw$/i.test(dir) || /[\\/]ulnclaw-desktop$/i.test(dir)) {
      return dir
    }
  ...
  // Unpacked electron-builder tree: …/linux-unpacked/ulnclaw
  const dir = p.dirname(exe)
 ...
 * being deleted); `pythonPath` is prepended to PYTHONPATH so `import ulnclaw_cli`
 ...
 */
function buildPosixCleanupScript({ desktopPid, pythonExe, pythonPath, agentRoot, uninstallArgs, appPath, ulnclawHome }) {
  ...
  const lines = [
    ...
    '  done',
    'fi',
    `export ULNCLAW_HOME=${q(ulnclawHome)}`
  ]
 ...
 * modes + `pythonPath`=agentRoot so `import ulnclaw_cli` resolves from source
...
function buildWindowsCleanupScript({
  ...
  // a problem, but ulnclaw install paths don't use them.
  const q = s => `"${String(s).replace(/"/g, '')}"`

  const lines = [
    '@echo off',
    'setlocal enableextensions',
    `set "ULNCLAW_HOME=${String(ulnclawHome).replace(/"/g, '')}"`,
    `set "PID=${pid}"`
  ]
