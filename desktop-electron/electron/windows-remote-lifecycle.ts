...
const READY_RE = /^ULNCLAW_(?:BACKEND|DASHBOARD)_READY port=(\d+)/gm
...
async function probeWindowsRemote(ssh, explicitulnclawPath = '') {
  const explicit = psLiteral(explicitulnclawPath)

  const script = [
    '$ErrorActionPreference="Stop"',
    `$explicit=${explicit}`,
    '$ulnclawHome=$env:ULNCLAW_HOME',
    'if(-not $ulnclawHome){$ulnclawHome=Join-Path $env:LOCALAPPDATA "ulnclaw"}',
    '$candidates=@()',
    'if($explicit){$candidates+=$explicit}',
    '$cmd=Get-Command ulnclaw.exe -ErrorAction SilentlyContinue',
    'if($cmd){$candidates+=$cmd.Source}',
    '$candidates+=(Join-Path $ulnclawHome "ulnclaw-agent\\venv\\Scripts\\ulnclaw.exe")',
    '$candidates+=(Join-Path $HOME "ulnclaw-agent\\.venv\\Scripts\\ulnclaw.exe")',
    '$ulnclaw=$candidates|Where-Object{Test-Path -LiteralPath $_ -PathType Leaf}|Select-Object -First 1',
    'if(-not $ulnclaw){throw "ulnclaw is not installed on the remote Windows host."}',
    'if($explicit -and $ulnclaw -ne $explicit){throw "The configured ulnclaw path is not an executable file."}',
    '$python=Join-Path (Split-Path $ulnclaw) "python.exe"',
    'if(-not (Test-Path -LiteralPath $python -PathType Leaf)){throw "The remote ulnclaw Python runtime was not found."}',
    '[ordered]@{os="Windows";arch=$env:PROCESSOR_ARCHITECTURE;ulnclawHome=$ulnclawHome;ulnclawPath=$ulnclaw;python=$python}|ConvertTo-Json -Compress'
  ].join(';')
...
async function detectRemotePlatform(ssh, explicitulnclawPath = '') {
  try {
  ...
  try {
    return await probeWindowsRemote(ssh, explicitulnclawPath)
  } catch (cause: any) {
...
function helperCommand(runtime, operation, args = []) {
  const argv = [runtime.python, '-m', 'ulnclaw_cli.windows_ssh_runtime', operation, ...args]
...
function validLock(lock, ownershipId) {
  ...
  return Boolean(
    ...
    typeof lock.ulnclawPath === 'string' &&
    typeof lock.ulnclawHome === 'string'
  )
}
...
function reusableWindowsLock(lock, state, profile, reuseToken, runtime) {
  return Boolean(
    ...
    lock.ulnclawPath === runtime.ulnclawPath &&
    lock.ulnclawHome === runtime.ulnclawHome
  )
}
...
async function connectWindowsRemote(deps) {
  ...
  assertCurrent(signal)
  const runtime = await probeWindowsRemote(ssh, remoteulnclawPath)
  const inspection = await helper(ssh, runtime, 'inspect', [runtime.ulnclawPath])

  if (!inspection.supported) {
    const error: any = new Error('Update ulnclaw on the remote Windows host before connecting with Desktop SSH.')
    error.kind = 'update-required'
    throw error
  }

  runtime.ulnclawPath = inspection.path
  const ulnclawVersion = inspection.version || ''
  rememberLog(`[ssh-lifecycle] remote platform Windows/${runtime.arch}`)
  rememberLog(`[ssh-lifecycle] located ulnclaw at ${runtime.ulnclawPath}`)
  ...
  try {
    spawned = await helper(
      ...
      'spawn',
      [],
      JSON.stringify({ ownershipId, spawnNonce, profile, ulnclawPath: runtime.ulnclawPath })
    )
  } catch (error) {
  ...
  const owned = {
    ...
    profile,
    ulnclawPath: runtime.ulnclawPath,
    ulnclawHome: runtime.ulnclawHome,
  ...
  try {
    ...
    const baseUrl = `http://127.0.0.1:${localPort}`
    await waitForulnclaw(baseUrl, token)
    assertCurrent(signal)
...
function buildWindowsInteractiveCommand(remoteCwd = '') {
  ...
  script.push('$host.UI.RawUI.WindowTitle="ulnclaw SSH"', 'powershell.exe -NoLogo')
