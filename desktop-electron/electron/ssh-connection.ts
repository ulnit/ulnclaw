...
const _REDACTIONS: Array<[RegExp, string]> = [
  [/(ULNCLAW_DASHBOARD_SESSION_TOKEN=)(\S+)/g, '$1<redacted>'],
  [/(X-ulnclaw-Session-Token["']?\s*[:=]\s*["']?)([^\s"'&]+)/gi, '$1<redacted>'],
...
// root under a short per-user base (`~/.ulnclaw/desktop-ssh`) so even worst case
...
function defaultControlDir() {
  ...
  if (process.platform === 'win32') {
    return path.join(os.tmpdir(), 'ulnclaw-desktop-ssh')
  }

  return path.join(os.homedir(), '.ulnclaw', 'desktop-ssh')
}
