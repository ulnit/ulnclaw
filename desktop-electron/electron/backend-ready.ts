...
// `ulnclaw serve` announces ULNCLAW_BACKEND_READY; the legacy `ulnclaw dashboard`
// backend announces ULNCLAW_DASHBOARD_READY. Accept either so the desktop spawn
...
// compile and import the whole `ulnclaw_cli.main` → `web_server` → FastAPI/
 ...
 * ULNCLAW_DESKTOP_PORT_ANNOUNCE_TIMEOUT_MS env override (for users on slow
...
function resolvePortAnnounceTimeoutMs(env = process.env) {
  const parsed = Number(env.ULNCLAW_DESKTOP_PORT_ANNOUNCE_TIMEOUT_MS)
...
/**
 * Watch a child process's stdout for the `ULNCLAW_(BACKEND|DASHBOARD)_READY
...
function waitForDashboardPort(child, timeoutMs = resolvePortAnnounceTimeoutMs()) {
  return new Promise((resolve, reject) => {
    ...
    function onExit(code, signal) {
      cleanup()
      reject(new Error(`ulnclaw backend: exited before port announcement (${signal || code})`))
    }
    ...
    const timer = setTimeout(() => {
      cleanup()
      reject(new Error(`Timed out waiting for ulnclaw backend port announcement (${timeoutMs}ms)`))
    }, timeoutMs)
