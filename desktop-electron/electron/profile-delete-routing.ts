// Profile-delete routing logic for the `ulnclaw:api` IPC handler.
//
...
// pool backend -- spawning a fresh one would call ensure_ulnclaw_home() and
...
/**
 * Parse a `ulnclaw:api` request into the profile name a DELETE targets, or
...
/**
 * Route the next `ulnclaw:api` request away from the primary/window backend
 ...
 * ensure_ulnclaw_home() recreates the directory the delete just removed.
 */
