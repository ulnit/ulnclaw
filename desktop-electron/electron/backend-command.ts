// Backend subcommand routing for the desktop-managed ulnclaw process.
//
// The desktop app launches its own headless backend via `ulnclaw serve` — it
...
// app hasn't updated yet, or an older `ulnclaw` resolved from PATH) only knows
 ...
 * @param {string} [profile] optional ulnclaw profile to pin via `--profile`.
 */
export function serveBackendArgs(profile?: string) {
  const head = profile ? ['--profile', profile] : []

  return [...head, 'gateway', '--host', '127.0.0.1', '--port', '0']
}
 ...
 * `-m ulnclaw_cli.main` and any `--profile <name>`). Returns a copy; if there is
...
/**
 * True when a runtime's `ulnclaw_cli/subcommands/dashboard.py` source registers
