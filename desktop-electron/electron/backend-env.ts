...
/**
 * ulnclaw-managed Node.js directories, in preferred lookup order.
 *
 * There are two on-disk layouts. `scripts/install.ps1` unpacks portable Node
 * straight into `%LOCALAPPDATA%\ulnclaw\node` (node.exe at the root, no `bin\`);
 * `scripts/install.sh` and the node-bootstrap helper use the POSIX
 * `$ULNCLAW_HOME/node/bin`. Emit BOTH on every platform so mixed and migrated
 ...
 * `iter_ulnclaw_node_dirs()` in ulnclaw_constants.py, which the Electron main
 * process cannot import.
 */
function ulnclawManagedNodePathEntries(
  ulnclawHome,
  ...
  const root = pathModule.join(ulnclawHome, 'node')
...
function buildDesktopBackendPath({
  ...
  const ulnclawNodeDirs = ulnclawManagedNodePathEntries(ulnclawHome, { platform, pathModule })
  const venvBin = venvRoot ? pathModule.join(venvRoot, platform === 'win32' ? 'Scripts' : 'bin') : null
  const saneEntries = platform === 'win32' ? [] : POSIX_SANE_PATH_ENTRIES

  return appendUniquePathEntries([ulnclawNodeDirs, venvBin, currentPath, saneEntries], { delimiter })
}

function normalizeulnclawHomeRoot(ulnclawHome, { pathModule = pathModuleForPlatform(process.platform) }: any = {}) {
  if (!ulnclawHome) {
    return ulnclawHome
  }

  const resolved = pathModule.resolve(String(ulnclawHome))
...
function buildDesktopBackendEnv({
  ...
  return {
    ...
    // cp1252, ...). ulnclaw_bootstrap sets this inside the child too, but only
...
export {
  ...
  ulnclawManagedNodePathEntries,
  normalizeulnclawHomeRoot,
  pathEnvKey,
