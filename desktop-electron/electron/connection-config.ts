 ...
 *     `X-ulnclaw-Session-Token` header; WS uses `?token=`.
...
// ulnclaw_cli/dashboard_auth/cookies.py.
//
// Two cookies are in play (see that module):
//   - ulnclaw_session_at: the OAuth access token. Short-lived (~15 min); its
//     Max-Age tracks the access-token TTL, so the cookie jar drops it the
//     instant the AT expires.
//   - ulnclaw_session_rt: the OAuth refresh token. Long-lived (24h rotating,
//     reuse-detected — Portal NAS #293 / ulnclaw #37247). When the AT cookie
...
const AT_COOKIE_VARIANTS = ['__Host-ulnclaw_session_at', '__Secure-ulnclaw_session_at', 'ulnclaw_session_at']
const RT_COOKIE_VARIANTS = ['__Host-ulnclaw_session_rt', '__Secure-ulnclaw_session_rt', 'ulnclaw_session_rt']

// The Nous portal (NAS) does NOT use ulnclaw gateway session cookies — it is a
...
// Keep this aligned with ulnclaw_cli.profiles.validate_profile_name(). `default`
// is the built-in root alias; these names cannot be created as profiles.
const RESERVED_REMOTE_PROFILES = new Set(['ulnclaw', 'test', 'tmp', 'root', 'sudo'])
...
// ulnclaw Cloud connection (cloud-auto-discovery Q3/Q6): it carries a
...
function normalizeSshConfig(entry) {
  ...
  const remoteulnclawPath = String(entry.remoteulnclawPath || '').trim()

  if (remoteulnclawPath) {
    out.remoteulnclawPath = remoteulnclawPath
  }

  // A Desktop profile can be a local routing label rather than the profile
  // name used by the remote ulnclaw installation. Preserve an explicit mapping
  // when it is a valid ulnclaw profile identifier; otherwise fall back to the
 ...
 *     `--profile`, so its `ULNCLAW_HOME` scopes it.
 *
...
function pathWithGlobalRemoteProfile(path, profile, opts: ProfileRouteOptions = {}) {
  ...
  try {
    parsed = new URL(rawPath, 'http://ulnclaw.local')
  } catch {
    return path
  }
...
/**
 * True if any cookie in `cookies` is a ulnclaw session ACCESS-token cookie
 ...
 * the ulnclaw gateway session cookies, so cloud sign-in / discovery liveness
