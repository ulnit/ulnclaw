 ...
 *   1. ULNCLAW_GIT_BASH_PATH env var override
 *   2. PortableGit under %LOCALAPPDATA%\ulnclaw\git\ (install.ps1)
...
export function findGitBash(opts: GitBashOptions): string | null {
  ...
  // Respect ULNCLAW_GIT_BASH_PATH if set (mirrors tools/environments/local.py:_find_bash).
  const gitBashPath = env.ULNCLAW_GIT_BASH_PATH
  ...
  if (localAppData) {
    candidates.push(joinWin(localAppData, 'ulnclaw', 'git', 'bin', 'bash.exe'))
    candidates.push(joinWin(localAppData, 'ulnclaw', 'git', 'usr', 'bin', 'bash.exe'))
  }
