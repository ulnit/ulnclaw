 ...
 *
 * The Tauri installer self-copies into ULNCLAW_HOME on *every* platform
 * (`ulnclaw-setup.exe` on Windows, `ulnclaw-setup` elsewhere — see
 * apps/bootstrap-installer `paths::installer_dest` and
 * `bootstrap::copy_self_to_ulnclaw_home`), so finding that binary on macOS or
 ...
 * than the hand-off protocol holds the update marker, spawns `ulnclaw update`,
...
export function resolveStagedUpdaterBinary(
  ...
  const candidate = path.join(ulnclawHome, 'ulnclaw-setup.exe')
 ...
 *
 * `copy_self_to_ulnclaw_home` deliberately no-ops during `--update`
 ...
 * "Another ulnclaw update is already running (PID <itself>, started 1s ago)" —
 * the observed infinite "Install didn't finish" loop. Skipping the pre-write
 * for those binaries lets them acquire cleanly and run `ulnclaw update`, which
 ...
 * the installer is written to ULNCLAW_HOME at install/repair time, making mtime
