...
const SCAN_MODULE = 'ulnclaw_cli._scan_venv_blockers'
...
export function formatBlockerMessage(result: VenvBlockerScanResult): string {
  const lines = [
    'Update aborted: another ulnclaw process is using this installation.',
    '',
...
export function formatProbeFailedMessage(): string {
  return (
    'Update aborted: Desktop could not verify the ulnclaw installation is free.\n' +
    '\n' +
    'Close other ulnclaw windows and terminals, then retry.  If the problem\n' +
    'persists, run `ulnclaw update` in a terminal for detailed diagnostics.'
  )
}
