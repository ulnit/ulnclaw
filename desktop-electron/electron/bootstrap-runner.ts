 ...
 * bootstrap-runner.ts
 *
 * Drives apps/desktop's first-launch install of ulnclaw Agent by spawning
...
function readExistingPinnedCommit(activeRoot: string | null | undefined): string | null {
  ...
  try {
    const raw = fs.readFileSync(path.join(activeRoot, '.ulnclaw-bootstrap-complete'), 'utf8')
...
function bootstrapCacheDir(ulnclawHome) {
  return path.join(ulnclawHome, 'bootstrap-cache')
}

// The install.sh / install.ps1 that ships inside the already-installed agent
// checkout under ~/.ulnclaw/ulnclaw-agent. Used as a last-resort fallback when
// the pinned commit can't be fetched from GitHub (e.g. a locally-built desktop
// app stamped to an unpushed HEAD).
function installedAgentInstallScript(ulnclawHome) {
  if (!ulnclawHome) {
    return null
  }

  const candidate = path.join(ulnclawHome, 'ulnclaw-agent', 'scripts', installScriptName())
...
function cachedScriptPath(ulnclawHome, commit) {
  return path.join(bootstrapCacheDir(ulnclawHome), `install-${commit}.${process.platform === 'win32' ? 'ps1' : 'sh'}`)
}
...
function downloadInstallScript(ref, destPath) {
  ...
  const url = `https://raw.githubusercontent.com/NousResearch/ulnclaw-agent/${ref}/scripts/${scriptName}`
...
async function resolveInstallScript({
  ...
  const cached = cachedScriptPath(ulnclawHome, installRef.cacheKey)
  ...
  try {
    ...
    const installed = installedAgentInstallScript(ulnclawHome)
...
function spawnPowerShell(scriptPath, args, { emit, stageName, abortSignal, ulnclawHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
    ...
    const child = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Pass ULNCLAW_HOME through so install.ps1 respects the caller's
          // choice rather than re-computing the default.
          ULNCLAW_HOME: ulnclawHome || process.env.ULNCLAW_HOME || ''
        }
      })
    )
...
function spawnBash(scriptPath, args, { emit, stageName, abortSignal, ulnclawHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
...
function buildPosixPinArgs({ installStamp, activeRoot, ulnclawHome, pinCommit = true }) {
  const args = ['--dir', activeRoot, '--ulnclaw-home', ulnclawHome]
...
async function fetchManifest({ scriptPath, installerKind, emit, ulnclawHome, activeRoot, installStamp, pinCommit }) {
  const isPosix = installerKind === 'posix'

  const args = isPosix
    ? ['--manifest', ...buildPosixPinArgs({ installStamp, activeRoot, ulnclawHome, pinCommit })]
...
async function runStage({
  ...
  const args = isPosix
    ? [
        ...
        '--non-interactive',
        '--json',
        ...buildPosixPinArgs({ installStamp, activeRoot, ulnclawHome, pinCommit })
      ]
...
async function runBootstrap(opts) {
  ...
  const runLog = openRunLog(logRoot || path.join(ulnclawHome, 'logs'))
  ...
  try {
    ...
    const scriptInfo = await resolveInstallScript({ installStamp, sourceRepoRoot, ulnclawHome, emit })
