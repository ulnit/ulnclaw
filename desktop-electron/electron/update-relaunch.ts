 ...
 * Background
 * ----------
 * After `ulnclaw update` + `ulnclaw desktop --build-only`, the freshly-rebuilt
...
// backend/profile/root it talks to). Anything ULNCLAW_DESKTOP_* is preserved
// plus ULNCLAW_HOME. We snapshot the values, not the live env, so the new
...
const PRESERVED_ENV_KEYS = ['ULNCLAW_HOME', 'ELECTRON_DISABLE_SANDBOX']
const PRESERVED_ENV_PREFIXES = ['ULNCLAW_DESKTOP_']
