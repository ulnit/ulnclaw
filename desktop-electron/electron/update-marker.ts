...
export function markerPath(ulnclawHome) {
  return path.join(ulnclawHome, '.ulnclaw-update-in-progress')
}
...
export function readLiveUpdateMarker(
  ...
  const file = markerPath(ulnclawHome)
  let raw
 ...
 *
 * The Tauri-based ulnclaw-setup.exe takes several seconds to initialise its
 ...
 */
export function writeUpdateMarker(ulnclawHome, pid, { now = Date.now } = {}) {
  const file = markerPath(ulnclawHome)
 ...
 * ulnclaw to exit…") clobbers that still-running updater's claim: the
...
export function updateHandoffConflict(
  ...
  const owner = readLiveUpdateMarker(ulnclawHome, opts)
