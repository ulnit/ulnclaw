...
export interface ulnclawReadyOptions {
   ...
   * two very different meanings of a 401 (see `waitForulnclawReady`).
   */
...
export async function waitForulnclawReady(baseUrl: string, options: ulnclawReadyOptions): Promise<void> {
  ...
  throw new Error(`ulnclaw backend did not become ready: ${detail}`)
}
