 ...
 * remote-lifecycle.ts
 *
 * Pure, electron-free remote ulnclaw dashboard lifecycle over SSH for Desktop
 ...
 *
 *   - locate the ulnclaw install on the remote (login-shell probe),
 ...
 *     `ULNCLAW_DASHBOARD_READY port=<n>` readiness line,
 ...
 * the real SshConnection, fetch, adoptServedDashboardToken, and waitForulnclaw in.
 *
 * The minted ULNCLAW_DASHBOARD_SESSION_TOKEN is the SPAWN credential. After
...
const READY_RE = /^ULNCLAW_(?:BACKEND|DASHBOARD)_READY port=(\d+)/m
const REMOTE_LOCK_DIR = '~/.ulnclaw/desktop-ssh'
...
// Resolve the remote ulnclaw executable. An EXPLICIT path is honored strictly
...
async function locateulnclaw(ssh, remoteulnclawPath) {
  ...
  if (remoteulnclawPath) {
    if (await isExecutable(remoteulnclawPath)) {
      return resolveLauncher(remoteulnclawPath)
    }

    const err: any = new Error(
      `The ulnclaw path you set is not an executable on the remote host: "${remoteulnclawPath}". ` +
        'Check the path (it must be the full path to the `ulnclaw` binary on the remote, e.g. ' +
        '~/ulnclaw-agent/.venv/bin/ulnclaw), or clear it to auto-detect.'
    )

    err.kind = 'ulnclaw-not-found'
    throw err
  }
  ...
  try {
    const found = (await ssh.exec(`bash -lc ${shq('command -v ulnclaw')}`)).trim()
  ...
  candidates.push('~/.local/bin/ulnclaw')
  candidates.push('/usr/local/bin/ulnclaw')
  candidates.push('~/.ulnclaw/ulnclaw-agent/venv/bin/ulnclaw')
  ...
  const err: any = new Error(
    'ulnclaw is not installed on the remote host (could not find a `ulnclaw` executable). ' +
      'Install it on the remote with:  curl -fsSL https://ulnclaw-agent.nousresearch.com/install.sh | sh  ' +
      '— or set the ulnclaw path explicitly in the SSH connection settings.'
  )
...
// Probe the resolved binary's version string (first line of `<ulnclaw> --version`,
// e.g. "ulnclaw Agent v0.18.2 ..."), or '' on failure. Surfaces WHICH ulnclaw a
// connection uses, so a stale/unexpected install is visible.
async function probeulnclawVersion(ssh, ulnclawPath) {
  try {
    const out = (await ssh.exec(`${expandRemotePath(ulnclawPath)} --version 2>&1`)).trim()
...
async function probeRemotePlatform(ssh) {
  ...
  if (!SUPPORTED_REMOTE_OS.has(osName)) {
    const err: any = new Error(
      `Unsupported remote platform "${osName || 'unknown'}". ulnclaw Desktop SSH mode supports Linux, macOS, and Windows remote hosts.`
    )
...
// The ULNCLAW_HOME the remote dashboard will use (explicit env wins, else
// ~/.ulnclaw). Recorded in the lockfile so a future reuse can tell it's the same
// state store; best-effort.
async function probeRemoteulnclawHome(ssh) {
  try {
    const out = (await ssh.exec('echo "${ULNCLAW_HOME:-$HOME/.ulnclaw}"')).trim().split('\n').pop()

    return out || '~/.ulnclaw'
  } catch (cause) {
    const error: any = new Error('Could not resolve the remote ulnclaw home.')
...
async function readLockfile(ssh, ownershipId) {
  ...
  for (const field of ['profile', 'ulnclawPath', 'ulnclawHome', 'logPath', 'startedAt']) {
...
async function pidIsOurDashboard(ssh, pid, spawnNonce, ulnclawPath = '') {
  if (!pid || !/^[0-9a-f]{16}$/.test(String(spawnNonce || '')) || !ulnclawPath) {
    return false
  }
  ...
  try {
    const script =
      ...
      `pid=${Number(pid)}\n` +
      `expected=os.path.expanduser(${shq(ulnclawPath)})\n` +
...
async function cleanupStale(ssh, ownershipId, lock, pidAlive = true) {
  if (pidAlive && lock && (await pidIsOurDashboard(ssh, lock.pid, lock.spawnNonce, lock.ulnclawPath))) {
    try {
      const result = (
        await ssh.exec(
...
function buildSpawnCommand(ulnclawPath, profile, opts: any = {}) {
  const ulnclaw = expandRemotePath(ulnclawPath)
  ...
  const dashCmd = `env ULNCLAW_DESKTOP=1 ${ulnclaw} ${profileArgs}${subCmd}`
...
async function remoteSupportsSshOwnership(ssh, ulnclawPath) {
  const ulnclaw = expandRemotePath(ulnclawPath)

  const out = await ssh.exec(
    `help="$(${ulnclaw} serve --help 2>&1)"; ` +
...
async function spawnRemoteDashboard(ssh, { ulnclawPath, profile, token, ownershipId }) {
  if (!(await remoteSupportsSshOwnership(ssh, ulnclawPath))) {
    const err: any = new Error(
      'The remote ulnclaw install does not support --ssh-session-token-file and --ssh-owner-nonce. ' +
        'Update ulnclaw on the remote host to continue using Desktop SSH mode.'
    )
  ...
  try {
    out = await ssh.exec(buildSpawnCommand(ulnclawPath, profile, { spawnNonce, tokenFilePath, logPath }))
  } catch (error) {
    try {
      await ssh.exec(`rm -f ${expandRemotePath(tokenFilePath)}`)
    } catch {
 ...
 * opened SshConnection, forward/pickLocalPort/waitForulnclaw, a token-gated
...
async function connect(deps) {
  ...
  const ulnclawPath = await locateulnclaw(ssh, remoteulnclawPath)
  log(`located ulnclaw at ${ulnclawPath}`)
  const ulnclawVersion = await probeulnclawVersion(ssh, ulnclawPath)

  if (ulnclawVersion) {
    log(`remote ulnclaw version: ${ulnclawVersion}`)
  }

  const reuseToken = deps.reuseToken || ''
  const ulnclawHome = await probeRemoteulnclawHome(ssh)
  const lock = await readLockfile(ssh, ownershipId)

  if (lock) {
    const pidAlive = await remotePidAlive(ssh, lock.pid)
    const owned = pidAlive && (await pidIsOurDashboard(ssh, lock.pid, lock.spawnNonce, lock.ulnclawPath))
    ...
    const reusable =
      ...
      lock.ulnclawPath === ulnclawPath &&
      lock.ulnclawHome === ulnclawHome
  ...
  try {
    ...
    assertNotAborted(signal)
    const baseUrl = `http://127.0.0.1:${localPort}`
    await waitForulnclaw(baseUrl, spawnToken)
    assertNotAborted(signal)
