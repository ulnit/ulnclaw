/**
 * windows-ulnclaw-path.ts
 *
 * Pure, dependency-injected pieces of Windows `ulnclaw` resolution pulled out
 * of main.ts's findOnPath(), handOffWindowsBootstrapRecovery(), and
 * unwrapWindowsVenvulnclawCommand(). Each of the three functions here pins one
 ...
 *      FIRST, so an extensionless Git-Bash `ulnclaw` shim shadowed the real
 *      ulnclaw.cmd/ulnclaw.exe; the shim then failed the --version probe and
 ...
 *      venv\Scripts\ulnclaw.exe (the console-script shim, written at the END
 ...
 *   3. resolveVenvulnclawCommand() — unwrapWindowsVenvulnclawCommand() returned
 ...
 * extensionless file (e.g. a Git-Bash shell-script shim named `ulnclaw`) must
 * not shadow `ulnclaw.cmd`/`ulnclaw.exe`. The empty entry is kept LAST so
 ...
 * signals (venv python interpreter, venv ulnclaw shim, bootstrap-complete
 * marker) — gating on just the ulnclaw.exe console-script shim alone is the
...
export interface ResolveVenvulnclawCommandDeps {
  isWindows: boolean
  isCommandScript: (command: string) => boolean
  fileExists: (filePath: string) => boolean
  directoryExists: (filePath: string) => boolean
  canImportulnclawCli: (python: string, opts?: { env?: Record<string, string> }) => boolean
...
/**
 * If `command` is a Windows venv `ulnclaw`/`ulnclaw.exe` console-script shim
 * (i.e. `<venvRoot>/Scripts/ulnclaw(.exe)`), resolve it to the underlying
 * venv python invoked as `python -m ulnclaw_cli.main <backendArgs>` — but
 * ONLY after smoke-testing that interpreter with canImportulnclawCli(). A
 * venv whose update died mid-`pip install` still has python.exe + ulnclaw.exe
 ...
 *
 * Returns null when `command` is not a venv ulnclaw shim, the underlying
 * python doesn't exist, or the import probe fails. Otherwise returns the
 * resolved backend descriptor.
 */
export function resolveVenvulnclawCommand(
  command: string,
  backendArgs: string[],
  deps: ResolveVenvulnclawCommandDeps
): {
  label: string
  command: string
  args: string[]
  ...
  if (!/^ulnclaw(?:\.exe)?$/i.test(basename(resolved))) {
    return null
  }
  ...
  if (
    ...
    rememberLog?.(
      `Ignoring venv ulnclaw at ${python}: runtime import probe failed (broken/partial venv); falling through to bootstrap.`
    )
  ...
  return {
    label: `existing ulnclaw Python at ${python}`,
    command: python,
