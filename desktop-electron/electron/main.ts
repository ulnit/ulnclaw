...
import { buildDesktopBackendEnv, ulnclawManagedNodePathEntries, normalizeulnclawHomeRoot } from './backend-env'
import { isReauthRequiredError, waitForulnclawReady } from './backend-health'
import {
  canImportulnclawCli,
  execProbeSync,
  PROBE_TIMEOUT_MS,
...
import {
  ...
  resolveVenvulnclawCommand
} from './windows-ulnclaw-path'
import {
...
const DEV_SERVER = process.env.ULNCLAW_DESKTOP_DEV_SERVER
const IS_PACKAGED = app.isPackaged || Boolean(process.env.ULNCLAW_DESKTOP_IS_PACKAGED)
...
// switches only apply pre-launch. Override with ULNCLAW_DESKTOP_DISABLE_GPU
...
if (REMOTE_DISPLAY_REASON) {
  ...
  console.log(
    `[ulnclaw] remote display detected (${REMOTE_DISPLAY_REASON}); disabling GPU hardware acceleration to prevent flicker`
  )
}
...
if (DEV_CDP.port) {
  ...
  console.log(
    `[ulnclaw] renderer debugging on http://127.0.0.1:${DEV_CDP.port} — anything that can reach it ` +
      'can run code in the renderer. ULNCLAW_DESKTOP_CDP_PORT=off to disable.'
  )
} else {
  const why = describeDevCdpDecision(DEV_CDP)

  if (why) {
    console.warn(`[ulnclaw] ${why}`)
  }
}
...
if (IS_WSL && !REMOTE_DISPLAY_REASON && fs.existsSync('/dev/dxg')) {
  ...
  console.log('[ulnclaw] WSL GPU passthrough (/dev/dxg) detected; enabling GPU acceleration')
}
...
// never go through `ulnclaw desktop`; it is version-scoped so an app update
...
if (IS_WINDOWS) {
  ...
  // expose ulnclaw sessions/config to every packaged app on the machine.
  if (shouldAttemptAclRepair(priorMarker)) {
    const exeDir = path.dirname(process.execPath)
    const acl = grantAllApplicationPackagesAcl(exeDir, { execFileSync })

    if (acl.ok) {
      console.log(`[ulnclaw] granted ALL APPLICATION PACKAGES RX on ${exeDir} (#38216)`)
    } else if (acl.error && acl.error !== 'missing-target-or-exec') {
      console.warn(`[ulnclaw] AppContainer ACL grant failed on ${exeDir}: ${acl.error}`)
    }
  }
  ...
  if (sandboxDecision.enable && sandboxDecision.reason !== 'already-enabled') {
    ...
    console.log(
      `[ulnclaw] Windows sandbox fallback enabled (${sandboxDecision.reason}); launching with --no-sandbox (#38216)`
    )
  }
  ...
  app.on('child-process-gone', (_event, details) => {
    ...
    console.warn(
      `[ulnclaw] Windows GPU sandbox crashed (exit=${details?.exitCode}); relaunching once with --no-sandbox (#38216)`
    )
    ...
    try {
      ...
      console.error(`[ulnclaw] --no-sandbox relaunch failed: ${error?.message || error}`)
    }
  })
}

ipcMain.handle('ulnclaw:get-remote-display-reason', () => REMOTE_DISPLAY_REASON)
...
// always-on timers. A completely idle, minimized ulnclaw burned ~20% CPU
    ...
    try {
      ...
      if (parsed && typeof parsed === 'object' && typeof parsed.commit === 'string' && parsed.commit.length >= 7) {
        if (parsed.schemaVersion !== INSTALL_STAMP_SCHEMA_VERSION) {
          console.warn(
            `[ulnclaw] install-stamp.json schemaVersion ${parsed.schemaVersion} != expected ${INSTALL_STAMP_SCHEMA_VERSION}; ignoring`
          )
      ...
      console.warn(`[ulnclaw] install-stamp.json found at ${p} , but parsing failed with ${e}`)
...
if (INSTALL_STAMP) {
  console.log(
    `[ulnclaw] install stamp: ${INSTALL_STAMP.commit.slice(0, 12)}${INSTALL_STAMP.branch ? ` (${INSTALL_STAMP.branch})` : ''}${INSTALL_STAMP.dirty ? ' [DIRTY]' : ''} from ${INSTALL_STAMP.source || 'unknown'}`
  )
} else if (IS_PACKAGED) {
  ...
  console.error(
    '[ulnclaw] WARNING: no install-stamp.json found in packaged build. First-launch bootstrap will not have a pinned ref to install.'
  )
}

// ULNCLAW_HOME — the user-facing root for everything ulnclaw-related. Mirrors
// scripts/install.ps1's $ulnclawHome and scripts/install.sh's $ULNCLAW_HOME.
//
// Defaults:
//   Windows: %LOCALAPPDATA%\ulnclaw (matches install.ps1)
//   macOS / Linux: ~/.ulnclaw (matches install.sh)
//
// Special case for Windows: if the user has a legacy ~/.ulnclaw directory
// (e.g., from a prior pip install or a manual setup) AND no
// %LOCALAPPDATA%\ulnclaw yet, prefer the legacy path so we don't orphan their
// existing config / sessions / .env. New installs go to %LOCALAPPDATA%.
//
// ULNCLAW_DESKTOP_USER_DATA_DIR (used by test:desktop:fresh) puts the sandbox
// ULNCLAW_HOME beneath the throwaway userData dir so a fresh-install run never
// touches the user's real ~/.ulnclaw / %LOCALAPPDATA%\ulnclaw.
function resolveulnclawHome() {
  if (process.env.ULNCLAW_HOME) {
    return normalizeulnclawHomeRoot(process.env.ULNCLAW_HOME)
  }

  if (USER_DATA_OVERRIDE) {
    return path.join(path.resolve(USER_DATA_OVERRIDE), 'ulnclaw-home')
  }

  if (IS_WINDOWS) {
    // A GUI app launched from Explorer inherits the environment block captured
    // at login, so a ULNCLAW_HOME set via `setx` AFTER login is invisible in
    // process.env even though the CLI (a fresh shell) sees it. Without this the
    // backend silently falls back to %LOCALAPPDATA%\ulnclaw and reports "No
    // inference provider configured" despite a valid configured home (#45471).
    // Consult the live User-scoped registry value before the default below.
    const fromRegistry = readWindowsUserEnvVar('ULNCLAW_HOME')

    if (fromRegistry) {
      return normalizeulnclawHomeRoot(fromRegistry)
    }
  }

  if (IS_WINDOWS && process.env.LOCALAPPDATA) {
    const localappdata = path.join(process.env.LOCALAPPDATA, 'ulnclaw')
    const legacy = path.join(app.getPath('home'), '.ulnclaw')

    // Migrate transparently to LOCALAPPDATA, but honour an existing legacy
    // ~/.ulnclaw setup (no LOCALAPPDATA install yet) so users don't lose state.
  ...
  return path.join(app.getPath('home'), '.ulnclaw')
}

const ULNCLAW_HOME = resolveulnclawHome()

function pathWithulnclawManagedNode(...entries) {
  const managed = ulnclawManagedNodePathEntries(ULNCLAW_HOME).filter(directoryExists)

  return [...managed, ...entries, process.env.PATH].filter(Boolean).join(path.delimiter)
}

// ACTIVE_ULNCLAW_ROOT — the canonical mutable ulnclaw install. Same path
// install.ps1 / install.sh use, so a desktop-only user and a CLI-only user end
// up with identical layouts and can share one install.
const ACTIVE_ULNCLAW_ROOT = path.join(ULNCLAW_HOME, 'ulnclaw-agent')
// VENV_ROOT — venv lives inside the repo, exactly like install.ps1 does it.
const VENV_ROOT = path.join(ACTIVE_ULNCLAW_ROOT, 'venv')
...
// boots, going straight to `resolveulnclawBackend()`. Missing or stale marker
...
//
// We deliberately put the marker INSIDE ACTIVE_ULNCLAW_ROOT (not alongside)
...
const BOOTSTRAP_COMPLETE_MARKER = path.join(ACTIVE_ULNCLAW_ROOT, '.ulnclaw-bootstrap-complete')
...
// active-profile.json records which ulnclaw profile the desktop launches its
// local backend as. When set, startulnclaw() passes `ulnclaw --profile <name>
// dashboard …`, which deterministically pins ULNCLAW_HOME (see
// _apply_profile_override in ulnclaw_cli/main.py) and bypasses the sticky
// ~/.ulnclaw/active_profile file. Unset (null) preserves the legacy behavior:
// no --profile flag, so the backend honors active_profile / default.
const DESKTOP_PROFILE_CONFIG_PATH = path.join(app.getPath('userData'), 'active-profile.json')
// Mirrors ulnclaw_cli.profiles._PROFILE_ID_RE so we never hand the backend a
// value its profile resolver would reject and exit on.
const PROFILE_NAME_RE = /^[a-z0-9][a-z0-9_-]{0,63}$/
// Branch we track for self-update. The GUI work has merged to main, so this
// tracks main. User can also override at runtime via
// ulnclawDesktop.updates.setBranch().
const DEFAULT_UPDATE_BRANCH = 'main'
// desktop.log lives under ULNCLAW_HOME/logs/ so it sits next to agent.log,
// errors.log, gateway.log produced by ulnclaw_logging.setup_logging — one log
// directory per user, regardless of which UI surface produced the line.
const DESKTOP_LOG_PATH = path.join(ULNCLAW_HOME, 'logs', 'desktop.log')
...
//
// Mirror the Python logs (ulnclaw_logging.py RotatingFileHandler, maxBytes x
...
const BOOT_FAKE_MODE = process.env.ULNCLAW_DESKTOP_BOOT_FAKE === '1'
const BOOT_FAKE_ERROR = process.env.ULNCLAW_DESKTOP_BOOT_FAKE_ERROR || ''
...
const SKIP_QUIT_CONFIRM = process.env.ULNCLAW_DESKTOP_SKIP_QUIT_CONFIRM === '1'

const BOOT_FAKE_STEP_MS = (() => {
  const raw = Number.parseInt(String(process.env.ULNCLAW_DESKTOP_BOOT_FAKE_STEP_MS || ''), 10)
...
const APP_NAME = process.env.ULNCLAW_DESKTOP_APP_NAME || 'ulnclaw'
...
// 'ulnclaw:native-theme' ('dark' | 'light' | 'system'); we pin
...
// electron-builder derives from the build `appId` (com.nousresearch.ulnclaw) —
// keep this string in sync with package.json `build.appId`. macOS/Linux don't
// need this, so gate it on Windows. (Fixes: desktop approval/turn notifications
// never firing on Windows.)
if (IS_WINDOWS) {
  app.setAppUserModelId('com.nousresearch.ulnclaw')
}

// Seed the native About panel with the live ulnclaw version. This is refreshed
// on every open via the explicit "About" menu handler (refreshAboutPanel), so
// an in-place `ulnclaw update` mid-session is reflected without an app restart;
// the seed here just covers the first open and any non-menu invocation path.
app.setAboutPanelOptions({
  applicationName: APP_NAME,
  applicationVersion: resolveulnclawVersion(),
...
const MEDIA_PROTOCOL = 'ulnclaw-media'

// Only audio/video may be streamed. Without this the handler would read any
// non-blocklisted local file (no size cap) for any `fetch(ulnclaw-media://…)`.
...
// startulnclaw(); this pool only holds EXTRA profile
...
const POOL_MAX_BACKENDS = Math.max(1, Number(process.env.ULNCLAW_DESKTOP_POOL_MAX) || 3)
const POOL_IDLE_MS = Math.max(60_000, Number(process.env.ULNCLAW_DESKTOP_POOL_IDLE_MS) || 10 * 60_000)
...
// onto the error so subsequent startulnclaw() calls (e.g. the renderer's
// ensureGatewayOpen retrying after the WS won't open) return the same error
// instead of re-running install.ps1 in a hot loop. Cleared explicitly by
// the renderer's "Reload and retry" path or by quitting the app.
let bootstrapFailure = null
// Latched non-bootstrap backend spawn failure — stops getConnection() from
// respawning ulnclaw serve backend children in a tight loop while boot is broken.
...
let bootProgressState = {
  ...
  message: 'Waiting to start ulnclaw backend',
  phase: 'idle',
  progress: 0,
  running: false,
  timestamp: Date.now()
...
function rememberLog(chunk) {
  ...
  const lines = text.split(/\r?\n/).map(line => `[ulnclaw] ${line}`)
  ulnclawLog.push(...lines)

  if (ulnclawLog.length > 300) {
    ulnclawLog.splice(0, ulnclawLog.length - 300)
  }
...
function ensureWslWindowsFonts() {
  ...
  try {
    ...
    const confPath = path.join(confDir, '99-ulnclaw-wsl-windows-fonts.conf')
    let existing = ''
...
function broadcastBootProgress() {
  ...
  webContents.send('ulnclaw:boot-progress', bootProgressState)
}
...
//
// The snapshot is queryable via the ulnclaw:bootstrap:get IPC handler so a
...
function broadcastBootstrapEvent(ev) {
  ...
  webContents.send('ulnclaw:bootstrap:event', ev)
}
...
function promptFirstRunSetupChoice(backend) {
  broadcastBootstrapEvent({
    ...
    activeRoot: backend.activeRoot || ACTIVE_ULNCLAW_ROOT
  })
}
...
// The Tauri updater writes ULNCLAW_HOME/.ulnclaw-update-in-progress for the whole
...
// (`force_kill_other_ulnclaw`, taskkill /IM ulnclaw.exe) kills it, the launch
...
function updateGateDeps() {
  return {
    hasLiveMarker: () => Boolean(readLiveUpdateMarker(ULNCLAW_HOME)),
  ...
  const outcome = await waitForUpdateClearance(updateGateDeps(), {
    onWaitTick: async reason => {
      ...
      await advanceBootProgress(
        'backend.update-wait',
        'An update is finishing — ulnclaw will start automatically when it completes…',
        12
      )
    },
...
function findOnPath(command) {
  ...
  // shell-script shim named `ulnclaw` — must not shadow `ulnclaw.cmd`/`ulnclaw.exe`.
...
function unwrapWindowsVenvulnclawCommand(command, backendArgs) {
  return resolveVenvulnclawCommand(command, backendArgs, {
...
// spawns `ulnclaw serve`; runtimes older than serve only have `dashboard`. We
...
// (covers a bare `ulnclaw` resolved from PATH with no known source root). Result
...
function backendSupportsServe(backend) {
  ...
  if (backend.root) {
    try {
      const src = fs.readFileSync(path.join(backend.root, 'ulnclaw_cli', 'subcommands', 'dashboard.py'), 'utf8')
  ...
  if (supported === null) {
    try {
      ...
      // `ulnclaw --version` (~10.5s measured cold), and a false negative here
      ...
      execProbeSync(backend.command, [...prefix, 'serve', '--help'], {
        ...
        env: { ...process.env, ULNCLAW_HOME, ...(backend.env || {}) },
        timeout: PROBE_TIMEOUT_MS,
        stdio: 'ignore',
        // `.cmd`/`.bat` shim backends carry shell: true in their descriptor
        // (see resolveulnclawBackend step 4); execFileSync of a .cmd without
...
function isulnclawSourceRoot(root) {
  return directoryExists(root) && fileExists(path.join(root, 'ulnclaw_cli', 'main.py'))
}

function findPythonForRoot(root) {
  const override = process.env.ULNCLAW_DESKTOP_PYTHON
...
function findSystemPython() {
  ...
  // CPython but several ulnclaw deps (notably pywinpty's Rust-built
      ...
      try {
        const out = execFileSync(
          ...
          hiddenWindowsChildOptions({
            ...
            // Bare interpreter startup — much lighter than the ulnclaw-import
...
// findGitBash — locate bash.exe on Windows. Resolves ULNCLAW_GIT_BASH_PATH
...
// normal ULNCLAW_DASHBOARD_READY stdout line and no ready-file side channel is
// needed.
...
// install only has PortableGit under %LOCALAPPDATA%\ulnclaw\git (never on
...
function resolveGitBinary() {
  ...
  if (localAppData) {
    candidates.push(path.join(localAppData, 'ulnclaw', 'git', 'cmd', 'git.exe'))
    candidates.push(path.join(localAppData, 'ulnclaw', 'git', 'bin', 'git.exe'))
  }
...
function recentulnclawLog() {
  return ulnclawLog.slice(-20).join('\n')
}

// ─── Self-update (git-pull against the running backend's ulnclaw root) ──────
...
// Dev → SOURCE_REPO_ROOT. Packaged/CLI install → ACTIVE_ULNCLAW_ROOT.
// ULNCLAW_DESKTOP_ULNCLAW_ROOT always wins so devs can pin a worktree.
function resolveUpdateRoot() {
  const candidates = [
    process.env.ULNCLAW_DESKTOP_ULNCLAW_ROOT && path.resolve(process.env.ULNCLAW_DESKTOP_ULNCLAW_ROOT),
    !IS_PACKAGED && isulnclawSourceRoot(SOURCE_REPO_ROOT) ? SOURCE_REPO_ROOT : null,
    isulnclawSourceRoot(ACTIVE_ULNCLAW_ROOT) ? ACTIVE_ULNCLAW_ROOT : null
  ].filter(Boolean)

  return candidates.find(c => directoryExists(path.join(c, '.git'))) || candidates[0] || ACTIVE_ULNCLAW_ROOT
}
...
function emitUpdateProgress(payload) {
  ...
  for (const window of BrowserWindow.getAllWindows()) {
    window.webContents.send('ulnclaw:updates:progress', merged)
  }
}
...
// Windows that binary owns ALL repo mutation — running `ulnclaw update` +
...
function resolveUpdaterBinary() {
  return resolveStagedUpdaterBinary(ULNCLAW_HOME, { fileExists, isWindows: IS_WINDOWS })
}
...
// Path to the venv shim whose lock decides whether `ulnclaw update` can write
// fresh entry points. On Windows this is the file the running backend
// `ulnclaw.exe` holds open; on POSIX it's never mandatory-locked.
function venvulnclawShimPath(updateRoot) {
  return IS_WINDOWS
    ? path.join(updateRoot, 'venv', 'Scripts', 'ulnclaw.exe')
    : path.join(updateRoot, 'venv', 'bin', 'ulnclaw')
}
...
// only signals the direct child, so on Windows a backend `ulnclaw.exe` that
// spawned its own grandchildren (a `ulnclaw` REPL, a pty terminal session, the
...
// `ulnclawProcess.kill('SIGTERM')` + `app.quit()` fire-and-forget: SIGTERM on
// Windows doesn't reap the backend's grandchildren, and quit didn't wait for
// teardown, so the updater raced a still-locked `ulnclaw.exe`, the quarantine
...
// problem: the desktop's backend (and the grandchildren IT spawned — a ulnclaw
// REPL, a pty terminal, the gateway) keep `ulnclaw.exe` and other files in the
...
async function releaseBackendLock(updateRoot, tag) {
  ...
  const pids = []
  const ulnclawProcess = backendConnectionState.getProcess()

  if (ulnclawProcess && Number.isInteger(ulnclawProcess.pid)) {
    pids.push(ulnclawProcess.pid)
  }
  ...
  if (ulnclawProcess && !ulnclawProcess.killed) {
    try {
      ulnclawProcess.kill('SIGTERM')
    } catch {
      void 0
    }
  }
  ...
  const shim = venvulnclawShimPath(updateRoot)
  ...
  while (Date.now() < deadlineMs) {
    ...
    const currentulnclawProcess = backendConnectionState.getProcess()

    if (currentulnclawProcess && Number.isInteger(currentulnclawProcess.pid)) {
      stragglers.push(currentulnclawProcess.pid)
    }
...
// `ulnclaw update`). Instead we spawn the staged ulnclaw-Setup binary with
// --update and quit, so it can run `ulnclaw update` (which refuses while we
...
async function applyUpdates(opts = {}) {
  ...
  try {
    ...
    if (!updater && !IS_WINDOWS) {
      // macOS/Linux: never hand off, staged ulnclaw-setup or not — the resolver
      ...
      // `ulnclaw update` (backend) + `ulnclaw desktop --build-only` (OS-aware GUI
    ...
    if (!updater) {
      ...
      // `ulnclaw desktop`, never the Tauri installer that self-copies
      // ulnclaw-setup.exe into ULNCLAW_HOME). They DO have a working `ulnclaw`
      ...
      // checkout they're on — bare `ulnclaw update` defaults to main and would
      ...
      try {
        ...
        if (head.code === 0 && current && current !== 'HEAD') {
          ...
          if (branch !== 'main') {
            command = `ulnclaw update --branch ${branch}`
          }
        }
      } catch {
        // Best-effort: fall back to bare `ulnclaw update` if branch detection fails.
      }

      rememberLog(`[updates] no staged updater; surfacing manual \`${command}\` for CLI install at ${updateRoot}`)
      emitUpdateProgress({ stage: 'manual', message: command, percent: null })

      return { ok: true, manual: true, command, ulnclawRoot: updateRoot }
    }

    const handoffConflict = updateHandoffConflict(ULNCLAW_HOME)
    ...
    emitUpdateProgress({
      stage: 'restart',
      message:
        'Updating ulnclaw — this window will close and the updater will open. Don’t reopen ulnclaw yourself; it restarts automatically when the update finishes.',
      percent: 100
    })
    ...
    preflightStateDb(ULNCLAW_HOME, rememberLog)

    // Stop our own backend(s) and wait for the venv shim to unlock BEFORE we
    // spawn the updater. Without this the updater races a still-locked
    // ulnclaw.exe (held by the backend child / its grandchildren) and the update
    ...
    if (!lock.unlocked) {
      ...
      // terminal running ulnclaw, an unkillable child). Handing off anyway
      ...
      const message =
        'Update aborted: another process is holding the ulnclaw install open ' +
        '(a second ulnclaw window or a terminal running ulnclaw?). Close it and retry.'
    ...
    // ulnclaw processes running from this venv.  The updater normally refuses
    ...
    // `ulnclaw update` will run (the venv shim is locked while we live).
    const child = spawnUpdaterProcess(updater, updaterArgs, {
      cwd: ULNCLAW_HOME,
      env: {
        ...process.env,
        ULNCLAW_HOME,
        PATH: pathWithulnclawManagedNode(venvBin)
      },
      detached: true,
      stdio: 'ignore'
    })
    ...
    // abort with "Another ulnclaw update is already running (PID <itself>)" —
    ...
    if (Number.isInteger(child.pid) && stagedUpdaterSupportsPrewrittenMarker(updater)) {
      writeUpdateMarker(ULNCLAW_HOME, child.pid)
...
async function handOffWindowsBootstrapRecovery(reason) {
  ...
  const venvBin = path.join(updateRoot, 'venv', IS_WINDOWS ? 'Scripts' : 'bin')
  const venvulnclaw = path.join(venvBin, IS_WINDOWS ? 'ulnclaw.exe' : 'ulnclaw')
  ...
  // not just the `ulnclaw.exe` console-script shim. That shim is generated at the
  ...
  const haveRealInstall =
    fileExists(venvPython) || fileExists(venvulnclaw) || fileExists(path.join(updateRoot, '.ulnclaw-bootstrap-complete'))
...
// Resolve the ulnclaw CLI to drive an in-app update: prefer the venv shim in
// the install we're updating, fall back to `ulnclaw` on PATH.
function resolveulnclawCliBinary(updateRoot) {
  const venvulnclaw = path.join(updateRoot, 'venv', 'bin', 'ulnclaw')

  if (fileExists(venvulnclaw)) {
    return venvulnclaw
  }

  return findOnPath('ulnclaw') || null
}
...
// Python-level pre-update snapshot inside `ulnclaw update`.
function preflightStateDb(ulnclawHome, rememberLog) {
  const stateDbPath = path.join(ulnclawHome, 'state.db')
  ...
  try {
    ...
    if (stat.size > 100) {
      ...
      const emergencyPath = path.join(ulnclawHome, `state.db.pre-update-emergency-${ts}.bak`)
      ...
      try {
        ...
        try {
          const homeDir = fs.readdirSync(ulnclawHome)
          ...
          for (const old of backups.slice(2)) {
            try {
              fs.unlinkSync(path.join(ulnclawHome, old))
            } catch {
              void 0
            }
          }
...
// macOS/Linux in-app update: backend (`ulnclaw update`) + OS-aware GUI rebuild
// (`ulnclaw desktop --build-only`), then atomically swap the running .app bundle
...
async function applyUpdatesPosixInApp(opts: any) {
  const updateRoot = resolveUpdateRoot()
  const ulnclaw = resolveulnclawCliBinary(updateRoot)

  if (!ulnclaw) {
    emitUpdateProgress({ stage: 'manual', message: 'ulnclaw update', percent: null })

    return { ok: true, manual: true, command: 'ulnclaw update', ulnclawRoot: updateRoot }
  }

  // ── Pre-flight state.db integrity guard (#68474) ──
  preflightStateDb(ULNCLAW_HOME, rememberLog)

  // Put the ulnclaw-managed Node and the venv on PATH so `ulnclaw desktop`'s
  // npm build can find them on a machine with no system Node. Windows portable
  // Node lives directly under %LOCALAPPDATA%\\ulnclaw\\node, not node\\bin.
  // PYTHONUNBUFFERED: `ulnclaw update` writes to a pipe here, so CPython
  ...
  const env: Record<string, string> = {
    ULNCLAW_HOME,
    PYTHONUNBUFFERED: '1',
    PATH: pathWithulnclawManagedNode(path.join(updateRoot, 'venv', 'bin'))
  }

  // `ulnclaw update` reaps stale `ulnclaw serve` backends (a code update
  ...
  // _kill_stale_dashboard_processes reads ULNCLAW_DESKTOP_CHILD_PID and excludes
  ...
  if (ulnclawProcess && Number.isInteger(ulnclawProcess.pid)) {
    desktopChildPids.push(ulnclawProcess.pid)
  }
  ...
  if (desktopChildPids.length) {
    env.ULNCLAW_DESKTOP_CHILD_PID = desktopChildPids.join(',')
  }
  ...
  emitUpdateProgress({ stage: 'update', message: 'Updating ulnclaw (git + dependencies)…', percent: 10 })

  const updated = (await runStreamedUpdate(ulnclaw, ['update', '--yes', ...branchArgs], {
    cwd: updateRoot,
    env,
    stage: 'update'
  })) as any
  ...
  if (updated.code !== 0) {
    emitUpdateProgress({ stage: 'error', message: 'ulnclaw update failed.', error: updated.error || 'update-failed' })

    return { ok: false, error: 'ulnclaw update failed' }
  }
  ...
  const rebuilt = await runRebuildWithRetry(attempt => {
    ...
    return runStreamedUpdate(ulnclaw, ['desktop', '--build-only'], { cwd: updateRoot, env, stage: 'rebuild' })
  })

  if (rebuilt.code !== 0) {
    emitUpdateProgress({
      stage: 'error',
      message: 'Backend updated, but the desktop rebuild failed. Restart ulnclaw to retry.',
  ...
  // Linux in-app update terminal state (#45205). `ulnclaw desktop --build-only`
  ...
  if (!IS_MAC) {
    ...
    if (outcome === 'relaunch') {
      emitUpdateProgress({ stage: 'restart', message: 'Restarting ulnclaw…', percent: 100 })
      ...
      const scriptPath = path.join(app.getPath('temp'), `ulnclaw-desktop-update-${Date.now()}.sh`)
      ...
      try {
        ...
        return {
          ...
          guiUpdated: false,
          manualRestart: true,
          message: 'Backend updated. Quit and reopen ulnclaw to load the new version.'
        }
      }
    }
    ...
    if (outcome === 'guiSkew') {
      emitUpdateProgress({
        ...
        message:
          ...
          'Update or reinstall the ulnclaw desktop app to match.',
        percent: 100
      })
      rememberLog(
    ...
    return {
      ...
      message:
        ...
        '(sandbox helper needs root). Quit and reopen ulnclaw to finish.'
    }
  }

  const rebuiltApp = [
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac-arm64', 'ulnclaw.app'),
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac', 'ulnclaw.app')
  ...
  if (!rebuiltApp || !targetApp) {
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend updated. Restart ulnclaw to load the new version.',
      percent: 100
    })
...
if [ "$SRC" != "$DST" ]; then
  if /usr/bin/ditto "$SRC" "$DST.ulnclaw-update-new"; then
    rm -rf "$DST.ulnclaw-update-old" 2>/dev/null || true
    mv "$DST" "$DST.ulnclaw-update-old" 2>/dev/null || rm -rf "$DST"
    mv "$DST.ulnclaw-update-new" "$DST"
    rm -rf "$DST.ulnclaw-update-old" 2>/dev/null || true
  fi
fi
...
`
  ...
  try {
    ...
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend + app updated. Restart ulnclaw to load the new version.',
      percent: 100
    })
...
// Marker-independent: is the canonical install at ACTIVE_ULNCLAW_ROOT actually
...
function isActiveRuntimeUsable() {
  ...
  return (
    isulnclawSourceRoot(ACTIVE_ULNCLAW_ROOT) &&
    fileExists(venvPython) &&
    canImportulnclawCli(venvPython, {
      env: {
        PYTHONPATH: [ACTIVE_ULNCLAW_ROOT, process.env.PYTHONPATH].filter(Boolean).join(path.delimiter)
      }
    })
  )
}
...
function activeRuntimeState() {
  ...
  // pinned commit -- users update via the in-app update path or `ulnclaw
...
function resolveWebDist() {
  const override = process.env.ULNCLAW_DESKTOP_WEB_DIST
  ...
  if (IS_PACKAGED && /app\.asar(?=$|[\\/])/.test(fallback) && !directoryExists(fallback)) {
    rememberLog(
      `[web-dist] dashboard frontend dir resolved to an asar-internal path that ` +
        ...
        `Ensure dist/** is unpacked (asarUnpack) or set ULNCLAW_DESKTOP_WEB_DIST.`
    )
  }
...
function resolveRendererIndex() {
  ...
  rememberLog(
    `[renderer] index.html not found — the desktop app was packaged without a ` +
      ...
      `Rebuild with: ulnclaw desktop --force-build`
  )
...
function resolveulnclawCwd() {
  // In a packaged build, `process.cwd()` resolves to the install root (e.g.
  // `…/win-unpacked` on Windows or `/Applications/ulnclaw.app/Contents/...`
  ...
  const candidates = [
    ...
    process.env.ULNCLAW_DESKTOP_CWD,
...
function sanitizeWorkspaceCwd(cwd) {
  ...
  if (!trimmed || isPackagedInstallPath(trimmed)) {
    return { cwd: resolveulnclawCwd(), sanitized: Boolean(trimmed) }
  }
...
function createPythonBackend(root, label, backendArgs, options: any = {}) {
  ...
  return {
    ...
    label,
    command,
    args: ['-m', 'ulnclaw_cli.main', ...backendArgs],
...
// createActiveBackend — build a backend pointing at ACTIVE_ULNCLAW_ROOT, the
...
function createActiveBackend(backendArgs) {
  ...
  return {
    kind: 'python',
    label: `ulnclaw at ${ACTIVE_ULNCLAW_ROOT}`,
    command,
    args: ['-m', 'ulnclaw_cli.main', ...backendArgs],
    env: buildDesktopBackendEnv({
      ulnclawHome: ULNCLAW_HOME,
      pythonPathEntries: [ACTIVE_ULNCLAW_ROOT, ...getVenvSitePackagesEntries(VENV_ROOT)],
      venvRoot: VENV_ROOT
    }),
    root: ACTIVE_ULNCLAW_ROOT,
    bootstrap: true,
    shell: false
  }
}

function resolveulnclawBackend(backendArgs) {
  // 1. Explicit override -- ULNCLAW_DESKTOP_ULNCLAW_ROOT points at a developer
  //    checkout. Honour it as-is (no bootstrap; the user is driving).
  const overrideRoot = process.env.ULNCLAW_DESKTOP_ULNCLAW_ROOT && path.resolve(process.env.ULNCLAW_DESKTOP_ULNCLAW_ROOT)

  if (overrideRoot && isulnclawSourceRoot(overrideRoot)) {
    const backend = createPythonBackend(overrideRoot, `ulnclaw source at ${overrideRoot}`, backendArgs)
  ...
  //    installed `ulnclaw` on PATH so local Python edits are actually exercised.
  //    (In dev with no checkout, SOURCE_REPO_ROOT won't pass isulnclawSourceRoot.)
  if (!IS_PACKAGED && isulnclawSourceRoot(SOURCE_REPO_ROOT)) {
    const backend = createPythonBackend(SOURCE_REPO_ROOT, `ulnclaw source at ${SOURCE_REPO_ROOT}`, backendArgs)
  ...
  // 3. ACTIVE_ULNCLAW_ROOT — the canonical install at
  //    %LOCALAPPDATA%\\ulnclaw\\ulnclaw-agent (Windows) or ~/.ulnclaw/ulnclaw-agent.
  ...
  if (activeRuntime.shouldUseActiveRuntime && !bootstrapRepairRequested) {
    if (!activeRuntime.hasValidMarker) {
      rememberLog(
        `[bootstrap] Active ulnclaw runtime at ${ACTIVE_ULNCLAW_ROOT} is usable but the bootstrap marker is missing or stale; skipping first-run bootstrap.`
      )
    }
  ...
  // 4. Existing `ulnclaw` on PATH -- installed via install.ps1 / install.sh from
  //    a previous tool-only setup, or pip-installed system-wide. Use it but
  //    do NOT write a bootstrap marker; the user did this themselves and we
  //    don't want to take ownership of an install we didn't perform.
  //    ULNCLAW_DESKTOP_IGNORE_EXISTING=1 forces the bootstrap path for testing.
  if (process.env.ULNCLAW_DESKTOP_IGNORE_EXISTING !== '1') {
    let ulnclawCommand = null
    const ulnclawOverride = process.env.ULNCLAW_DESKTOP_ULNCLAW

    if (ulnclawOverride) {
      const resolvedOverride = findOnPath(ulnclawOverride)

      if (resolvedOverride) {
        ulnclawCommand = resolvedOverride
      } else if (!isWindowsBinaryPathInWsl(ulnclawOverride, { isWsl: IS_WSL })) {
        ulnclawCommand = ulnclawOverride
      } else {
    ...
    if (ulnclawCommand) {
      if (looksLikeDesktopAppBinary(ulnclawCommand)) {
        rememberLog(`Ignoring desktop app executable on PATH while resolving ulnclaw CLI: ${ulnclawCommand}`)
        ulnclawCommand = null
      }
    }

    if (ulnclawCommand) {
      const unwrapped = unwrapWindowsVenvulnclawCommand(ulnclawCommand, backendArgs)

      if (unwrapped) {
        return unwrapped
      }

      // Smoke-test the candidate before trusting it. A `ulnclaw` shim
      ...
      const shellForProbe = isCommandScript(ulnclawCommand)

      // ULNCLAW_DESKTOP_ULNCLAW is an explicit deployment override (used by
      ...
      if (shouldTrustulnclawOverride(ulnclawOverride) || verifyulnclawCli(ulnclawCommand, { shell: shellForProbe })) {
        ...
        // unwrapWindowsVenvulnclawCommand here: the second call repeats the
        // same un-memoized import probe, costing up to another full probe
        // timeout on the boot path for an answer we already have.
        return {
          label: `existing ulnclaw CLI at ${ulnclawCommand}`,
          command: ulnclawCommand,
          args: backendArgs,
          bootstrap: false,
          env: {},
          kind: 'command',
      ...
      rememberLog(
        `Ignoring existing ulnclaw CLI at ${ulnclawCommand}: --version probe failed; falling through to bootstrap.`
      )
    }
  }

  // 5. Last-ditch: pip-installed ulnclaw_cli module via system Python.
  ...
  if (python) {
    ...
    // having ulnclaw_cli installed -- common on dev boxes that have
    ...
    // a uv-managed 3.11 into %LOCALAPPDATA%\ulnclaw\ulnclaw-agent\venv.
    if (canImportulnclawCli(python)) {
      return {
        kind: 'python',
        label: `installed ulnclaw_cli module via ${python}`,
        command: python,
        args: ['-m', 'ulnclaw_cli.main', ...backendArgs],
        bootstrap: false,
        env: {},
    ...
    rememberLog(`Ignoring system Python ${python}: ulnclaw_cli is not importable; falling through to bootstrap.`)
  }
  ...
  //    resolveulnclawBackend was the old "no payload" path and forced the
  //    user into a dead end. With the bootstrap protocol, "no install yet"
  //    is a recoverable state the GUI can drive through.
  return {
    kind: 'bootstrap-needed',
    label: 'ulnclaw Agent not installed yet; bootstrap required',
    command: null,
    args: backendArgs,
    bootstrap: true,
    env: {},
    ...
    activeRoot: ACTIVE_ULNCLAW_ROOT,
...
async function ensureRuntime(backend) {
  ...
  // backend.kind === 'bootstrap-needed' means resolveulnclawBackend couldn't
  ...
  if (backend.kind === 'bootstrap-needed') {
    rememberLog('[bootstrap] no ulnclaw install found; starting first-launch bootstrap')

    if (await handOffWindowsBootstrapRecovery('bootstrap-needed')) {
      const handoffError: Error & { isBootstrapFailure?: boolean; bootstrapHandedOff?: boolean } = new Error(
        'ulnclaw recovery was handed off to ulnclaw Setup. The desktop will restart when recovery completes.'
      )
    ...
    const bootstrapResult = await runBootstrap({
      ...
      ulnclawHome: ULNCLAW_HOME,
      logRoot: path.join(ULNCLAW_HOME, 'logs'),
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('ulnclaw install was cancelled.') as any
    ...
    if (!bootstrapResult.ok) {
      const bootstrapError = new Error(
        `ulnclaw bootstrap failed${bootstrapResult.failedStage ? ` at stage '${bootstrapResult.failedStage}'` : ''}: ` +
          `${bootstrapResult.error || 'unknown error'}. ` +
          `Check ${path.join(ULNCLAW_HOME, 'logs', 'desktop.log')} for the full transcript.`
      ) as any

      bootstrapError.isBootstrapFailure = true
      bootstrapError.failedStage = bootstrapResult.failedStage || null
      // Latch the failure so subsequent startulnclaw() calls return this
      // same error without re-running install.ps1.  Cleared by the
      // ulnclaw:bootstrap:reset IPC (renderer's "Reload and retry").
    ...
    return ensureRuntime(resolveulnclawBackend(backend.args))
  }
  ...
  if (!isulnclawSourceRoot(ACTIVE_ULNCLAW_ROOT)) {
    throw new Error(
      `ulnclaw install at ${ACTIVE_ULNCLAW_ROOT} is missing or incomplete. ` +
        'Reinstall via the desktop installer or scripts/install.ps1.'
    )
  }

  // On Windows, preflight Git Bash. ulnclaw' terminal tool calls bash.exe
  ...
  // %LOCALAPPDATA%\ulnclaw\git\, which findGitBash() picks up, so for any
  // user who completed the bootstrap this is a no-op. For users who got
  // here via an external `ulnclaw` on PATH, this check still helps.
  if (IS_WINDOWS && !findGitBash()) {
    throw new Error(
      'Git for Windows is required for ulnclaw on Windows (provides Git Bash, ' +
        "which the agent's terminal tool uses). Install it from " +
        'https://git-scm.com/download/win or run `winget install -e --id Git.Git`, ' +
        'then relaunch ulnclaw.'
    )
  }
  ...
  if (!fileExists(venvPython)) {
    ...
    // normal flow because activeRuntimeState() requires isulnclawSourceRoot()
    // plus an importable ulnclaw_cli before it hands back the active runtime.
    // If we hit this, the user (or a deleted venv) broke the invariant; tell
    // them to re-run the install.
    throw new Error(
      `ulnclaw venv missing at ${VENV_ROOT}. Re-run the desktop installer or ` + '`scripts/install.ps1` to rebuild it.'
    )
  }

  backend.command = getVenvPython(VENV_ROOT)
  backend.label = `ulnclaw at ${ACTIVE_ULNCLAW_ROOT} (venv: ${VENV_ROOT})`
  updateBootProgress({
    phase: 'runtime.ready',
    message: 'ulnclaw runtime is ready',
    progress: 82,
    running: true,
    error: null
  })
...
function multipartBody(upload) {
  const boundary = `----ulnclaw-${crypto.randomBytes(12).toString('hex')}`
...
function fetchJson(url, token, options: any = {}) {
  return new Promise((resolve, reject) => {
    ...
    if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
      reject(new Error(`Unsupported ulnclaw backend URL protocol: ${parsed.protocol}`))
            ...
            reject(
              new Error(
                `Expected JSON from ${url} but got HTML (status ${res.statusCode}). ` +
                  'The endpoint is likely missing on the ulnclaw backend.'
              )
            )
    ...
    req.setTimeout(timeoutMs, () => {
      req.destroy(new Error(`Timed out connecting to ulnclaw backend after ${timeoutMs}ms`))
    })
...
function fetchPublicJson(url, options: any = {}) {
  ...
  // NO ``X-ulnclaw-Session-Token`` header — used by the auth-mode probe before
...
function getLinkTitleSession() {
  ...
  linkTitleSession = session.fromPartition('ulnclaw:link-titles', { cache: false })
...
async function previewFileTarget(rawTarget, baseDir) {
  ...
  const base = baseDir ? path.resolve(expandUserPath(baseDir)) : resolveulnclawCwd()
...
function sendPreviewFileChanged(payload) {
  ...
  webContents.send('ulnclaw:preview-file-changed', payload)
}
...
// waitForulnclawReady how to read a 401 — rejected session vs gated route.
...
async function waitForulnclaw(baseUrl, token, signal?, authMode?) {
  const { probeHealth, probeIsCredentialed } = await buildReadinessHealthProbe(baseUrl, authMode, token)

  return waitForulnclawReady(baseUrl, {
    token,
    signal,
    fetchPublicJson,
...
function sendBackendExit(payload) {
  ...
  webContents.send('ulnclaw:backend-exit', payload)
}
...
function sendClosePreviewRequested() {
  ...
  webContents.send('ulnclaw:close-preview-requested')
}
...
function sendOpenFolderRequested() {
  ...
  webContents.send('ulnclaw:open-folder-requested')
}

// Tell the renderer the machine just woke. Sleep silently drops the
// renderer's WebSocket to the local backend; the renderer reconnects on this
// signal so the chat composer doesn't stay stuck on "Starting ulnclaw...".
function sendPowerResume() {
  ...
  webContents.send('ulnclaw:power-resume')
}
...
// 'ulnclaw:power-battery' push below.
ipcMain.handle('ulnclaw:power-battery:get', () => onBatteryPower === true)
...
function broadcastBatteryState(next: boolean) {
  ...
  for (const win of BrowserWindow.getAllWindows()) {
    ...
    if (webContents && !webContents.isDestroyed()) {
      webContents.send('ulnclaw:power-battery', next)
    }
  }
}
...
function sendOpenUpdatesRequested() {
  ...
  webContents.send('ulnclaw:open-updates')
...
function sendWindowStateChanged(nextIsFullscreen?: boolean, target = mainWindow) {
  ...
  webContents.send('ulnclaw:window-state-changed', state)
}
...
//
// Hosted ulnclaw gateways gate the dashboard behind an OAuth provider (e.g.
// Nous Research) instead of a static session token. The auth model is
// fundamentally different from the token path:
//
//   * REST is authed by HttpOnly session cookies (``ulnclaw_session_at``),
...
//     alongside the ~15-min access token (Portal NAS #293 / ulnclaw #37247).
//     Both are set as HttpOnly cookies (``ulnclaw_session_at`` ~15 min,
//     ``ulnclaw_session_rt`` 24h). When the AT cookie lapses but the RT cookie
...
const OAUTH_SESSION_PARTITION = 'persist:ulnclaw-remote-oauth'
...
// "not signed in", which on the initial boot path (startulnclaw → the renderer's
// single-shot boot() with no retry) surfaced as the "ulnclaw couldn't start"
...
async function hasLiveOauthSession(baseUrl) {
  ...
  // for a signed-in user — the exact race that produced the transient "ulnclaw
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: silent ? 'Connecting to ulnclaw Cloud agent…' : 'Sign in to ulnclaw gateway',
...
function fetchJsonViaOauthSession(url, options: any = {}) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error(`Timed out connecting to ulnclaw backend after ${timeoutMs}ms`))
    }, timeoutMs)
...
// feature; the server half lives in ulnclaw_cli/dashboard_auth/native_flow.py.
// ---------------------------------------------------------------------------
...
// --- ulnclaw Cloud discovery + silent per-agent sign-in (cloud-auto-discovery
// Phase 3) ---------------------------------------------------------------
//
...
// portal session cookie living in `persist:ulnclaw-remote-oauth`:
...
// convention (ulnclaw_cli/auth.py DEFAULT_NOUS_PORTAL_URL + the same env names)
// so a single override flips every ulnclaw surface to the same portal.
const DEFAULT_NOUS_PORTAL_URL = 'https://portal.nousresearch.com'

function resolvePortalBaseUrl() {
  const raw = process.env.ULNCLAW_PORTAL_BASE_URL || process.env.NOUS_PORTAL_BASE_URL || DEFAULT_NOUS_PORTAL_URL
...
// authenticates via PRIVY, not the ulnclaw gateway session cookies, so this
// checks for the `privy-token` cookie on the portal host (NOT
// hasLiveOauthSession, which looks for ulnclaw_session_at/rt that the portal
...
function openPortalLoginWindow() {
  ...
  return new Promise((resolve, reject) => {
    if (!app.isReady()) {
      reject(new Error('Desktop is not ready to start a ulnclaw Cloud sign-in.'))
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: 'Sign in to ulnclaw Cloud',
        autoHideMenuBar: true,
        webPreferences: {
          contextIsolation: true,
          nodeIntegration: false,
...
// Discover the hosted (ulnclaw Cloud) agents the signed-in user can see. Calls
...
async function discoverCloudAgents(org?: string) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error(
      'You are not signed in to ulnclaw Cloud. Open Settings → Gateway, choose ulnclaw Cloud, and sign in.'
    ) as any
  ...
  try {
    ...
    if (error && error.statusCode === 401) {
      const err = new Error('Your ulnclaw Cloud session has expired. Open Settings → Gateway and sign in again.') as any
      err.needsCloudLogin = true
      err.cause = error
      throw err
    }
...
async function cloudAgentSilentSignIn(dashboardUrl) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error('Your ulnclaw Cloud session has expired. Sign in to ulnclaw Cloud again.') as any
    err.needsCloudLogin = true
    throw err
  }
...
function sanitizeConnectionProfiles(raw: Record<string, any>) {
  ...
  for (const [name, entry] of Object.entries(raw)) {
    ...
    // Preserve the ulnclaw Cloud org tag on cloud-mode entries so Settings can
...
// a valid stored value (pins the root ULNCLAW_HOME explicitly); null means "no
...
async function sanitizeDesktopConnectionConfig(config = readDesktopConnectionConfig(), profile = null) {
  ...
  const envOverride = key ? false : Boolean(process.env.ULNCLAW_DESKTOP_REMOTE_URL)
  ...
  const remoteUrl = envOverride ? String(process.env.ULNCLAW_DESKTOP_REMOTE_URL || '') : String(block.url || '')
  ...
  return {
    ...
    remoteUrl,
    // The persisted ulnclaw Cloud org (slug/id) for a cloud connection, or '' for
    ...
    sshRemoteulnclawPath: (ssh || savedSsh)?.remoteulnclawPath || '',
    sshRemoteProfile: (ssh || savedSsh)?.remoteProfile || '',
    // The env override only forces the global/primary connection; a per-profile
    // scope is never overridden by ULNCLAW_DESKTOP_REMOTE_URL.
    envOverride
  }
}
...
// `org` (optional) is the ulnclaw Cloud org slug/id the instance was discovered
...
function coerceDesktopConnectionConfig(input: any = {}, existing = readDesktopConnectionConfig(), options: any = {}) {
  ...
  // describe a discovered ulnclaw Cloud instance, NOT a user-owned remote gateway,
...
function buildSshBlock(input: any, existingBlock: any = {}) {
  ...
  const merged = normalizeSshConfig({
    ...
    remoteulnclawPath: input.sshRemoteulnclawPath ?? existingBlock.remoteulnclawPath,
...
async function buildRemoteConnection(
  ...
  if (authMode === 'oauth') {
    ...
    // refresh token (ulnclaw #37247), and the gateway middleware transparently
    ...
    if (
      ...
      const err = new Error(
        'Remote ulnclaw gateway uses OAuth, but you are not signed in. ' +
    ...
    try {
      ...
      throw gatewayTicketFailure(
        ...
        'Could not reach the remote ulnclaw gateway while refreshing its WebSocket ticket. Try reconnecting.'
      )
    }
  ...
  if (!token) {
    throw new Error(
      'Remote ulnclaw gateway is selected, but no session token is saved. ' +
...
function activeSshTerminalTarget() {
  ...
  if (process.env.ULNCLAW_DESKTOP_REMOTE_URL) {
    return null
  }
...
async function bootstrapSshConnectionInner(profile, sshConfig, reuseToken, source, fingerprint, lease) {
  ...
  try {
    const platform = await detectRemotePlatform(ssh, sshConfig.remoteulnclawPath || '')
    ...
    result = await lifecycle({
      ...
      remoteulnclawPath: sshConfig.remoteulnclawPath || '',
      ownershipId: sshOwnershipKey(profile),
      ...
      pickLocalPort,
      waitForulnclaw: (baseUrl, token) => waitForulnclaw(baseUrl, token, lease.signal, 'token'),
  ...
  sshConnections.set(scope, {
    ...
    host: sshConfig.host,
    hostLabel,
    ulnclawVersion: result.ulnclawVersion || '',
  ...
  sshRememberLog(
    `[ssh] connection ${result.reused ? 'REUSED' : 'spawned'} dashboard: ` +
      `${result.ulnclawVersion || 'ulnclaw (version unknown)'} at ${result.ulnclawPath || '?'}`
  )
  ...
  return { ...connection, remoteulnclawVersion: result.ulnclawVersion || '' }
}
...
//   2. env override (ULNCLAW_DESKTOP_REMOTE_URL/_TOKEN) — applies app-wide
...
async function resolveRemoteBackend(profile) {
  ...
  const rawEnvUrl = process.env.ULNCLAW_DESKTOP_REMOTE_URL
  const rawEnvToken = process.env.ULNCLAW_DESKTOP_REMOTE_TOKEN

  if (rawEnvUrl) {
    if (!rawEnvToken) {
      throw new Error(
        'ULNCLAW_DESKTOP_REMOTE_URL is set but ULNCLAW_DESKTOP_REMOTE_TOKEN is not. ' +
          'Both must be provided to connect to a remote ulnclaw backend.'
      )
    }
...
// env → global) so a startulnclaw() failure can be classified as remote (never
...
async function probeRemoteAuthMode(rawUrl) {
  ...
  // sending any credentials. ``/api/status`` is public on every ulnclaw
...
async function testDesktopConnectionConfig(input: any = {}) {
  if (input.mode === 'ssh') {
    const sshConfig = normalizeSshConfig({
      ...
      remoteulnclawPath: input.sshRemoteulnclawPath
    })
    ...
    try {
      ...
      for (;;) {
        try {
          await ssh.open()
          const platform: any = await detectRemotePlatform(ssh, sshConfig.remoteulnclawPath || '')
          let ulnclawPath
          let ulnclawVersion
          let supported
          ...
          if (platform.os === 'Windows') {
            ...
            ulnclawPath = runtime.ulnclawPath
            const inspection = await helper(ssh, runtime, 'inspect', [runtime.ulnclawPath])
            ulnclawVersion = inspection.version
            supported = inspection.supported
          } else {
            ulnclawPath = await remoteLifecycle.locateulnclaw(ssh, sshConfig.remoteulnclawPath || '')
            ulnclawVersion = await remoteLifecycle.probeulnclawVersion(ssh, ulnclawPath)
            supported = await remoteLifecycle.remoteSupportsSshOwnership(ssh, ulnclawPath)
          }
          ...
          if (!supported) {
            return {
              ...
              error: 'Update ulnclaw on the remote host before connecting with Desktop SSH.'
            }
          }
          ...
          return {
            ...
            remoteulnclawPath: ulnclawPath,
            remoteulnclawVersion: ulnclawVersion,
  ...
  if (wantRemote && block?.url) {
    ...
    const remote = (await resolveRemoteBackend(key)) || (await startulnclaw())
  ...
  // connect to ulnclaw gateway". Mirror the renderer's connect here so the test
...
function resetulnclawConnection({ soft = false } = {}) {
  backendStartFailure = null
  remoteReauthFailure = null
  remoteLiveness.clear()
  const ulnclawProcess = backendConnectionState.invalidate()
  stopBackendChild(ulnclawProcess)
...
// startulnclaw() spawns fresh instead of racing the dying one. Shared by the
// connection-config and profile switch flows.
async function teardownPrimaryBackendAndWait({ soft = false } = {}) {
  // Capture the reference before resetulnclawConnection() invalidates it.
  const ulnclawProcess = backendConnectionState.getProcess()
  const dying = ulnclawProcess && !ulnclawProcess.killed ? ulnclawProcess : null
...
function sendConnectionApplied() {
  ...
  webContents.send('ulnclaw:connection:applied')
}
...
async function ensureBackend(profile) {
  ...
  if (route.backend === 'primary') {
    const connection = await startulnclaw()
...
// local-spawn portion of startulnclaw() but without the boot-progress UI,
...
async function spawnPoolBackend(profile, entry) {
  ...
  if (remote) {
    await waitForulnclaw(remote.baseUrl, remote.token, undefined, remote.authMode)
  ...
  // --profile wins over the inherited ULNCLAW_HOME env (see _apply_profile_override
  // step 3 in ulnclaw_cli/main.py), so the child re-homes to this profile.
  // --port 0: the OS assigns an ephemeral port; the child announces it on stdout.
  const backendArgs = ['--profile', profile, 'serve', '--host', '127.0.0.1', '--port', '0']
  const backend = await ensureRuntime(resolveulnclawBackend(backendArgs))
  // Route old runtimes (no `serve`) through the legacy `dashboard --no-open`.
  backend.args = getBackendArgsForRuntime(backend)
  const ulnclawCwd = resolveulnclawCwd()
  const webDist = resolveWebDist()
  const readyFile = backend.readyFile ? makeDashboardReadyFile() : null

  rememberLog(`Starting ulnclaw backend for profile "${profile}" via ${backend.label}`)
  ...
  const child = spawn(
    ...
    hiddenWindowsChildOptions({
      ...
      env: {
        ...
        TERMINAL_CWD: ulnclawCwd,
        ULNCLAW_DASHBOARD_SESSION_TOKEN: token,
        // Marks this dashboard backend as desktop-spawned so it runs the cron
        // scheduler tick loop (the gateway isn't running under the app).
        ULNCLAW_DESKTOP: '1',
        ULNCLAW_WEB_DIST: webDist,
        ...(readyFile ? { ULNCLAW_DESKTOP_READY_FILE: readyFile } : {})
      },
      shell: backend.shell,
      stdio: ['ignore', 'pipe', 'pipe']
    })
  ...
  child.once('error', error => {
    rememberLog(`ulnclaw backend for profile "${profile}" failed to start: ${error.message}`)
    backendPool.delete(profile)
    rejectStart?.(error)
  })
  child.once('exit', (code, signal) => {
    rememberLog(`ulnclaw backend for profile "${profile}" exited (${signal || code})`)
    backendPool.delete(profile)

    if (!ready) {
      rejectStart?.(
        new Error(`ulnclaw backend for profile "${profile}" exited before it became ready (${signal || code}).`)
      )
    }
  })
  ...
  await Promise.race([waitForulnclaw(baseUrl, token), startFailed])
  ready = true

  const authToken = await adoptServedDashboardToken(baseUrl, token, {
    childAlive: () => child.exitCode === null && !child.killed,
    label: `ulnclaw backend for profile "${profile}"`,
    rememberLog
  })
  ...
  if (!wsProbe.ok) {
    throw new Error(
      `ulnclaw backend for profile "${profile}" is HTTP-reachable but the WebSocket (/api/ws) rejected the session token: ${wsProbe.reason}`
    )
  }
...
// backend whose ensure_ulnclaw_home() recreates the deleted profile directory.
//
...
async function startulnclaw() {
  // Latched-failure short-circuit: once bootstrap has failed in this
  // process, every subsequent startulnclaw() call re-throws the same error
  ...
  if (BOOT_FAKE_ERROR) {
    await advanceBootProgress('backend.resolve', 'Resolving ulnclaw backend', 8)
  ...
  const connectionPromise = (async () => {
    const connectRemote = async remote => {
      await advanceBootProgress('backend.remote', `Connecting to remote ulnclaw backend at ${remote.baseUrl}`, 24)
      await waitForulnclaw(remote.baseUrl, remote.token, undefined, remote.authMode)
      updateBootProgress({
        phase: 'backend.ready',
        message: 'Remote ulnclaw backend is ready',
        progress: 94,
        running: true,
        error: null
      })
      ...
      return {
        ...
        remoteulnclawVersion: remote.remoteulnclawVersion,
        token: remote.token,
        wsUrl: remote.wsUrl,
        logs: ulnclawLog.slice(-80),
        ...getWindowState()
    ...
    // deterministic (it wins over the sticky ~/.ulnclaw/active_profile file) and
    // resolves ULNCLAW_HOME the same way `ulnclaw -p <name>` does on the CLI. An
    ...
    const setup = await runPrimaryBackendStartup({
      ...
      prepareLocalBackend: async () => {
        await advanceBootProgress('backend.runtime', 'Resolving ulnclaw runtime', 28)

        return resolveulnclawBackend(backendArgs)
      },
      resolveRemote: () => {
    ...
    await advanceBootProgress('backend.spawn', `Starting ulnclaw backend via ${backend.label}`, 84)
    rememberLog(`Starting ulnclaw backend via ${backend.label}`)

    const ulnclawProcess = spawn(
      backend.command,
      backend.args,
      hiddenWindowsChildOptions({
        cwd: ulnclawCwd,
        env: {
          ...process.env,
          // Explicitly pin ULNCLAW_HOME for the child so Python's get_ulnclaw_home()
          // resolves to the SAME location our resolveulnclawHome() picked. Without
          // this pin, Python falls back to ~/.ulnclaw on every platform — fine on
          // mac/linux (where our default matches), but on Windows our default is
          // %LOCALAPPDATA%\ulnclaw, which differs from C:\Users\<u>\.ulnclaw.
          // Mismatch would split config / sessions / .env / logs across two
          // directories. install.ps1 sets ULNCLAW_HOME via setx; the desktop
    ...
    const processOwner = backendConnectionState.attachProcess(connectionAttempt, ulnclawProcess)

    if (!processOwner) {
      stopBackendChild(ulnclawProcess)
      throw new Error('ulnclaw backend start was superseded by a newer connection attempt.')
    }

    ulnclawProcess.stdout.on('data', rememberLog)
    ulnclawProcess.stderr.on('data', rememberLog)
    ...
    ulnclawProcess.once('error', error => {
      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale ulnclaw backend error: ${error.message}`)
        rejectBackendStart?.(new Error('ulnclaw backend start was superseded by a newer connection attempt.'))

        return
      }

      rememberLog(`ulnclaw backend failed to start: ${error.message}`)
      updateBootProgress(
        {
          error: error.message,
          message: `ulnclaw backend failed to start: ${error.message}`,
          phase: 'backend.error',
          running: false
        },
        { allowDecrease: true }
    ...
    ulnclawProcess.once('exit', (code, signal) => {
      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale ulnclaw backend exit (${signal || code})`)
      ...
      rememberLog(`ulnclaw backend exited (${signal || code})`)
      sendBackendExit({ code, signal })

      if (!backendReady) {
        const message = `ulnclaw backend exited before it became ready (${signal || code}).`
        updateBootProgress(
          {
            error: message,
            message,
        ...
        rejectBackendStart?.(
          new Error(
            `ulnclaw backend exited before it became ready (${signal || code}). Log: ${DESKTOP_LOG_PATH}\n${recentulnclawLog()}`
          )
        )
      }
    })

    await advanceBootProgress('backend.port', 'Waiting for ulnclaw backend to launch', 86)

    // Discover the ephemeral port the child bound to
    const port = await Promise.race([
      waitForDashboardPortAnnouncement(ulnclawProcess, { readyFile }),
      backendStartFailed
    ])
    ...
    const baseUrl = `http://127.0.0.1:${port}`
    await advanceBootProgress('backend.wait', 'Waiting for ulnclaw backend to become ready', 90)
    await Promise.race([waitForulnclaw(baseUrl, token), backendStartFailed])
    backendReady = true
    backendStartFailure = null

    const authToken = await adoptServedDashboardToken(baseUrl, token, {
      childAlive: () => ulnclawProcess.exitCode === null && !ulnclawProcess.killed,
      rememberLog
    })
    ...
    if (!wsProbe.ok) {
      throw new Error(
        `Local ulnclaw backend is HTTP-reachable but the WebSocket (/api/ws) rejected the session token: ${wsProbe.reason}`
      )
    }

    updateBootProgress({
      phase: 'backend.ready',
      message: 'ulnclaw backend is ready. Finalizing desktop startup',
      progress: 94,
      running: true,
      error: null
    })
...
// here so it can leave the app's bounds and stay visible while ulnclaw is
...
// pushes pet state over IPC (ulnclaw:pet-overlay:state); the overlay just renders
// it. Control flows back (pop-in, composer submit) via ulnclaw:pet-overlay:control.
...
function spawnPetOverlayWindow(bounds) {
  const win = new BrowserWindow({
    ...
    // instead, leaving the main ulnclaw app as the Dock/cmd-tab anchor.
    skipTaskbar: !IS_MAC,
    hasShadow: false,
    alwaysOnTop: true,
    ...
    // anchor and the ulnclaw icon drops out of cmd/alt-tab — especially when the
    // main window is minimized. We flip this on only while the composer needs
    // the keyboard (see ulnclaw:pet-overlay:set-focusable).
    focusable: false,
    show: false,
  ...
  try {
    ...
    // which briefly hides the Dock/cmd-tab presence. Keep ulnclaw in the normal
  ...
  win.on('closed', () => {
    ...
    if (mainWindow && !mainWindow.isDestroyed()) {
      mainWindow.webContents.send('ulnclaw:pet-overlay:control', { type: 'pop-in' })
    }
  })
...
function spawnQuickEntryWindow() {
  ...
  win.webContents.on('did-finish-load', () => {
    if (!win.isDestroyed() && quickEntryLastState) {
      win.webContents.send('ulnclaw:quick-entry:state', quickEntryLastState)
    }
  })
...
function showQuickEntryWindow() {
  ...
  quickEntryWindow.webContents.send('ulnclaw:quick-entry:shown')
}
...
function createWindow() {
  ...
  startulnclaw().catch(error => rememberLog(error.stack || error.message))
...
ipcMain.handle('ulnclaw:connection', async (_event, profile) => ensureBackend(profile))
...
// "Starting ulnclaw…". Before the renderer's backoff loop reconnects, it asks us
// to confirm the cached PRIMARY backend is still reachable; if a remote one is
// not, we drop the cache so the next getConnection() rebuilds it. Local backends
// self-heal via their child 'exit' handler, so we never touch them here.
ipcMain.handle('ulnclaw:connection:revalidate', async () => {
  ...
  return remoteRevalidation.run(connectionPromise, async () => {
    const [result] = await Promise.all([
      revalidateRemoteConnection({
        ...
        resetConnection: resetulnclawConnection,
...
ipcMain.handle('ulnclaw:backend:touch', async (_event, profile) => {
  touchPoolBackend(profile)

  return { ok: true }
})
ipcMain.handle('ulnclaw:gateway:ws-url', async (_event, profile) => {
  return gatewayWsUrlIpcResult(() => freshGatewayWsUrl(profile))
})
ipcMain.handle('ulnclaw:window:openSession', async (_event, sessionId, opts) => {
...
ipcMain.handle('ulnclaw:window:openInstance', async () => {
  createInstanceWindow()

  return { ok: true }
})
ipcMain.handle('ulnclaw:wake-indicator:get', () => wakeIndicatorController.getState())
ipcMain.on('ulnclaw:wake-indicator:set', (_event, state) => {
...
ipcMain.handle('ulnclaw:zoom:get', event => {
...
ipcMain.on('ulnclaw:zoom:set-percent', (event, percent) => {
  const window = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('ulnclaw:pet-overlay:open', async (_event, request) => {
...
ipcMain.handle('ulnclaw:pet-overlay:close', async () => {
  closePetOverlay()
...
ipcMain.on('ulnclaw:pet-overlay:set-bounds', (_event, bounds) => {
...
ipcMain.on('ulnclaw:pet-overlay:ignore-mouse', (_event, ignore) => {
  if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
...
ipcMain.on('ulnclaw:pet-overlay:set-focusable', (_event, focusable) => {
  if (!petOverlayWindow || petOverlayWindow.isDestroyed()) {
    return
  }
...
ipcMain.on('ulnclaw:pet-overlay:state', (_event, payload) => {
  if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
    petOverlayWindow.webContents.send('ulnclaw:pet-overlay:state', payload)
  }
})
// Overlay → main renderer: control messages (pop back in, composer submit).
ipcMain.on('ulnclaw:pet-overlay:control', (_event, payload) => {
  if (!mainWindow || mainWindow.isDestroyed()) {
    return
  }
  ...
  mainWindow.webContents.send('ulnclaw:pet-overlay:control', payload)
})
ipcMain.handle('ulnclaw:bootstrap:reset', async () => {
  // Renderer's "Reload and retry" path. Clear the latched failure and
  // reset connection state so the next startulnclaw() call restarts the
...
ipcMain.handle('ulnclaw:bootstrap:repair', async () => {
  // Forceful repair: force the next startulnclaw() through the full installer
  ...
  // "bypass active runtime" path inside startulnclaw() and fall through
  ...
  resetulnclawConnection()

  return { ok: true }
})
ipcMain.handle('ulnclaw:bootstrap:continue-local', async () => {
  rememberLog('[bootstrap] local install selected by renderer; continuing first-launch bootstrap')
  continueFirstRunLocalBootstrap()

  return { ok: true }
})
ipcMain.handle('ulnclaw:bootstrap:cancel', async () => {
...
ipcMain.handle('ulnclaw:boot-progress:get', async () => bootProgressState)
ipcMain.handle('ulnclaw:bootstrap:get', async () => getBootstrapState())
ipcMain.handle('ulnclaw:connection-config:get', async (_event, profile) =>
  sanitizeDesktopConnectionConfig(readDesktopConnectionConfig(), profile)
)
ipcMain.handle('ulnclaw:ssh-config:hosts', async () => ({ hosts: collectSshConfigHosts() }))
ipcMain.handle('ulnclaw:ssh-config:resolve', async (_event, host) => {
...
ipcMain.handle('ulnclaw:connection-config:test', async (_event, payload) => testDesktopConnectionConfig(payload))
ipcMain.handle('ulnclaw:connection-config:probe', async (_event, rawUrl) => probeRemoteAuthMode(rawUrl))
ipcMain.handle('ulnclaw:connection-config:oauth-login', async (_event, rawUrl) => {
  ...
  if (strategy === 'native') {
    try {
      ...
      // startulnclaw() re-dials instead of replaying the stale rejection.
      remoteReauthFailure = null
...
ipcMain.handle('ulnclaw:connection-config:oauth-logout', async (_event, rawUrl) => {
...
// --- ulnclaw Cloud (cloud-auto-discovery Phase 3) ---
...
ipcMain.handle('ulnclaw:cloud:agent-sign-in', async (_event, dashboardUrl) => {
...
ipcMain.handle('ulnclaw:connection-config:save', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)

  return sanitizeDesktopConnectionConfig(config, payload?.profile)
})
ipcMain.handle('ulnclaw:connection-config:apply', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)
...
ipcMain.handle('ulnclaw:profile:get', async () => ({ profile: readActiveDesktopProfile() }))
ipcMain.handle('ulnclaw:profile:set', async (_event, name) => {
  const next = writeActiveDesktopProfile(name)

  // Switching profiles is a backend re-home: relaunch the dashboard under the
  // new ULNCLAW_HOME. Pool backends keep their own homes, so only the primary
  // is torn down.
  await teardownPrimaryBackendAndWait()
  mainWindow?.reload()
...
ipcMain.on('ulnclaw:previewShortcutActive', (_event, active) => {
  previewShortcutActive = Boolean(active)
})

ipcMain.handle('ulnclaw:requestMicrophoneAccess', async () => {
...
ipcMain.handle('ulnclaw:api', async (_event, request) => {
  ...
  // backend calls ensure_ulnclaw_home() which recreates the profile directory,
...
ipcMain.handle('ulnclaw:ambient:claim', (_event, key) => !claimedAmbientCue(String(key ?? '')))

ipcMain.handle('ulnclaw:notify', (_event, payload) => {
  ...
  notification.on('click', () => {
    ...
    if (payload?.sessionId) {
      mainWindow.webContents.send('ulnclaw:focus-session', payload.sessionId)
    }
  })
  notification.on('action', (_actionEvent, index) => {
    ...
    if (action?.id) {
      mainWindow.webContents.send('ulnclaw:notification-action', { sessionId: payload?.sessionId, actionId: action.id })
    }
  })
  notification.show()
...
ipcMain.handle('ulnclaw:data-url-read-max:get', () => ({
...
ipcMain.handle('ulnclaw:data-url-read-max:set', (_event, maxMb) => {
...
ipcMain.handle('ulnclaw:readFileDataUrl', async (_event, filePath) => {
...
ipcMain.handle('ulnclaw:readFileDataUrlForAttach', async (_event, filePath) => {
  return readFileDataUrlForIpc(filePath, {
...
ipcMain.handle('ulnclaw:readFileText', async (_event, filePath) => {
...
ipcMain.handle('ulnclaw:selectPaths', async (_event, options: any = {}) => {
...
ipcMain.handle('ulnclaw:writeClipboard', (_event, text) => {
...
ipcMain.handle('ulnclaw:readClipboard', () => clipboard.readText())

ipcMain.handle('ulnclaw:saveImageFromUrl', (_event, url) => saveImageFromUrl(String(url || '')))

ipcMain.handle('ulnclaw:saveImageBuffer', async (_event, payload) => {
...
ipcMain.handle('ulnclaw:saveClipboardImage', async () => {
...
ipcMain.handle('ulnclaw:normalizePreviewTarget', (_event, target, baseDir) =>
  normalizePreviewTarget(String(target || ''), baseDir ? String(baseDir) : '')
)

ipcMain.handle('ulnclaw:watchPreviewFile', (_event, url) => watchPreviewFile(String(url || '')))

ipcMain.handle('ulnclaw:watchDirectory', (_event, dir) => watchDirectory(String(dir || '')))

ipcMain.handle('ulnclaw:stopPreviewFileWatch', (_event, id) => stopPreviewFileWatch(String(id || '')))
...
ipcMain.on('ulnclaw:active-work', (event, payload) => {
...
ipcMain.on('ulnclaw:titlebar-theme', (_event, payload) => {
...
ipcMain.on('ulnclaw:native-theme', (_event, mode) => {
...
ipcMain.on('ulnclaw:translucency', (_event, payload) => {
...
ipcMain.on('ulnclaw:keep-awake', (_event, on) => {
...
ipcMain.handle('ulnclaw:quick-entry:settings:get', async () => {
...
ipcMain.handle('ulnclaw:quick-entry:settings:set', async (_event, patch) => {
...
ipcMain.on('ulnclaw:quick-entry:submit', (_event, payload) => {
  hideQuickEntryWindow()
  ...
  mainWindow.webContents.send('ulnclaw:quick-entry:submit', {
...
ipcMain.on('ulnclaw:quick-entry:state', (_event, payload) => {
  quickEntryLastState = payload ?? null

  if (quickEntryWindow && !quickEntryWindow.isDestroyed()) {
    quickEntryWindow.webContents.send('ulnclaw:quick-entry:state', payload)
  }
})

ipcMain.on('ulnclaw:quick-entry:dismiss', () => hideQuickEntryWindow())

ipcMain.handle('ulnclaw:openExternal', (_event, url) => {
...
// per-session secondary windows spawned via `ulnclaw:window:openSession`).
...
ipcMain.handle('ulnclaw:find-in-page', (event, query, options) => {
...
ipcMain.handle('ulnclaw:stop-find-in-page', event => {
  const win = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('ulnclaw:openPreviewInBrowser', async (_event, url) => {
...
// it via writeDefaultProjectDir so resolveulnclawCwd picks it up on the next
// session spawn (no app restart needed).
ipcMain.handle('ulnclaw:setting:defaultProjectDir:get', async () => ({
  dir: readDefaultProjectDir(),
  defaultLabel: app.getPath('home'),
  resolvedCwd: resolveulnclawCwd()
}))

ipcMain.handle('ulnclaw:workspace:sanitize', async (_event, cwd) => sanitizeWorkspaceCwd(cwd))

ipcMain.handle('ulnclaw:setting:defaultProjectDir:set', async (_event, dir) => {
...
ipcMain.handle('ulnclaw:setting:defaultProjectDir:pick', async () => {
...
ipcMain.handle('ulnclaw:fetchLinkTitle', (_event, url) => fetchLinkTitle(url))

ipcMain.handle('ulnclaw:logs:reveal', async () => {
  try {
    await fs.promises.mkdir(path.dirname(DESKTOP_LOG_PATH), { recursive: true })
...
ipcMain.handle('ulnclaw:logs:recent', async () => ({ path: DESKTOP_LOG_PATH, lines: ulnclawLog.slice(-200) }))
...
function terminalShellCommand() {
  // ULNCLAW_DESKTOP_SHELL is the cross-platform escape hatch (a path or a bare
  ...
  const override = (process.env.ULNCLAW_DESKTOP_SHELL || (IS_WINDOWS ? '' : process.env.SHELL) || '').trim()
...
function terminalShellEnv() {
  ...
  // /TERM=dumb; some terminals set COLORFGBG which would flip ulnclaw' TUI into
  ...
  env.TERM_PROGRAM = 'ulnclaw'
  env.TERM_PROGRAM_VERSION = app.getVersion()

  // Let a ulnclaw/--tui launched in this pane know it's embedded in the desktop
  // GUI (build_environment_hints surfaces this). Distinct from ULNCLAW_DESKTOP,
  // which marks the agent *backend* and gates cron/gateway behavior.
  env.ULNCLAW_DESKTOP_TERMINAL = '1'

  return env
}

function terminalChannel(id, suffix) {
  return `ulnclaw:terminal:${id}:${suffix}`
}
...
ipcMain.handle('ulnclaw:fs:readDir', async (_event, dirPath) => readDirForIpc(dirPath))

ipcMain.handle('ulnclaw:fs:gitRoot', async (_event, startPath) => gitRootForIpc(startPath))

// Reveal a path in the OS file manager (Finder / Explorer / Files).
ipcMain.handle('ulnclaw:fs:reveal', async (_event, targetPath) => {
...
ipcMain.handle('ulnclaw:fs:openDir', async (_event, dirPath) => {
...
// The LOCAL Desktop runtime-plugin root: `<ULNCLAW_HOME>/desktop-plugins`,
// resolved from the main-process ULNCLAW_HOME (see resolveulnclawHome) — NOT from
// the connected backend. A remote backend reports its own `ulnclaw_home` over
...
ipcMain.handle('ulnclaw:fs:desktopPluginsRoot', async () => {
  // Profile-aware: a named Desktop profile gets its own plugin root under
  // profiles/<name>/, matching the profile-scoped ulnclaw_home the backend
  // reported before this resolver existed. 'default'/unset pins the global root.
  const profile = readActiveDesktopProfile()
  const base = profile && profile !== 'default' ? path.join(ULNCLAW_HOME, 'profiles', profile) : ULNCLAW_HOME
...
ipcMain.handle('ulnclaw:fs:rename', async (_event, targetPath, newName) => {
...
ipcMain.handle('ulnclaw:fs:writeText', async (_event, filePath, content) => {
...
ipcMain.handle('ulnclaw:fs:trash', async (_event, targetPath) => {
  const target = String(targetPath || '').trim()
...
ipcMain.handle('ulnclaw:git:worktreeList', async (_event, repoPath) => listWorktrees(repoPath, resolveGitBinary()))

ipcMain.handle('ulnclaw:git:worktreeAdd', async (_event, repoPath, options) =>
  addWorktree(repoPath, options || {}, resolveGitBinary())
)

ipcMain.handle('ulnclaw:git:worktreeRemove', async (_event, repoPath, worktreePath, options) =>
  removeWorktree(repoPath, worktreePath, options || {}, resolveGitBinary())
)

ipcMain.handle('ulnclaw:git:branchSwitch', async (_event, repoPath, branch) =>
  switchBranch(repoPath, branch, resolveGitBinary())
)

ipcMain.handle('ulnclaw:git:branchList', async (_event, repoPath) => listBranches(repoPath, resolveGitBinary()))

ipcMain.handle('ulnclaw:git:baseBranchList', async (_event, repoPath) => listBaseBranches(repoPath, resolveGitBinary()))
...
ipcMain.handle('ulnclaw:git:repoStatus', async (_event, repoPath) => repoStatus(repoPath, resolveGitBinary()))
...
ipcMain.handle('ulnclaw:git:review:list', async (_event, repoPath, scope, baseRef) =>
  reviewList(repoPath, scope, baseRef, resolveGitBinary())
)
ipcMain.handle('ulnclaw:git:review:diff', async (_event, repoPath, filePath, scope, baseRef, staged) =>
...
ipcMain.handle('ulnclaw:git:fileDiff', async (_event, repoPath, filePath) =>
  fileDiffVsHead(repoPath, filePath, resolveGitBinary())
)
ipcMain.handle('ulnclaw:git:review:stage', async (_event, repoPath, filePath) =>
  reviewStage(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('ulnclaw:git:review:unstage', async (_event, repoPath, filePath) =>
  reviewUnstage(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('ulnclaw:git:review:revert', async (_event, repoPath, filePath) =>
  reviewRevert(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('ulnclaw:git:review:revParse', async (_event, repoPath, ref) =>
  reviewRevParse(repoPath, ref, resolveGitBinary())
)
ipcMain.handle('ulnclaw:git:review:commit', async (_event, repoPath, message, push) =>
  reviewCommit(repoPath, message, Boolean(push), resolveGitBinary())
)
ipcMain.handle('ulnclaw:git:review:commitContext', async (_event, repoPath) =>
  reviewCommitContext(repoPath, resolveGitBinary())
)
ipcMain.handle('ulnclaw:git:review:push', async (_event, repoPath) => reviewPush(repoPath, resolveGitBinary()))
ipcMain.handle('ulnclaw:git:review:shipInfo', async (_event, repoPath) => reviewShipInfo(repoPath, resolveGhBinary()))
ipcMain.handle('ulnclaw:git:review:createPr', async (_event, repoPath) =>
...
ipcMain.handle('ulnclaw:git:scanRepos', async (_event, roots, options) => {
  try {
...
ipcMain.handle('ulnclaw:terminal:start', async (event, payload = {}) => {
...
ipcMain.handle('ulnclaw:terminal:write', (_event, id, data) => {
...
ipcMain.handle('ulnclaw:terminal:resize', (_event, id, size = {}) => {
  const sessionInfo = terminalSessions.get(String(id || ''))
...
ipcMain.handle('ulnclaw:terminal:cwd', async (_event, id) => {
  const sessionInfo = terminalSessions.get(String(id || ''))
...
ipcMain.handle('ulnclaw:terminal:dispose', (_event, id) => disposeTerminalSession(String(id || '')))

ipcMain.handle('ulnclaw:updates:check', async () =>
...
ipcMain.handle('ulnclaw:updates:apply', async (_event, payload) =>
...
ipcMain.handle('ulnclaw:updates:branch:get', async () => readDesktopUpdateConfig())

ipcMain.handle('ulnclaw:updates:branch:set', async (_event, name) => {
...
// Resolve the canonical ulnclaw version (the one `release.py` bumps in
// ulnclaw_cli/__init__.py + pyproject.toml) so the desktop About panel shows the
// real ulnclaw version instead of the Electron app's own package.json version,
// which historically drifted (stuck at 0.0.2). Falls back to app.getVersion()
// when the source tree can't be read (e.g. a packaged build without the repo).
function resolveulnclawVersion() {
  try {
    const root = resolveUpdateRoot()
    const initPath = path.join(root, 'ulnclaw_cli', '__init__.py')
...
// Re-resolve the live ulnclaw version and push it into the native About panel
// just before showing it, so an in-place `ulnclaw update` is reflected without
...
ipcMain.handle('ulnclaw:version', async () => ({
  appVersion: resolveulnclawVersion(),
  electronVersion: process.versions.electron,
  nodeVersion: process.versions.node,
  platform: process.platform,
  ulnclawRoot: resolveUpdateRoot()
}))
...
// the actual removal via `ulnclaw uninstall …` so the cross-platform PATH /
// registry / service / node-symlink cleanup all lives in one place
// (ulnclaw_cli/uninstall.py + ulnclaw_cli/gui_uninstall.py).
//
...
async function getUninstallSummary() {
  ...
  const agentRoot = ACTIVE_ULNCLAW_ROOT
  ...
  const fallback = () => ({
    ulnclaw_home: ULNCLAW_HOME,
    agent_installed: isulnclawSourceRoot(agentRoot) && fileExists(py),
    gui_installed: true,
  ...
  return new Promise(resolve => {
    ...
    try {
      const child = spawn(
        py,
        ['-m', 'ulnclaw_cli.main', 'uninstall', '--gui-summary'],
        hiddenWindowsChildOptions({
          cwd: agentRoot,
          env: { ...process.env, ULNCLAW_HOME, NO_COLOR: '1' },
          stdio: ['ignore', 'pipe', 'ignore']
        })
      )
...
async function runDesktopUninstall(mode) {
  ...
  if (!fileExists(venvPy)) {
    return {
      ok: false,
      error: 'agent-missing',
      message: `Can't run the uninstaller: no ulnclaw agent venv at ${VENV_ROOT}.`
    }
  }
  ...
  // Python with PYTHONPATH=<agentRoot> so `import ulnclaw_cli` resolves from
  ...
  if (modeRemovesAgent(mode)) {
    ...
    if (sysPy) {
      py = sysPy
      pythonPath = ACTIVE_ULNCLAW_ROOT
    } else if (IS_WINDOWS) {
      rememberLog(
  ...
  try {
    await releaseBackendLock(ACTIVE_ULNCLAW_ROOT, 'uninstall')
  } catch (error) {
  ...
  try {
    if (IS_WINDOWS) {
      scriptPath = path.join(app.getPath('temp'), `ulnclaw-uninstall-${Date.now()}.cmd`)
      ...
      scriptPath = path.join(app.getPath('temp'), `ulnclaw-uninstall-${Date.now()}.sh`)
...
ipcMain.handle('ulnclaw:uninstall:summary', async () => getUninstallSummary())
ipcMain.handle('ulnclaw:uninstall:run', async (_event, payload) => {
...
ipcMain.handle('ulnclaw:vscode-theme:fetch', async (_event, id) => fetchMarketplaceThemes(String(id || '')))

// Search the Marketplace for color-theme extensions (empty query = top installs).
ipcMain.handle('ulnclaw:vscode-theme:search', async (_event, query) => searchMarketplaceThemes(String(query || ''), 20))

// ---------------------------------------------------------------------------
// ulnclaw:// deep links (e.g. ulnclaw://blueprint/morning-brief?time=08:00).
// A docs/dashboard "Send to App" button opens this URL; we route it into the
// running app's chat composer. Three delivery paths: macOS 'open-url',
// Win/Linux running-app 'second-instance' (argv), Win/Linux cold-start argv.
// ---------------------------------------------------------------------------
const ULNCLAW_PROTOCOL = 'ulnclaw'
...
function _extractDeepLink(argv) {
  ...
  return argv.find(a => typeof a === 'string' && a.startsWith(`${ULNCLAW_PROTOCOL}://`)) || null
}
...
function handleDeepLink(url) {
  ...
  // ulnclaw://blueprint/<key>?slot=val  -> host="blueprint", path="/<key>"
  ...
  try {
    ...
    mainWindow.focus()
    mainWindow.webContents.send('ulnclaw:deep-link', payload)
...
ipcMain.handle('ulnclaw:deep-link-ready', () => {
  ...
  if (_pendingDeepLink) {
    ...
    handleDeepLink(
      `${ULNCLAW_PROTOCOL}://${queued.kind}/${encodeURIComponent(queued.name)}` +
...
function registerDeepLinkProtocol() {
  try {
    if (process.defaultApp && process.argv.length >= 2) {
      ...
      app.setAsDefaultProtocolClient(ULNCLAW_PROTOCOL, process.execPath, [path.resolve(process.argv[1])])
    } else {
      app.setAsDefaultProtocolClient(ULNCLAW_PROTOCOL)
    }
  } catch (err) {
...
// second-instance argv. Without the lock a second `ulnclaw://` launch spawns a
...
app.on('before-quit', event => {
  ...
  // quitting ulnclaw never keeps another app's chord hostage.
