...
//   · its own ULNCLAW_HOME      → its own backend + sessions, no shared state.
//   · its own --remote-debugging-port → a private CDP endpoint.
//   · ULNCLAW_DESKTOP_BOOT_FAKE=1 → deterministic boot overlay.
...
// Seed an isolated ULNCLAW_HOME with just enough config (NOT sessions) so the
// spawned instance reaches an empty chat view instead of the onboarding wizard.
// A separate ULNCLAW_HOME dir means a separate gateway lock — no collision with
...
export async function startIsolatedInstance({
  ...
  const home = ulnclawHome ?? mkTemp('ulnclaw-perf-home-')
  const userData = userDataDir ?? mkTemp('ulnclaw-perf-ud-')
  const devUrl = prod ? null : `http://127.0.0.1:${devPort}`

  if (seedConfig && !ulnclawHome) {
    seedConfigFrom(join(homedir(), '.ulnclaw'), home)
  }
  ...
  try {
    ...
    // ULNCLAW_HOME (backend + sessions). No DEV_SERVER env in prod → dist load.
    const electronBin = require('electron')
    // NB: do NOT set ULNCLAW_DESKTOP_BOOT_FAKE here — it injects artificial
    ...
    if (devUrl) {
      env.ULNCLAW_DESKTOP_DEV_SERVER = devUrl
    }
...
export async function coldStartSamples({ runs = 3, port = 9222, devPort = 5174, prod = false, warm = true } = {}) {
  ...
  if (warm) {
    ...
    const home = mkdtempSync(join(tmpdir(), 'ulnclaw-perf-cold-home-'))
    const userDataDir = mkdtempSync(join(tmpdir(), 'ulnclaw-perf-cold-ud-'))
    seedConfigFrom(join(homedir(), '.ulnclaw'), home)
