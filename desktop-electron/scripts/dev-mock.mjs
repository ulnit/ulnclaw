...
function createSandbox() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `ulnclaw-dev-mock-${Date.now()}`))
  const ulnclawHome = path.join(root, 'ulnclaw-home')
  const userDataDir = path.join(root, 'electron-user-data')
  fs.mkdirSync(ulnclawHome, { recursive: true })
  fs.mkdirSync(userDataDir, { recursive: true })
  return { root, ulnclawHome, userDataDir, cleanup: () => fs.rmSync(root, { recursive: true, force: true }) }
}

function writeMockConfig(ulnclawHome, mockUrl) {
  fs.writeFileSync(
    path.join(ulnclawHome, 'config.yaml'),
...
`,
  ...
  fs.writeFileSync(path.join(ulnclawHome, '.env'), 'MOCK_API_KEY=e2e-mock-key\n', 'utf8')
}
...
async function main() {
  ...
  writeMockConfig(sandbox.ulnclawHome, mock.url)
  console.log(`  ULNCLAW_HOME: ${sandbox.ulnclawHome}`)

  const electronBin = findElectron()

  const env = {
    ...process.env,
    ULNCLAW_HOME: sandbox.ulnclawHome,
    ULNCLAW_DESKTOP_USER_DATA_DIR: sandbox.userDataDir,
    ULNCLAW_DESKTOP_IGNORE_EXISTING: '1',
    ULNCLAW_DESKTOP_ULNCLAW_ROOT: REPO_ROOT,
    ULNCLAW_DESKTOP_APP_NAME: `ulnclawDevMock-${Date.now()}`,
  }
