...
// no longer bundles the ulnclaw Agent Python payload (that's fetched at first
// launch via install.ps1 / install.sh, per the Phase 1 thin-installer flow).
const APP = (() => {
  if (PLATFORM === 'darwin') {
    const appPath = path.join(RELEASE_ROOT, `mac-${ARCH}`, 'ulnclaw.app')
    return {
      appPath,
      binary: path.join(appPath, 'Contents', 'MacOS', 'ulnclaw'),
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      appPath: unpacked,
      binary: path.join(unpacked, 'ulnclaw.exe'),
  ...
  return {
    appPath: unpacked,
    binary: path.join(unpacked, 'ulnclaw'),
    resourcesPath: path.join(unpacked, 'resources'),
    asarPath: path.join(unpacked, 'resources', 'app.asar'),
    unpackedDistIndex: path.join(unpacked, 'resources', 'app.asar.unpacked', 'dist', 'index.html')
  }
...
// Default ULNCLAW_HOME for non-sandboxed runs -- matches main.ts's
// resolveulnclawHome(). On Windows it's %LOCALAPPDATA%\ulnclaw; elsewhere
// it's ~/.ulnclaw. The fresh-install sandbox launchFresh() sets its own
// ULNCLAW_HOME and never touches this.
const DEFAULT_ULNCLAW_HOME = (() => {
  if (PLATFORM === 'win32' && process.env.LOCALAPPDATA) {
    return path.join(process.env.LOCALAPPDATA, 'ulnclaw')
  }
  return path.join(os.homedir(), '.ulnclaw')
})()
const VENV_ROOT = path.join(DEFAULT_ULNCLAW_HOME, 'ulnclaw-agent', 'venv')
const FRESH_SANDBOX_ROOT = path.join(os.tmpdir(), 'ulnclaw-desktop-fresh-install')
...
function ensurePackagedApp() {
  if (process.env.ULNCLAW_DESKTOP_SKIP_BUILD === '1' && exists(APP.binary)) {
    return
  }
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `ulnclaw-${PACKAGE_JSON.version}-${ARCH}.dmg`)
  }

  const prefix = `ulnclaw-${PACKAGE_JSON.version}`
  const candidates = fs
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `ulnclaw-${PACKAGE_JSON.version}-${ARCH}.dmg`)
}

function resolveNsisPath() {
  // electron-builder NSIS artifactName template is 'ulnclaw-${version}-${os}-${arch}.${ext}'
...
function ensureDmg() {
  ...
  if (process.env.ULNCLAW_DESKTOP_SKIP_BUILD === '1' && exists(resolveDmgPath())) {
    return
  }
...
function ensureNsis() {
  ...
  if (process.env.ULNCLAW_DESKTOP_SKIP_BUILD === '1' && resolveNsisPath()) {
    return
  }
...
function launchFresh() {
  ...
  const ulnclawHome = path.join(sandbox, 'ulnclaw-home')
  const cwd = path.join(sandbox, 'workspace')

  fs.mkdirSync(userDataDir, { recursive: true })
  fs.mkdirSync(ulnclawHome, { recursive: true })
  ...
  env.ULNCLAW_DESKTOP_CWD = cwd
  env.ULNCLAW_DESKTOP_IGNORE_EXISTING = '1'
  env.ULNCLAW_DESKTOP_TEST_MODE = 'fresh-install'
  env.ULNCLAW_DESKTOP_USER_DATA_DIR = userDataDir
  env.ULNCLAW_HOME = ulnclawHome
  delete env.ULNCLAW_DESKTOP_ULNCLAW
  delete env.ULNCLAW_DESKTOP_ULNCLAW_ROOT
  ...
  console.log(`  ULNCLAW_HOME: ${ulnclawHome}`)
  console.log(`  cwd: ${cwd}`)

  return { runtimeRoot: path.join(ulnclawHome, 'ulnclaw-agent', 'venv') }
}

// Validate the packaged bundle matches the thin-installer architecture:
//   - The ulnclaw Agent Python payload is NOT shipped (it's fetched at first
...
function validateBundle() {
  ...
  // present anymore. If a stray ship of ulnclaw_cli sneaks back in we want
  // to fail loudly rather than re-introduce the 400MB delta we just removed.
  const staleFactoryMarker = path.join(APP.resourcesPath, 'ulnclaw-agent', 'ulnclaw_cli', 'main.py')
...
function help() {
  console.log(`Usage:
  npm run test:desktop:existing  # build packaged app, launch with normal PATH/existing ulnclaw
  npm run test:desktop:fresh     # build packaged app, launch with temp userData + ULNCLAW_HOME
...
Fast rerun (skip rebuild if the packaged app already exists):
  ULNCLAW_DESKTOP_SKIP_BUILD=1 npm run test:desktop:fresh
`)
}
