 ...
 * product name (`ulnclaw`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/ulnclaw'
 *
 * This is a hard failure with no obvious cause for the user — `ulnclaw desktop`
 ...
 * (the mac-unpacked ulnclaw.app bundle) and Windows (win-unpacked), so we
 ...
 * ulnclaw.exe that Windows can't load (truncated PE from a corrupt cached
 * Electron zip, wrong arch), the updater's integrity gate in
 * `ulnclaw desktop --build-only` (ulnclaw_cli/main.py
 ...
 */
export function preserveRollbackBackup(appOutDir, productExeName = 'ulnclaw.exe') {
...
export default async function beforePack(context) {
  ...
  try {
    ...
    // rename fails.
    const productExe = `${(context && context.packager?.appInfo?.productFilename) || 'ulnclaw'}.exe`
