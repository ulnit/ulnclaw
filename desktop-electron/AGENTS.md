...
How to build ulnclaw Desktop well. This is a judgment guide, not an inventory —
...
- The **backend** is authoritative for anything another ulnclaw surface can also
...
- A **runtime home change** (switching the underlying `ULNCLAW_HOME` profile) is
...
its shape. "Plugin" means several unrelated things across ulnclaw — do not assume
