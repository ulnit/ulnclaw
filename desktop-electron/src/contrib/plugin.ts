 ...
 * `ulnclawPlugin`; it never touches the registry directly — it receives a
...
import { pluginRest, type PluginRestOptions, pluginSocket } from '@/ulnclaw'
...
export type { PluginRestOptions } from '@/ulnclaw'
 ...
 *  under `ulnclaw.plugin.<id>.` — plugins can't read or clobber each other. */
...
export interface ulnclawPlugin {
...
function createPluginStorage(pluginId: string): PluginStorage {
  const scoped = (key: string) => `ulnclaw.plugin.${pluginId}.${key}`
