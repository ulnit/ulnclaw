...
export interface OAuthProvider {
   ...
   *  embedded terminal. Null when ulnclaw doesn't know how to remove it. */
...
export interface EnvVarInfo {
  ...
  // in ulnclaw_cli/provider_catalog.py). When present, the Keys tab groups by
  // this provider identity — the SAME one `ulnclaw model` uses — instead of
...
export interface ulnclawConfig {
  agent?: {
...
export type ulnclawConfigRecord = Record<string, unknown>
...
export interface ModelOptionProvider {
   ...
   *  the `ulnclaw model` CLI / onboarding OAuth flow. */
  auth_type?: string
...
// folders. Mirrors ulnclaw_cli/projects_db.Project.to_dict().
...
export interface SkillInfo {
  ...
  usage?: number
  /** 'agent' = learned/local (editable), 'bundled' = ships with ulnclaw, 'hub' = installed. */
...
export interface ToolProvider {
   ...
   *  CLI `ulnclaw tools` active-provider detection). */
  is_active: boolean
 ...
 *  not ulnclaw); elsewhere, driver health from `cua-driver doctor`. `null`
...
/** Shape of `GET /api/ulnclaw/update/check` — the backend's own update state.
