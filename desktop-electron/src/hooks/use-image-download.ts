...
function isMissingIpcHandler(error: unknown): boolean {
  ...
  return message.includes("No handler registered for 'ulnclaw:saveImageFromUrl'")
}
...
export function useImageDownload(src?: string) {
  ...
  const download = useCallback(async () => {
    ...
    try {
      if (window.ulnclawDesktop?.saveImageFromUrl) {
        if (await window.ulnclawDesktop.saveImageFromUrl(src)) {
