...
import { getulnclawConfigRecord, type ulnclawConfigRecord, saveulnclawConfig } from '@/ulnclaw'
...
export interface I18nConfigClient {
  getConfig: () => Promise<ulnclawConfigRecord>
  saveConfig: (config: ulnclawConfigRecord) => Promise<{ ok: boolean }>
}

const defaultConfigClient: I18nConfigClient = {
  getConfig: () => {
    if (typeof window === 'undefined' || !window.ulnclawDesktop?.api) {
      return Promise.resolve({})
    }

    return getulnclawConfigRecord()
  },
  saveConfig: config => {
    if (typeof window === 'undefined' || !window.ulnclawDesktop?.api) {
    ...
    return saveulnclawConfig(config)
  }
}
...
export function getConfigDisplayLanguage(config: ulnclawConfigRecord): unknown {
  return isRecord(config.display) ? config.display.language : undefined
}

export function withConfigDisplayLanguage(config: ulnclawConfigRecord, locale: Locale): ulnclawConfigRecord {
