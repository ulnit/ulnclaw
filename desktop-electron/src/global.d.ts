import type { GatewayWsUrlResult } from '@ulnclaw/shared'
...
declare global {
  interface Window {
    ulnclawDesktop: {
      ...
      getConnection: (profile?: string | null) => Promise<ulnclawConnection>
      ...
      // ulnclaw Cloud: one portal login powers discovery + silent per-agent
      ...
      profile: {
        ...
        // backend under the new ULNCLAW_HOME (reloads the window). Pass null to
        // clear the preference.
        set: (name: string | null) => Promise<DesktopActiveProfile>
      }
      api: <T>(request: ulnclawApiRequest) => Promise<T>
      notify: (payload: ulnclawNotification) => Promise<boolean>
      ...
      readFileText: (filePath: string) => Promise<ulnclawReadFileTextResult>
      selectPaths: (options?: ulnclawSelectPathsOptions) => Promise<string[]>
      ...
      normalizePreviewTarget: (target: string, baseDir?: string) => Promise<ulnclawPreviewTarget | null>
      watchPreviewFile: (url: string) => Promise<ulnclawPreviewWatch>
      ...
      watchDirectory?: (dir: string) => Promise<ulnclawPreviewWatch>
      stopPreviewFileWatch: (id: string) => Promise<boolean>
      setActiveWork?: (payload: ulnclawActiveWork) => void
      setTitleBarTheme?: (payload: ulnclawTitleBarTheme) => void
      ...
      readDir: (path: string) => Promise<ulnclawReadDirResult>
      ...
      // Local Desktop runtime-plugin root (<ULNCLAW_HOME>/desktop-plugins),
      ...
      git?: {
        worktreeList: (repoPath: string) => Promise<ulnclawGitWorktree[]>
        worktreeAdd: (
          repoPath: string,
        ...
        branchList: (repoPath: string) => Promise<ulnclawGitBranch[]>
        ...
        baseBranchList: (repoPath: string) => Promise<ulnclawGitBaseBranch[]>
        ...
        repoStatus: (repoPath: string) => Promise<ulnclawRepoStatus | null>
        ...
        review: {
          list: (repoPath: string, scope: ulnclawReviewScope, baseRef?: null | string) => Promise<ulnclawReviewList>
          diff: (
            repoPath: string,
            filePath: string,
            scope: ulnclawReviewScope,
            baseRef?: null | string,
            staged?: boolean
          ) => Promise<string>
          ...
          shipInfo: (repoPath: string) => Promise<ulnclawReviewShipInfo>
      ...
      terminal: {
        ...
        onExit: (id: string, callback: (payload: ulnclawTerminalExit) => void) => () => void
        resize: (id: string, size: { cols: number; rows: number }) => Promise<boolean>
        start: (options?: { cols?: number; cwd?: string; rows?: number }) => Promise<ulnclawTerminalSession>
      ...
      onWindowStateChanged?: (callback: (payload: ulnclawWindowState) => void) => () => void
      onFocusSession?: (callback: (sessionId: string) => void) => () => void
      onNotificationAction?: (callback: (payload: { actionId: string; sessionId?: string }) => void) => () => void
      onPreviewFileChanged: (callback: (payload: ulnclawPreviewFileChanged) => void) => () => void
...
export interface ulnclawTerminalSession {
  cwd: string
  id: string
  shell: string
}

export interface ulnclawTerminalExit {
  code: number | null
  signal: string | null
}
...
export interface DesktopUpdateApplyResult {
   ...
   *  `ulnclaw update` themselves. `command` is the exact line to run. */
  manual?: boolean
  command?: string
  ulnclawRoot?: string
...
export interface ulnclawConnection {
  baseUrl: string
...
export interface ulnclawTitleBarTheme {
  background: string
  foreground: string
}

/** Turns in flight, so the main process can confirm before a quit kills them. */
export interface ulnclawActiveWork {
  count: number
  titles: string[]
}

export interface ulnclawWindowState {
  isFullscreen: boolean
  isMinimized?: boolean
  isVisible?: boolean
  nativeOverlayWidth: number
...
export interface DesktopConnectionConfig {
  envOverride: boolean
  // The saved connection mode. 'cloud' is a ulnclaw Cloud connection: it carries
  ...
  remoteUrl: string
  // For a 'cloud' connection: the persisted ulnclaw Cloud org (slug or id) the
...
export interface DesktopConnectionConfigInput {
  ...
  remoteToken?: string
  remoteUrl?: string
  // For a 'cloud' connection: the selected ulnclaw Cloud org (slug or id) to
...
export interface DesktopCloudStatus {
  ...
  // the ulnclaw gateway session cookies. See cookiesHavePrivySession.
  signedIn: boolean
}

// A discovered ulnclaw Cloud agent — the trimmed DTO from NAS GET /api/agents.
...
export interface ulnclawApiRequest {
  path: string
  method?: string
  body?: unknown
...
export interface ulnclawNotification {
  title?: string
  body?: string
  silent?: boolean
  kind?: string
...
export interface ulnclawPreviewTarget {
  binary?: boolean
  byteSize?: number
  kind: 'file' | 'url'
  label: string
...
export interface ulnclawReadFileTextResult {
  binary?: boolean
  byteSize?: number
  language?: string
  mimeType?: string
...
export interface ulnclawPreviewWatch {
  id: string
  path: string
}
...
export interface ulnclawGitWorktree {
  path: string
  branch: null | string
  isMain: boolean
  detached: boolean
...
export interface ulnclawGitBranch {
  name: string
  checkedOut: boolean
  isDefault: boolean
...
// preselect it.
export interface ulnclawGitBaseBranch {
  name: string
  isRemote: boolean
  isDefault: boolean
}
...
export interface ulnclawRepoStatusFile {
  path: string
  staged: boolean
  unstaged: boolean
  untracked: boolean
...
export interface ulnclawRepoStatus {
  branch: null | string
...
// turn began.
export type ulnclawReviewScope = 'branch' | 'lastTurn' | 'uncommitted'

// One changed file in the review pane (status letter, +/- lines, staged flag).
export interface ulnclawReviewFile {
  path: string
  added: number
  removed: number
...
export interface ulnclawReviewList {
  files: ulnclawReviewFile[]
...
export interface ulnclawReviewPr {
  url: string
  state: string
  number: number
}
...
export interface ulnclawReviewShipInfo {
  ghReady: boolean
  pr: ulnclawReviewPr | null
}

export interface ulnclawReadDirEntry {
  name: string
  path: string
  isDirectory: boolean
}

export interface ulnclawReadDirResult {
  entries: ulnclawReadDirEntry[]
  error?: string
}

export interface ulnclawPreviewFileChanged {
  id: string
  path: string
  url: string
}

export interface ulnclawSelectPathsOptions {
  title?: string
  defaultPath?: string
  directories?: boolean
  multiple?: boolean
