...
const ULNCLAW_REF_TYPES = WIRE_REFERENCE_KINDS
type ulnclawRefType = (typeof ULNCLAW_REF_TYPES)[number]
...
const ULNCLAW_DIRECTIVE_RE = referenceRe()
...
export const ulnclawDirectiveFormatter: Unstable_DirectiveFormatter = {
...
function parseDirectiveText(text: string): Unstable_DirectiveSegment[] {
  const matches = [
    ...
    ...Array.from(text.matchAll(ULNCLAW_DIRECTIVE_RE)).map(match => {
...
function safeDirectiveSegments(text: string): Unstable_DirectiveSegment[] {
  try {
    return [...ulnclawDirectiveFormatter.parse(text)]
  } catch {
...
/**
 * Renders text containing ulnclaw directives (`@file:...`, `@image:...`) as
...
const DirectiveImage: FC<{ id: string; label: string }> = ({ id, label }) => {
  ...
  useEffect(() => {
    ...
    const load =
      window.ulnclawDesktop && isRemoteGateway() ? gatewayMediaDataUrl(id) : window.ulnclawDesktop?.readFileDataUrl(id)
