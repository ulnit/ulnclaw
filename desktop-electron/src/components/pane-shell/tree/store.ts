...
const STORAGE_KEY = 'ulnclaw.desktop.layoutTree.v2'

writeKey('ulnclaw.desktop.layoutTree.v1', null)
 ...
 */
export const $activePresetId = atom<string>(readKey('ulnclaw.desktop.layoutPreset.active') ?? 'default')

export function markActivePreset(id: string) {
  $activePresetId.set(id)
  writeKey('ulnclaw.desktop.layoutPreset.active', id)
}
 ...
 */
const DISMISSED_KEY = 'ulnclaw.desktop.dismissedPanes.v1'
...
const USER_PLACED_KEY = 'ulnclaw.desktop.userPlacedPanes.v1'
...
if ((import.meta.env.DEV || import.meta.env.VITE_PERF_PROBE === '1') && typeof window !== 'undefined') {
  ;(window as unknown as Record<string, unknown>).__ULNCLAW_LAYOUT_TREE__ = {
    close: closeTreePane,
