...
import type { ulnclawGitBranch } from '@/global'
...
const branchActionLabel = (branch: ulnclawGitBranch, copy: BranchActionCopy) => {
...
export function WorktreeDialog({ repoPath, onStarted, open, onOpenChange, initialBase }: WorktreeDialogProps) {
  ...
  const [branches, setBranches] = useState<ulnclawGitBranch[]>([])
  ...
  const convert = async (branch: ulnclawGitBranch) => {
    if (pending || !repoPath || !branch) {
      return
    }
