...
import type { ulnclawGitWorktree } from '@/global'
import type { SessionInfo } from '@/ulnclaw'
...
// zero ulnclaw sessions, and those arrive continuously — so once the user dragged
...
export function useRepoWorktreeMap(
  repoPaths: string[],
  enabled: boolean
): [Record<string, ulnclawGitWorktree[]>, boolean] {
  const [map, setMap] = useState<Record<string, ulnclawGitWorktree[]>>({})
