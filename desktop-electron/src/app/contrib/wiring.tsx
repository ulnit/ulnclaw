...
import { getSessionMessages, triggerCronJob } from '@/ulnclaw'
...
import { useulnclawConfig } from '../session/hooks/use-ulnclaw-config'
...
export function ContribWiring({ children }: { children: ReactNode }) {
  ...
  const { refreshulnclawConfig, sttEnabled, voiceMaxRecordingSeconds } = useulnclawConfig({ activeSessionIdRef })
  ...
  useEffect(() => {
    ...
    window.addEventListener('ulnclaw:open-keybinds', onOpenKeybinds)

    return () => window.removeEventListener('ulnclaw:open-keybinds', onOpenKeybinds)
  }, [navigate])
  ...
  useEffect(() => {
    ...
    void refreshulnclawConfig(true)
    void refreshActiveProfile()
  }, [activeGatewayProfile, refreshCurrentModel, refreshulnclawConfig])
