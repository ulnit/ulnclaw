import { isGatewayReauthRequired, resolveGatewayWsUrl } from '@ulnclaw/shared'
import { useEffect, useRef } from 'react'

import type { ulnclawConnection } from '@/global'
import { ulnclawGateway } from '@/ulnclaw'
...
import type { RpcEvent } from '@/types/ulnclaw'
...
interface GatewayBootOptions {
  ...
  onConnectionReady: (
    connection: Awaited<ReturnType<NonNullable<typeof window.ulnclawDesktop>['getConnection']>> | null
  ) => void
  onGatewayReady: (gateway: ulnclawGateway | null) => void
  refreshulnclawConfig: () => Promise<void>
...
export function useGatewayBoot({
  ...
  useEffect(() => {
    ...
    const publish = (next: ulnclawConnection | null) => {
    ...
    // composer stayed disabled forever on "Starting ulnclaw...". Once the
    ...
    const attemptReconnect = async () => {
      ...
      try {
        ...
        // "Starting ulnclaw…". The probe is a no-op for a healthy or local backend.
        ...
        // as an opaque "Could not connect to ulnclaw gateway". resolveGatewayWsUrl
        ...
        await callbacksRef.current.refreshulnclawConfig().catch(() => undefined)
    ...
    const softSwitch = async () => {
      ...
      try {
        ...
        await Promise.all([
          seedDefaultCwd(),
          callbacksRef.current.refreshulnclawConfig().catch(() => undefined),
    ...
    const offBootProgress = desktop.onBootProgress(payload => {
      // Soft switch / post-boot startulnclaw re-emits progress — ignore so the
    ...
    const gateway = adoptedFromHmr ? survivor!.gateway : new ulnclawGateway()
    ...
    async function boot() {
      try {
        ...
        await Promise.all([
          seedDefaultCwd(),
          callbacksRef.current.refreshulnclawConfig(),
          ...
          // retry it), not brick boot behind the "ulnclaw couldn't start"
