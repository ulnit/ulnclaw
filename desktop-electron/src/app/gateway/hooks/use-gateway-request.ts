...
export function useGatewayRequest() {
  ...
  const gateway = useStore($gateway) as ulnclawGateway | null
  const gatewayRef = useRef<ulnclawGateway | null>(null)

  const connectionRef = useRef<Awaited<ReturnType<NonNullable<typeof window.ulnclawDesktop>['getConnection']>> | null>(
    null
  )

  const gatewayStateRef = useRef(gatewayState)
  const reconnectingRef = useRef<Promise<ulnclawGateway | null> | null>(null)
  ...
  useEffect(
    () =>
      $gateway.subscribe(gateway => {
        gatewayRef.current = gateway as ulnclawGateway | null
      }),
    []
  )
  ...
  const requestGateway = useCallback(
    async <T>(method: string, params: Record<string, unknown> = {}, timeoutMs?: number, signal?: AbortSignal) => {
      ...
      if (!gateway) {
        throw new Error('ulnclaw gateway unavailable')
      }
