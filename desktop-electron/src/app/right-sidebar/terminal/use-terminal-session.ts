...
const ULNCLAW_PATHS_MIME = 'application/x-ulnclaw-paths'
...
function transferHasDropCandidates(t: DataTransfer): boolean {
  if (t.types?.includes(ULNCLAW_PATHS_MIME)) {
    return true
  }
...
function collectDroppedPaths(t: DataTransfer): string[] {
  ...
  try {
    const raw = t.getData(ULNCLAW_PATHS_MIME)
...
export function useTerminalSession({
  ...
  useEffect(() => {
    ...
    const terminalApi = window.ulnclawDesktop?.terminal
    ...
    const term = new Terminal({
      ...
      // Full-screen TUIs (ulnclaw --tui, vim) grab the mouse, so a plain drag
    ...
    term.attachCustomKeyEventHandler(event => {
      ...
      void (async () => {
        const text = (await window.ulnclawDesktop?.readClipboard?.()) ?? ''
    ...
    const mount = () => {
      ...
      try {
        ...
        console.warn('[ulnclaw-terminal] WebGL unavailable; falling back to DOM', err)
      }
  ...
  useEffect(() => {
    ...
    return $terminalInjection.subscribe(command => {
      ...
      hasSessionActivityRef.current = true
      void window.ulnclawDesktop?.terminal?.write(sessionId, `${command}\r`)
