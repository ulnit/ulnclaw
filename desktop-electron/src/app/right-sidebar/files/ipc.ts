...
import type { ulnclawReadDirEntry, ulnclawReadDirResult } from '@/global'
...
export type ProjectTreeEntry = ulnclawReadDirEntry
...
function ignoredBy(rules: GitignoreRule[], entry: ulnclawReadDirEntry) {
...
async function filterIgnored(entries: ulnclawReadDirEntry[], rootPath: string, dirPath: string) {
...
export async function readProjectDir(dirPath: string, rootPath = dirPath): Promise<ulnclawReadDirResult> {
  if (!window.ulnclawDesktop) {
