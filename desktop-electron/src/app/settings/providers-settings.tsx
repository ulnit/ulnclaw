...
import { disconnectOAuthProvider, listOAuthProviders } from '@/ulnclaw'
...
import type { EnvVarInfo, OAuthProvider } from '@/types/ulnclaw'
...
const canRunInTerminal = () => typeof window !== 'undefined' && Boolean(window.ulnclawDesktop?.terminal)
...
//      in ulnclaw_cli/provider_catalog.py) — the SAME provider identity
//      `ulnclaw model` uses. This is authoritative: a provider tagged by the
...
function ConnectedProviderRow({
  ...
  // ulnclaw can clear this provider's creds via the API.
  const canDisconnect = provider.disconnectable ?? provider.flow !== 'external'
  // External (CLI-managed) provider ulnclaw can't clear via the API, but ships a
...
export function ProvidersSettings({
  ...
  // ulnclaw never deletes creds another tool owns behind a silent API call.
