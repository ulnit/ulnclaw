...
// ulnclaw Cloud discovery lifecycle for the cloud-mode panel.
...
export function GatewaySettings({ embedded = false }: { embedded?: boolean } = {}) {
  ...
  // --- ulnclaw Cloud (cloud mode) state ---
  ...
  useEffect(() => {
    let cancelled = false
    const desktop = window.ulnclawDesktop
  ...
  useEffect(() => {
    if (state.mode !== 'ssh' || !window.ulnclawDesktop?.sshConfigHosts) {
      return
    }

    let cancelled = false
    void window.ulnclawDesktop
      .sshConfigHosts()
      .then(result => {
        if (!cancelled) {
  ...
  const payload = () => ({
    ...
    sshRemoteulnclawPath: state.sshRemoteulnclawPath.trim(),
  ...
  const save = async (apply: boolean) => {
    ...
    try {
      const next = apply
        ? await window.ulnclawDesktop.applyConnectionConfig(payload())
        : await window.ulnclawDesktop.saveConnectionConfig(payload())
      ...
      const errors = {
        ...
        'ulnclaw-not-found': g.sshErrNotInstalled,
  ...
  const signIn = async () => {
    ...
    try {
      ...
      const saved = await window.ulnclawDesktop.saveConnectionConfig({
        mode: state.mode,
        profile: scope ?? undefined,
      ...
      const result = await window.ulnclawDesktop.oauthLoginConnectionConfig(trimmedUrl)
      ...
      if (result.connected) {
        const refreshed = await window.ulnclawDesktop.getConnectionConfig(scope)
  ...
  const signOut = async () => {
    ...
    try {
      await window.ulnclawDesktop.oauthLogoutConnectionConfig(trimmedUrl || undefined)
      const refreshed = await window.ulnclawDesktop.getConnectionConfig(scope)
  ...
  const resolveSshHost = async (host: string) => {
    if (!host || !window.ulnclawDesktop?.sshResolveHost) {
      return
    }
    ...
    try {
      const resolved = await window.ulnclawDesktop.sshResolveHost(host)
  ...
  const testSsh = async () => {
    ...
    try {
      const result = await window.ulnclawDesktop.testConnectionConfig(payload())
  ...
  const testRemote = async () => {
    ...
    try {
      const result = await window.ulnclawDesktop.testConnectionConfig({
        mode: 'remote',
        profile: scope ?? undefined,
        remoteAuthMode: authMode,
        remoteToken: authMode === 'token' ? remoteToken.trim() || undefined : undefined,
  ...
  if (!window.ulnclawDesktop?.getConnectionConfig) {
  ...
  return (
    <SettingsContent bare={embedded}>
      ...
      {/* ulnclaw Cloud panel: one portal sign-in, then a discovered-agent picker
          ...
          <ListRow
            action={
              <Input
                ...
                placeholder="https://gateway.example.com/ulnclaw"
        ...
        <div className="mt-5 grid gap-1">
          ...
          <ListRow
            action={
              <Input
                className={cn('h-8 font-mono', CONTROL_TEXT)}
                onChange={event => setState(current => ({ ...current, sshRemoteulnclawPath: event.target.value }))}
                placeholder={g.sshulnclawPathPlaceholder}
                value={state.sshRemoteulnclawPath}
              />
            }
            description={g.sshulnclawPathDesc}
            title={g.sshulnclawPathTitle}
          />
          {scope !== null ? (
            <ListRow
              action={
      ...
      {embedded ? null : (
        <div className="mt-6 grid gap-1">
          <ListRow
            action={
              <Button onClick={() => void window.ulnclawDesktop?.revealLogs()} size="sm" variant="textStrong">
                <FileText />
                {g.openLogs}
              </Button>
            }
