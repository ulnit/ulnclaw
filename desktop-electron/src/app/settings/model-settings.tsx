...
import { invalidateulnclawConfig, setulnclawConfigCache, useulnclawConfigRecord } from '../hooks/use-config-record'
...
// backend now surfaces the full `ulnclaw model` universe (every canonical
...
// Mirrors `_AUX_TASK_SLOTS` in ulnclaw_cli/web_server.py. Friendly labels and
...
export function ModelSettings({ onMainModelChanged }: ModelSettingsProps) {
  ...
  const { data: config } = useulnclawConfigRecord()
  const setConfig = setulnclawConfigCache
  ...
  const activateApiKeyProvider = useCallback(async () => {
    ...
    try {
      ...
      // `ulnclaw model` curation). Best-effort — fall through to the refreshed
        ...
        {needsSetup && !setupIsApiKey && selectedProviderRow && (
          <p className="mt-2 text-xs text-muted-foreground">
            {selectedProviderRow?.auth_type === 'api_key'
              ...
              : `${selectedProviderRow?.name} signs in through your browser — ulnclaw runs the flow for you.`}
          </p>
        )}
