...
function reveal(file: string) {
  void window.ulnclawDesktop?.revealPath?.(file)?.catch(() => undefined)
}

async function revealPluginsDir() {
  try {
    // Electron owns the local plugin root — deriving it from the backend's
    // ulnclaw_home breaks against a remote backend (#66899).
    const dir = await window.ulnclawDesktop?.desktopPluginsRoot?.()
    ...
    const result = await window.ulnclawDesktop?.openDir?.(dir)
