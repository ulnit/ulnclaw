...
import { getulnclawConfigDefaults, getulnclawConfigRecord, saveulnclawConfig } from '@/ulnclaw'
...
export function SettingsView({ onClose, onConfigSaved, onMainModelChanged }: SettingsPageProps) {
  ...
  const exportConfig = async () => {
    try {
      const cfg = await getulnclawConfigRecord()
      const blob = new Blob([JSON.stringify(cfg, null, 2)], { type: 'application/json' })
      const url = URL.createObjectURL(blob)
      const a = document.createElement('a')
      a.href = url
      a.download = 'ulnclaw-config.json'
      a.click()
  ...
  const resetConfig = async () => {
    ...
    try {
      await saveulnclawConfig(await getulnclawConfigDefaults())
      triggerHaptic('success')
      onConfigSaved?.()
    } catch (err) {
