...
import type { ulnclawConfigRecord, SessionInfo } from '@/types/ulnclaw'
...
function AutoArchiveSetting() {
  ...
  const persist = useCallback(
    async (autoArchive: boolean, archiveDays: number) => {
      ...
      try {
        await saveulnclawConfig(updated)
      } catch (err) {
...
// / Program Files), which buried any files ulnclaw wrote there.
function DefaultProjectDirSetting() {
  ...
  useEffect(() => {
    ...
    // / Storybook / non-Electron context `window.ulnclawDesktop` is
    ...
    const settings = window.ulnclawDesktop?.settings
