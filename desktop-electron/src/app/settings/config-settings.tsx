...
import { getElevenLabsVoices, getulnclawConfigSchema, saveulnclawConfig } from '@/ulnclaw'
...
import type { ConfigFieldSchema, ulnclawConfigRecord } from '@/types/ulnclaw'

import { setulnclawConfigCache, useulnclawConfigRecord } from '../hooks/use-config-record'
...
export function voiceFieldVisible(key: string, config: ulnclawConfigRecord): boolean {
...
export function ConfigSettings({
  ...
  const [config, setConfig] = useState<ulnclawConfigRecord | null>(null)
  const { data: loadedConfig, isError: configLoadFailed, refetch: refetchConfig } = useulnclawConfigRecord()
  ...
  const {
    ...
    queryKey: ['ulnclaw-config-schema'],
    queryFn: getulnclawConfigSchema,
    staleTime: 5 * 60 * 1000
  })
  ...
  useEffect(() => {
    ...
    const t = window.setTimeout(() => {
      void (async () => {
        try {
          const result = await saveulnclawConfig(config)
  ...
  const updateConfig = (next: ulnclawConfigRecord) => {
