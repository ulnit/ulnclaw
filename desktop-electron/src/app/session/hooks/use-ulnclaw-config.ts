...
import { getulnclawConfig, getulnclawConfigDefaults } from '@/ulnclaw'
...
interface ulnclawConfigOptions {
  activeSessionIdRef: MutableRefObject<string | null>
}

export function useulnclawConfig({ activeSessionIdRef }: ulnclawConfigOptions) {
  const [voiceMaxRecordingSeconds, setVoiceMaxRecordingSeconds] = useState(DEFAULT_VOICE_SECONDS)
  const [sttEnabled, setSttEnabled] = useState(true)
  const profileRefreshEpochRef = useRef(0)

  const refreshulnclawConfig = useCallback(
    async (force = false) => {
      if (force) {
      ...
      try {
        const [config, defaults] = await Promise.all([getulnclawConfig(), getulnclawConfigDefaults().catch(() => ({}))])
        ...
        // rendering/applying ulnclaw' built-in medium over the user's config.
  ...
  return { refreshulnclawConfig, sttEnabled, voiceMaxRecordingSeconds }
}
