import type { BillingBlock } from '@ulnclaw/shared'
import type { ulnclawSkin } from '@ulnclaw/shared/skin'
 ...
 * billing-specific toast — never the generic "ulnclaw error" — with a smart CTA
...
export function useGatewayEventHandler(deps: GatewayEventDeps) {
  ...
  // refreshulnclawConfig — two REST calls (config + defaults) per event, per
  ...
  return useCallback(
    (event: RpcEvent) => {
      ...
      if (event.type === 'gateway.ready') {
        ...
        ingestBackendSkin((payload as { skin?: ulnclawSkin } | undefined)?.skin, { apply: false })
        ...
        // A runtime skin switch (ulnclaw activating an authored skin, or `/skin`
        ...
        if (fromActiveProfile) {
          ingestBackendSkin(payload as ulnclawSkin | undefined, { apply: true })
        }
        ...
        if (apply) {
          ...
          // everything refreshulnclawConfig applies is either active-session
        ...
        const failure =
          payload?.status === 'error'
            ? {
                error: coerceGatewayText(payload.error).trim() || finalText || 'ulnclaw reported an error',
        ...
        // native OS notification when ulnclaw is backgrounded; dispatch is gated
        ...
        const errorMessage = payload?.message || 'ulnclaw reported an error'
