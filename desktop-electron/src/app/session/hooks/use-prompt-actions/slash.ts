import { skillInvocationText } from '@ulnclaw/shared'
import { type MutableRefObject, useCallback, useRef } from 'react'

import { getProfiles } from '@/ulnclaw'
...
// 120s RPC budget (ULNCLAW_TUI_RPC_TIMEOUT_MS default) instead.
...
const renderWakeStatus = (status: WakeStatusResponse): string => {
  const lines = [
    ...
    `Phrase: "${status.phrase?.trim() || 'hey ulnclaw'}"`,
  ...
  return useCallback(
    async (rawCommand: string, options?: { sessionId?: string; recordInput?: boolean }) => {
      ...
      const actionHandlers: Record<DesktopActionId, (ctx: SlashActionCtx) => Promise<void>> = {
        ...
        // lease. Sending it through slash.exec creates a separate ulnclawCLI in
