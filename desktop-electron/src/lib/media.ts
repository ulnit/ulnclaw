...
export async function resolveMediaDisplaySrc(path: string): Promise<string> {
  ...
  if (window.ulnclawDesktop && isRemoteGateway()) {
    return gatewayMediaDataUrl(path)
  }

  if (!window.ulnclawDesktop?.readFileDataUrl) {
    return mediaExternalUrl(path)
  }

  return window.ulnclawDesktop.readFileDataUrl(filePathFromMediaPath(path))
}
...
export async function resolveMediaPlaybackSrc(path: string): Promise<string> {
  ...
  if (window.ulnclawDesktop && ['audio', 'video'].includes(mediaKind(path))) {
...
export function mediaStreamUrl(path: string): string {
  return `ulnclaw-media://stream/${encodeURIComponent(filePathFromMediaPath(path))}`
}
...
// (workspace, skills, ~/.ulnclaw/cache, etc.); /api/media is intentionally
