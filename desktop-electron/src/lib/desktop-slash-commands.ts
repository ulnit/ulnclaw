...
/** Why a known ulnclaw command has no desktop UI surface. */
...
const DESKTOP_COMMAND_SPECS: readonly DesktopCommandSpec[] = [
  ...
  { name: '/profile', description: 'Switch the active ulnclaw profile', surface: action('profile') },
  {
    name: '/skin',
  ...
  { name: '/version', description: 'Show ulnclaw Agent version', surface: exec() },
...
function isKnownulnclawSlashCommand(command: string): boolean {
  const normalized = normalizeCommand(command)
 ...
 * ulnclaw' built-in slash commands — i.e. skill commands (`/gif-search`,
...
export function isDesktopSlashExtensionCommand(command: string): boolean {
  ...
  return !isKnownulnclawSlashCommand(normalized)
}
 ...
 * activity — the ones that ship with ulnclaw and were never asked for. It is
