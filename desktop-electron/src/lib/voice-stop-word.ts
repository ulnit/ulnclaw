...
//
// When someone is in a hands-free "Hey ulnclaw" voice chat, the natural way to
...
// phrase (optionally addressed to ulnclaw), so a real turn that merely contains
...
// Optional address prefixes so "ulnclaw stop" / "ok stop" / "hey ulnclaw, stop"
// still count. Stripped before matching the core phrase.
const ADDRESS_PREFIXES: readonly string[] = ['hey ulnclaw', 'hey ulnclaw,', 'ulnclaw', 'ulnclaw,', 'ok', 'okay', 'hey']
...
function stripAddress(text: string): string {
  for (const prefix of ADDRESS_PREFIXES) {
    if (text === prefix) {
      // Bare address ("ulnclaw") is not a stop command on its own.
      continue
    }
 ...
 * to ulnclaw). Returns false for anything that merely contains "stop" as part of
