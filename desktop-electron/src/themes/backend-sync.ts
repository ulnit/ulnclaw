/**
 * Live skin sync from the ulnclaw backend.
 *
 * The backend resolves the active skin (built-in or `$ULNCLAW_HOME/skins/*.yaml`)
 ...
 * stomps the user's persisted desktop theme; only a genuine name change (ulnclaw
 ...
 */
export function ingestBackendSkin(skin: ulnclawSkin | undefined | null, { apply }: { apply: boolean }): void {
  ...
  if (name !== 'default' && !BUILTIN_THEMES[name]) {
    const theme = skinToDesktopTheme(skin as ulnclawSkin)
