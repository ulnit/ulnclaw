...
const SKIN_KEY = 'ulnclaw-desktop-theme-v2'
const MODE_KEY = 'ulnclaw-desktop-mode-v1'
...
const PROFILE_SKINS_KEY = 'ulnclaw-desktop-profile-themes-v1'
const PROFILE_MODES_KEY = 'ulnclaw-desktop-profile-modes-v1'
// Last active profile, recorded so the boot-time paint can pick that profile's
// theme before the gateway reports which profile actually launched.
const LAST_PROFILE_KEY = 'ulnclaw-desktop-active-profile-v1'
...
function applyTheme(theme: DesktopTheme, mode: 'light' | 'dark') {
  ...
  root.dataset.ulnclawTheme = skinName
  root.dataset.ulnclawMode = rendered
  ...
  window.ulnclawDesktop?.setTitleBarTheme?.({
    background: chromeBg,
  ...
  try {
    window.localStorage.setItem('ulnclaw-boot-background', chromeBg)
    window.localStorage.setItem('ulnclaw-boot-color-scheme', rendered)
  } catch {
  ...
  if (typo.fontUrl && !INJECTED_FONT_URLS.has(typo.fontUrl)) {
    ...
    link.dataset.ulnclawThemeFont = 'true'
...
const syncNativeTheme = (pref: ThemeMode, rendered: 'light' | 'dark') =>
  window.ulnclawDesktop?.setNativeTheme?.(pref === 'system' ? 'system' : rendered)
...
export function ThemeProvider({ children }: { children: ReactNode }) {
  ...
  // Drain a backend-driven skin switch (ulnclaw authoring/activating a skin from a
