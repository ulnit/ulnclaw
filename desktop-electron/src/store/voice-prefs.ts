...
import { getulnclawConfigRecord, saveulnclawConfig } from '@/ulnclaw'
...
export async function setAutoSpeakReplies(enabled: boolean): Promise<void> {
  ...
  try {
    const record = await getulnclawConfigRecord()
    const voice = record.voice && typeof record.voice === 'object' ? (record.voice as Record<string, unknown>) : {}

    await saveulnclawConfig({ ...record, voice: { ...voice, auto_tts: enabled } })
  } catch (error) {
