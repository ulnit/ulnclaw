 ...
 * app's bounds and stays visible while ulnclaw is minimized. That window carries
...
const OVERLAY_ACTIVE_KEY = 'ulnclaw.desktop.pet-overlay-active.v1'
const OVERLAY_BOUNDS_KEY = 'ulnclaw.desktop.pet-overlay-bounds.v1'
...
function pushNow(): void {
  window.ulnclawDesktop?.petOverlay?.pushState(currentPayload())
}
...
function openOverlay(request: PetOverlayOpenRequest): void {
  const api = window.ulnclawDesktop?.petOverlay
...
export function restorePetOverlay(): void {
  if (!window.ulnclawDesktop?.petOverlay || !$petOverlayActive.get() || stateUnsubs.length) {
    return
  }
...
export function popInPet(): void {
  ...
  stateUnsubs = []
  $petOverlayActive.set(false)
  void window.ulnclawDesktop?.petOverlay?.close()
}
