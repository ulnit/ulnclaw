...
export function setZoomPercent(percent: number): void {
  window.ulnclawDesktop?.zoom?.setPercent(percent)
}

if (typeof window !== 'undefined' && window.ulnclawDesktop?.zoom) {
  void window.ulnclawDesktop.zoom.get().then(({ percent }) => $zoomPercent.set(percent))
  window.ulnclawDesktop.zoom.onChanged(({ percent }) => $zoomPercent.set(percent))
}
