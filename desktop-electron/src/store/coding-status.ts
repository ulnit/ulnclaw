...
import type { ulnclawGitWorktree, ulnclawRepoStatus } from '@/global'
...
const EMPTY_WORKTREES: ulnclawGitWorktree[] = []
...
export const $repoStatusByCwd = atom<Record<string, ulnclawRepoStatus | null>>({})
export const $repoWorktreesByCwd = atom<Record<string, ulnclawGitWorktree[]>>({})
...
export const $repoStatus: ReadableAtom<ulnclawRepoStatus | null> = computed(
...
export const $repoWorktrees: ReadableAtom<ulnclawGitWorktree[]> = computed(
...
const statusAtomByCwd = new Map<string, ReadableAtom<ulnclawRepoStatus | null>>()
const worktreesAtomByCwd = new Map<string, ReadableAtom<ulnclawGitWorktree[]>>()
const $noRepoStatus = atom<ulnclawRepoStatus | null>(null)
const $noWorktrees = atom<ulnclawGitWorktree[]>(EMPTY_WORKTREES)

/** Reactive status for one repo cwd (a tile's worktree). Stable per cwd. */
export function repoStatusForCwd(cwd?: null | string): ReadableAtom<ulnclawRepoStatus | null> {
...
export function repoWorktreesForCwd(cwd?: null | string): ReadableAtom<ulnclawGitWorktree[]> {
  const key = normalizeCwd(cwd)
...
function setRepoStatusEntry(target: string, status: ulnclawRepoStatus | null): void {
...
function setRepoWorktreesEntry(target: string, worktrees: ulnclawGitWorktree[]): void {
...
interface RepoStatusRefreshRequest {
  probe: (cwd: string) => Promise<ulnclawRepoStatus | null>
  seq: number
  target: string
}
