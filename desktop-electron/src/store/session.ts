import type { ConnectionState } from '@ulnclaw/shared'
import { atom, computed } from 'nanostores'

import { lastVisibleMessageIsUser } from '@/app/chat/thread-loading'
import type { ContextSuggestion } from '@/app/types'
import type { ulnclawConnection } from '@/global'
import type { ChatMessage } from '@/lib/chat-messages'
import { persistBoolean, persistString, storedBoolean, storedString } from '@/lib/storage'
import type { SessionInfo, UsageStats } from '@/types/ulnclaw'

type Updater<T> = T | ((current: T) => T)
export type ComposerModelSource = '' | 'default' | 'manual'

const WORKSPACE_CWD_KEY = 'ulnclaw.desktop.workspace-cwd'
...
const COMPOSER_MODEL_KEY = 'ulnclaw.desktop.composer.model'
const COMPOSER_PROVIDER_KEY = 'ulnclaw.desktop.composer.provider'
const COMPOSER_MODEL_SOURCE_KEY = 'ulnclaw.desktop.composer.model-source'
const COMPOSER_EFFORT_KEY = 'ulnclaw.desktop.composer.reasoning-effort'
const COMPOSER_FAST_KEY = 'ulnclaw.desktop.composer.fast'
...
const LAST_SESSION_KEY = 'ulnclaw.desktop.lastSessionId'
...
const LAST_ROUTE_KEY = 'ulnclaw.desktop.lastRoute'
...
function workspaceCwdKey(connection: ulnclawConnection | null = $connection.get()): string {
...
export async function syncConfiguredDefaultProjectDir(): Promise<string> {
  const settings = window.ulnclawDesktop?.settings?.getDefaultProjectDir
...
export async function ensureDefaultWorkspaceCwd(): Promise<void> {
  const sanitize = window.ulnclawDesktop?.sanitizeWorkspaceCwd
...
export const $connection = atom<ulnclawConnection | null>(null)
...
export const setConnection = (next: Updater<ulnclawConnection | null>) => updateAtom($connection, next)
...
// until config loads, and re-seeded on every profile switch by useulnclawConfig.
