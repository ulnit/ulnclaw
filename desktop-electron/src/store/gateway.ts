import { type ConnectionState, type GatewayEvent, resolveGatewayWsUrl } from '@ulnclaw/shared'
...
const isOpen = (gateway: ulnclawGateway | null): boolean => gateway?.connectionState === 'open'
...
interface GatewayRegistryState {
  ...
  primaryGateway: ulnclawGateway | null
  primaryProfile: string
  activeKey: string
  secondaries: Map<string, Secondary>
  $gateway: ReturnType<typeof atom<ulnclawGateway | null>>
}

const STATE_KEY = Symbol.for('ulnclaw.desktop.gatewayRegistryState')
...
function createRegistryState(): GatewayRegistryState {
  return {
    ...
    $gateway: atom<ulnclawGateway | null>(null)
  }
}
...
export function setPrimaryGateway(gateway: ulnclawGateway | null, profile = 'default'): void {
...
export function activeGateway(): ulnclawGateway | null {
...
function createSecondary(profile: string): Secondary {
  const gateway = new ulnclawGateway()
...
export async function ensureActiveGatewayOpen(): Promise<ulnclawGateway | null> {
  if (g.activeKey === g.primaryProfile) {
    return g.primaryGateway
  }
