...
import type { ulnclawReviewFile, ulnclawReviewShipInfo } from '@/global'
...
//
// Scope is always "uncommitted" — ulnclaw' flow is agent edits you review BEFORE
...
const OPEN_KEY = 'ulnclaw.desktop.reviewOpen'
const COMMIT_DEFAULT_KEY = 'ulnclaw.desktop.reviewCommitDefault'
const TREE_MODE_KEY = 'ulnclaw.desktop.reviewTreeMode'
const SELECTED_KEY = 'ulnclaw.desktop.reviewSelectedPath'
...
export const $reviewFiles = atom<ulnclawReviewFile[]>([])
...
export const $reviewShipInfo = atom<ulnclawReviewShipInfo>({ ghReady: false, pr: null })
...
type ReviewBridge = NonNullable<NonNullable<NonNullable<Window['ulnclawDesktop']>['git']>['review']>
...
export async function selectReviewFile(file: ulnclawReviewFile): Promise<void> {
...
function matchReviewFile(files: readonly ulnclawReviewFile[], path: string): ulnclawReviewFile | undefined {
...
export async function createOrOpenPr(): Promise<void> {
  ...
  if (existing?.url) {
    void window.ulnclawDesktop?.openExternal?.(existing.url)
  ...
  await runShip(async () => {
    ...
    if (url) {
      void window.ulnclawDesktop?.openExternal?.(url)
    }
