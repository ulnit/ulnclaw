...
import { getProfiles, setApiRequestProfile, STARTUP_REQUEST_TIMEOUT_MS } from '@/ulnclaw'
...
import type { ProfileInfo } from '@/types/ulnclaw'
...
// /api/profiles/active `current`). "default" is the root ~/.ulnclaw. This is the
...
const PROFILE_ORDER_STORAGE_KEY = 'ulnclaw.desktop.profileOrder'
...
const PROFILE_COLORS_STORAGE_KEY = 'ulnclaw.desktop.profileColors'
...
export async function refreshActiveProfile(): Promise<void> {
  try {
    const res = await window.ulnclawDesktop.api<ActiveProfileResponse>({
...
// Persist the choice and relaunch the backend under the new ULNCLAW_HOME. The
...
export async function switchProfile(name: string): Promise<void> {
  ...
  await window.ulnclawDesktop.profile.set(name)
}
...
async function syncConnectionToActiveProfile(profile: string): Promise<void> {
  const getConnection = window.ulnclawDesktop?.getConnection
...
const SHOW_ALL_PROFILES_STORAGE_KEY = 'ulnclaw.desktop.showAllProfiles'
...
// Switch to the default (root ~/.ulnclaw) profile — bound to ⌘1.
...
export function touchActiveGatewayBackend(): void {
  ...
  void window.ulnclawDesktop?.touchBackend?.(target).catch(() => undefined)
}
