...
import type { ulnclawGitBaseBranch, ulnclawGitBranch } from '@/global'
import { getulnclawConfig, type ulnclawGateway } from '@/ulnclaw'
...
import type { ProjectInfo, ProjectsPayload } from '@/types/ulnclaw'
...
const PROJECT_SCOPE_KEY = 'ulnclaw.desktop.projectScope'
...
async function gatewayRequest<T>(method: string, params: Record<string, unknown> = {}): Promise<T> {
  ...
  if (!gateway) {
    throw new Error('ulnclaw gateway is not connected')
  }
...
async function activeProjectsContext(): Promise<ActiveProjectsContext> {
  ...
  if (!gateway || gateway !== activeGateway() || profile !== ($activeGatewayProfile.get() || 'default')) {
    throw new Error('Active ulnclaw profile changed while connecting')
  }
...
async function refreshProjectTreeOn(gateway: ulnclawGateway): Promise<void> {
...
const repoScanStates = new WeakMap<ulnclawGateway, RepoScanState>()
const scanningGatewayGenerations = new WeakMap<ulnclawGateway, number>()
...
export async function scanAndRecordRepos(force = false): Promise<void> {
  ...
  try {
    const policy = repoDiscoveryPolicyFromConfig(await getulnclawConfig(context.profile))
...
// repo, returning where ulnclaw should start working. Git is the source of
...
export async function listRepoBranches(repoPath: string): Promise<ulnclawGitBranch[]> {
  const git = desktopGit()
...
export async function listBaseBranches(repoPath: string): Promise<ulnclawGitBaseBranch[]> {
  const git = desktopGit()
...
export async function revealPath(path: null | string): Promise<void> {
  if (path) {
    await window.ulnclawDesktop?.revealPath?.(path)
  }
}
...
export async function copyPath(path: null | string): Promise<void> {
  if (path) {
    await window.ulnclawDesktop?.writeClipboard?.(path)
  }
}
