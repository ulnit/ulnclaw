...
const STORAGE_KEY = 'ulnclaw:native-notifications'
...
// "Backgrounded" = the user isn't on ulnclaw. `document.hidden` only flips when
...
export function dispatchNativeNotification(input: NativeNotificationInput): void {
  ...
  void window.ulnclawDesktop?.notify({
...
export async function sendTestNativeNotification(title: string, body: string): Promise<boolean> {
  const bridge = window.ulnclawDesktop
