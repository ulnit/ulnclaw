...
export interface Translations {
  ...
  sessionsView: {
    ...
    msgCount: string; project: string; source: string; metaEnded: string;
    stats: string;
...
const zhHant: Translations = {
  ...
  sessionsView: {
    ...
    exportTitle: "將選取會話匯出為 Markdown", exportHtmlTitle: "將選取會話匯出為獨立 HTML", exportJsonTitle: "將選取會話匯出為可攜式 JSON（可重新匯入）", import: "⭱ 匯入", importTitle: "從可攜式 JSON 匯出檔匯入會話", imported: "已匯入 {imported} 個會話、{messages} 則訊息；跳過 {skipped}。", importParseFailed: "匯入失敗：檔案不是會話匯出 JSON 負載。", importFailed: "匯入失敗：{error}", msgCount: "{count} 則訊息", project: "專案", source: "來源", metaEnded: "已結束", stats: "{sessions} 個會話 \u00b7 {messages} 則訊息 \u00b7 磁碟佔用 {size}",
