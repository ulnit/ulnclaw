...
const zhHant: Translations = {
  ...
  palette: {
    ...
    updateCheck: "檢查更新", shortcuts: "鍵盤快捷鍵", notifications: "通知歷史", kanbanDispatch: "派發看板任務", kanbanDispatched: "看板派發：{spawned} 派生 · {promoted} 晉升 · {reclaimed} 回收", kanbanDispatchFailed: "看板派發失敗：{error}", kanbanQuickAdd: "新增看板任務…", kanbanQuickAddPrompt: "任務標題：", kanbanQuickAdded: "已建立看板任務：{id} —— {title}", kanbanQuickAddFailed: "看板任務建立失敗：{error}", kanbanBoardSwitch: "切換看板…", kanbanBoardSwitched: "已切換看板：{board}", kanbanBoardsFailed: "看板清單載入失敗：{error}", kanbanBoardsEmpty: "暫無可用看板", toggleSidebar: "切換側欄", themePicker: "切換主題…", themePickerTitle: "儀表盤主題", themeFailed: "主題載入失敗：{error}", fontPicker: "切換字體…", fontPickerTitle: "儀表盤字體", fontFailed: "字體載入失敗：{error}", copySessionId: "複製工作階段 ID", exportPickerTitle: "將工作階段匯出為…", copyLastReply: "複製最後一則助手回覆", lastReplyCopied: "已複製最後一則助手回覆。", lastReplyNone: "還沒有可複製的助手回覆。", archiveSession: "歸檔目前工作階段", unarchiveSession: "還原目前工作階段（取消歸檔）", sessionArchived: "已歸檔工作階段：{label}", sessionUnarchived: "已還原工作階段：{label}", sessionArchiveFailed: "工作階段歸檔失敗：{error}",
