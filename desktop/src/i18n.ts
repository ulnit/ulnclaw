...
const ja: Translations = {
  ...
  slash: {
    ...
    status: "セッション + agent ステータス（/status）", context: "このセッションのコンテキスト使用量（/context）", whoami: "プロセス識別情報（/whoami）", version: "ulnclaw バージョン（/version）", commands: "/help のエイリアス", profile: "設定済みプロファイルを一覧（/profile）", model: "モデルを表示または永続化（/model [provider model]）", reasoning: "推論深度を表示または永続化（/reasoning [level|clear]）", memory: "メモリストアの状態（/memory）", sessions: "最近のセッションを一覧（/sessions [N]）", stop: "このセッションの実行中 run を停止（/stop）", retry: "最後のユーザー発言まで切り詰めて再実行（/retry）", undo: "直近 N ターンのユーザー発言を削除し、削除内容を表示（/undo [N]）", verbose: "詳細ログを表示または永続化（/verbose [on|off]）", yolo: "危険コマンドを自動承認 — approvals.mode=off（/yolo [on|off]）", personality: "パーソナリティを一覧・有効化（/personality [name|none]）",
