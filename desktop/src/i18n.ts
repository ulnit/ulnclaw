...
const en: Translations = {
  ...
  slash: {
    ...
    status: "session + agent status (/status)", context: "context-window usage for this session (/context)", whoami: "process identity (/whoami)", version: "ulnclaw version (/version)", commands: "alias of /help", profile: "list configured profiles (/profile)", model: "show or persist the model (/model [provider model])", reasoning: "show or persist the reasoning effort (/reasoning [level|clear])", memory: "memory store status (/memory)", sessions: "list recent sessions (/sessions [N])", stop: "stop this session's running run (/stop)", retry: "truncate to the last user turn and re-run it (/retry)",
...
const zhHant: Translations = {
  ...
  slash: {
    ...
    status: "工作階段與 agent 狀態（/status）", context: "本工作階段上下文視窗用量（/context）", whoami: "處理序身分（/whoami）", version: "ulnclaw 版本（/version）", commands: "/help 的別名", profile: "列出已設定的 profile（/profile）", model: "檢視或持久化模型（/model [provider model]）", reasoning: "檢視或持久化推理深度（/reasoning [level|clear]）", memory: "記憶庫狀態（/memory）", sessions: "列出最近工作階段（/sessions [N]）", stop: "停止本工作階段執行中的 run（/stop）", retry: "截斷到最近一次使用者提問並重新執行（/retry）",
    resume: "恢復近期工作階段（桌面）",
  },
...
const ja: Translations = {
  ...
  slash: {
    ...
    status: "セッション + agent ステータス（/status）", context: "このセッションのコンテキスト使用量（/context）", whoami: "プロセス識別情報（/whoami）", version: "ulnclaw バージョン（/version）", commands: "/help のエイリアス", profile: "設定済みプロファイルを一覧（/profile）", model: "モデルを表示または永続化（/model [provider model]）", reasoning: "推論深度を表示または永続化（/reasoning [level|clear]）", memory: "メモリストアの状態（/memory）", sessions: "最近のセッションを一覧（/sessions [N]）", stop: "このセッションの実行中 run を停止（/stop）", retry: "最後のユーザー発言まで切り詰めて再実行（/retry）",
