...
- **🚪 HTTP gateway** — `ulnclaw gateway`: OpenAI-compatible `/v1/chat/completions` + `/v1/responses` (with session continuity), `stream: true` SSE token streaming with tool-progress events, async `/v1/runs` with SSE events + approval resolution, sessions API, bearer auth
...
# Interactive chat (slash commands: /new /search /memory /skills /sessions ...)
./ulnclaw chat
...
- **🚪 HTTP 网关** —— `ulnclaw gateway`：OpenAI 兼容 `/v1/chat/completions` + `/v1/responses`（会话续接）、`stream: true` SSE 令牌流（含工具进度事件）、带 SSE 事件 + 审批处理的异步 `/v1/runs`、会话 API、Bearer 鉴权
...
# 交互式聊天（斜杠命令：/new /search /memory /skills /sessions ……）
./ulnclaw chat
