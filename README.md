...
- **📡 消息平台** —— Telegram/Discord/Slack 适配器运行于 `ulnclaw gateway` 内（`[messaging.*]`），WhatsApp Cloud 与 Microsoft Graph 接入挂载为网关 webhook 路由（`/webhooks/whatsapp` HMAC 校验、`/webhooks/msgraph` clientState 校验）：白名单配对（fail closed）+ hermes 风格交互式配对码（`pairing list/approve/revoke/clear-pending`）、媒体附件缓存于 `media-cache/` 并以路径引用交付（出站 `MEDIA:` 标签在 Telegram/Discord 原生上传）、每聊天一条持久会话、hermes 风格回复分块
