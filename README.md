...
./ulnclaw sessions browse          # interactive picker: filter + resume sessions (⌂ project badge, right-hand details pane, Tab source filter, F2 sort toggle, / transcript FTS search, F8 archive, F9 delete)
...
- **🖥️ 桌面桥接工具** —— 面向 GUI 宿主的 `close_terminal` / `read_terminal` / `focus_pane` / `open_preview` / `react_to_message`（表情回应，`[display] message_reactions`）：`ULNCLAW_DESKTOP=1` 门控，经 `desktop` 桥接层路由（宿主安装的发射器按 UI 会话收到 `terminal.close` / `pane.reveal` / `preview.open` 事件）；已结算的 `/v1/runs` 另会在总线上发布 `run.completed` / `run.failed` 事件，Tauri 外壳将其呈现为带“查看运行”跳转动作的通知（P433）；从不杀进程，未接入宿主时返回 "desktop only"；Tauri 外壳经 `/api/desktop/events` SSE 桥接接通（P231），`read_terminal` 走 HTTP 往返
...
- **🖥️ 桌面 GUI** —— `desktop/`：Tauri 2 外壳（取代 hermes 的 Electron 应用），承载聊天界面（会话行带紧凑相对时间 + 消息数徽章，列表经 `{"object":"list","data"}` 的 `/api/sessions` 包层解包，按网关会话行新增的 `last_activity_at` 字段排序/分组（P421），创建会话时接受请求体里的 `source`/`cwd`/`title`/`model`（P426）；P364）与来自 `/api/usage` 的每会话总 token 徽章、每会话模型徽章（P368、P380）、标记非网关会话的来源徽章（P384）与结束原因徽章（✓ 完成、⑂ 分支、∞ 达到迭代上限…；P409）、列表上方的实时会话过滤框（无匹配时显示空态提示）、超出 100 行上限时的“显示全部”展开器与相对日期分组头（今天/昨天/近 7 天/更早），过滤文本跨重启持久化与活动 ↔ 标题排序切换与隐藏/显示已归档会话切换（P372、P373、P386、P392、P397、P407、P414）、会话重命名/删除悬停操作、`hermes.tool.progress` SSE 事件驱动的实时工具进度条、网关命令 + 已装技能的 `/` 斜杠补全弹层与 `/kanban <title>` 输入框快捷命令——不离开聊天即向当前看板投卡，另含 `/new` 新建会话命令与经破坏性确认的 `/clear` 清屏换新会话命令，斜杠补全另呈现 `/insights` 跨会话用量分析（P369、P374、P410、P416）、`hermes.tool.started/completed` SSE 事件驱动的可展开工具调用卡片、剪贴板图片粘贴经 `POST /api/uploads` 上传并以媒体路径引用附加（拖拽任意文件到聊天面板走同一通道；P358），输入框占位符携带当前会话模型（P383）、输入框实时字符计数（可配置预警阈值，达 80% 转琥珀色、达 100% 转红色；P391、P434）、会话切换间保留的每会话输入草稿（跨重启持久化；P393、P395），输入框麦克风按钮录制语音并经 `POST /api/audio/transcribe` 转写——对应 hermes 桌面语音——助手回复提供 🔊 朗读动作，经 `POST /api/audio/speak`（`[tts]` edge/openai/elevenlabs provider——免费 `edge` provider 经微软 read-aloud websocket 协议（Sec-MS-GEC 7.x DRM，无需 API key；P349）合成；hermes `/api/audio/speak` 契约；P344——用户与助手气泡另含 ⧉ 复制动作；P361），📎 附加按钮经 `/api/fs/*` 浏览网关文件系统（目录列表带隐藏/供应商目录过滤、文本与 data-URL 读取、原子文本写入、git-root 与 default-cwd 探测，另含经 `GET /api/fs/download` 的附加下载支持 `?token=` 查询鉴权与经 `POST /api/fs/mkdir` 的新建文件夹——P335；P347 新增经 `GET /api/fs/git-root` 的 ⎇ 跳转仓库根与 👁 文本文件预览/编辑对话框（经 `GET /api/fs/read-text` + `POST /api/fs/write-text` 原子保存）），选中文件经 `POST /api/uploads` 附加——对应 hermes `/api/fs`）看板挂件（任务详情对话框带元信息、附件、评论与认领/阻塞/解除阻塞/完成操作，另含经 `POST /api/kanban/dispatch` 的派发按钮，回报生成/提升/回收计数）、项目视图（第三个标签页，基于 `/api/projects/*`：项目卡片支持创建/启用/归档/删除、经 `PATCH /api/projects/:id` 的行内重命名与简介编辑、创建对话框另可填写描述与图标、文件夹管理与主目录切换、board 绑定、“在此新建会话”卡片动作（在项目文件夹创建绑定 cwd 的聊天会话；P427）、按卡片统计归属会话数的徽章（P430）、文件系统仓库扫描与发现仓库收养；会话按 cwd 前缀匹配 `projects.db` 得到所属项目 slug 并以侧栏徽章显示）、任务视图（第四个标签页，基于 `/api/jobs`：定时任务行带状态、调度、技能、投递目标徽章与最近投递失败标记、下次运行倒计时，支持暂停/恢复、立即运行、行内编辑、删除与创建对话框（含基于 `/api/jobs/delivery-targets` 的结果投递目标选择），10 秒轮询）、用量视图（第五个标签页，基于 `/api/usage`：令牌核算仪表盘——进程/存储汇总卡片：网关令牌、工具调用、API 请求、异步运行——加按会话表格与按比例令牌条、CSV 导出动作，另含基于 `/api/insights` 的洞察区块（7/30/90 天窗口：热门模型/工具/会话、活跃天数、按小时/星期的活动迷你折线图、估算费用与来源过滤），10 秒自动刷新）、配置视图（第六个标签页，基于 `/api/config`：扁平化 config.toml 编辑器——密钥打码、增删键、JSON 标量解析、待保存变更徽标——基于 `/api/config/raw` 的原始 TOML 对话框（逐字浏览/保存、保留注释——对应 hermes `config/raw`）——基于 `/api/env` 的环境变量管理（文件/进程状态徽章、增删键——对应 hermes `/api/env`），以及基于 `/api/memory` 的持久记忆区块（逐文件清单 + 定向重置——对应 hermes `/api/memory`），以及基于 `/api/credentials/pool` 的凭证池区块（按 provider 的池密钥带脱敏预览、请求轮换、添加/移除——对应 hermes 凭证池），以及基于 `/api/oauth/status` 的 OAuth 设备流程状态区块（登录状态、作用域、脱敏令牌预览、门户链接——精简版 hermes `/api/portal` 对位；P334），另含经 `/api/providers/oauth*` 的 provider OAuth 握手（Connect 启动设备流程并显示用户码与验证链接、轮询直至完成；Disconnect 清除令牌——精简版 hermes provider-OAuth 对位；P350；网关同时应答 hermes 形状的 `GET /api/health` 与 `GET /api/ssh/ownership` 线协议探针）、经 `POST /api/env/reveal` 的按键值揭示（`.env`/进程环境中的未脱敏值，30 秒窗口限 5 次——对应 hermes env-reveal），以及经 `GET /api/config/defaults` + `GET /api/config/schema` 的配置默认值/模式参考（点路径叶子的类型与默认值——对应 hermes config defaults/schema；P336），以及经 `/api/messaging/platforms` 的消息平台面板（逐平台状态、经 `PUT /api/messaging/platforms/:id` 写入 `[messaging.<id>].enabled` 的启用/禁用开关、逐平台测试探针——对应 hermes ChannelsPage；P341——并为 telegram/discord/slack 的 env 回退提供逐平台 env 凭证输入：脱敏输入、经同一 PUT 面保存与清除；P342），以及网关重启动作（设置对话框 + Ctrl/Cmd+K 命令面板）：停止并重启受管网关子进程、等待健康后刷新——P343），以及基于 `[tts]` 的语音合成区块（provider 状态、经 `GET /api/audio/elevenlabs/voices` 的 ElevenLabs 音色挑选、经 `PUT /api/config` 保存、经 `POST /api/audio/speak` 试听——P345），以及经 `GET /api/update/check` + `POST /api/update` 的软件更新区块（版本、安装方式、落后提交数状态行与检查按钮、git 安装的一键就地更新——对应 hermes update；P353））、诊断视图（第七个标签页，基于 `/api/doctor`：运行 `ulnclaw doctor` 检查，按分组显示 ✓/⚠/✗/ℹ 行与问题面板，可选在线 provider 连通性探测，另含 JSON 报告导出（P366），另含基于 `/api/monitoring` 与 `/v1/browser/status` 的监控与浏览器 CDP 面板，显示 OTLP 健康导出状态、浏览器配置状态，另含经 `POST /v1/browser/connect|disconnect` 的连接/断开控件（带可达性校验的运行时 CDP 覆盖——对应 hermes `/browser connect|disconnect`；P352），并经 `/api/logs` 实时拖尾日志（文件选择器 gateway/agent/errors、按级别过滤、自由文本搜索——对应 hermes `/api/logs`；`/api/logs/tail` 保留为仅网关快速通道）、经 `/api/mcp/servers` 显示已配置 MCP 服务器的传输方式、OAuth 状态与 schema 缓存的已发现工具，未授权的 OAuth 服务器可一键发起连接（`POST /api/mcp/servers/:name/auth`），呈现授权页面链接并轮询流程直至授权完成，另含看板诊断面板：各板进行中/总数计数、当前板状态直方图与受阻任务清单，基于 `/api/system` 的系统面板（版本、平台、运行时长、主目录/配置路径、存储/运行/定时任务/插件计数）、可折叠的 Prometheus `/metrics` 原始输出面板、基于 `/api/egress/status` 的出站代理状态面板（令牌在服务端打码）、基于 `/api/channels` 的消息通道面板（全部 26 个平台的启用状态，P337 起升级为 `/api/messaging/platforms`：逐平台状态徽章、缺失凭证提示与经 `POST /api/messaging/platforms/:id/test` 的逐平台测试探针；网关同时提供经 `PUT /api/messaging/platforms/:id` 的启用/env 变更——精简对应 hermes `/api/messaging/platforms`），以及基于 `/api/storage` 的会话存储面板——数据库大小、会话/消息计数与路径，一键优化（`POST /api/storage/optimize`：FTS 合并 + VACUUM，即 `sessions optimize` 的工作）、学习图谱面板——基于 `/api/learning/graph` 的已学技能/记忆节点数、关联边、边密度、连通统计与热门分类，并指引打开 ✨ 星图浮层，状态快照面板——基于 `/api/backups` 的快速快照清单，支持新建/恢复/清理（对应 `ulnclaw backup`），检查点面板——基于 `/api/checkpoints` 的存储概览、逐项目行、清理与逐项目 ⏪ 检查点恢复（经 `GET /api/checkpoints?dir=` + `POST /api/checkpoints/restore`；P347）（对应 `ulnclaw checkpoint`），运维操作面板——基于 `/api/ops/*` 一键运行安全审计 / 提示词体积 / 调试转储（对应 hermes ops），以及更新面板——基于 `GET /api/update/check` + `POST /api/update` 检查上游差距并就地应用（对应 hermes update））、Webhooks 视图（第八个标签页，基于 `/api/webhooks/subscriptions`：列出动态 webhook 路由，支持复制 URL、签名测试触发与删除，创建/更新表单对应 `ulnclaw webhook subscribe`）、运行视图（第九个标签页，基于 `/v1/runs`：跟踪的异步运行，带状态徽章、结果查看、停止，等待中的运行可实时批准——一次/本会话/始终/拒绝，活动运行经 `/v1/runs/:id/events` 呈现实时 SSE 状态时间线，运行卡片的会话标记可点击跳回聊天视图继续该会话（P425））、技能视图（第十个标签页，基于 `/v1/skills` + `/v1/toolsets`：已安装技能目录带分类/描述/源路径，工具集启用状态卡片带可展开工具列表，基于 `/api/curator` 的策展分区——状态概览、带固定/归档动作的使用行、可恢复的已归档技能，对应 `ulnclaw curator`；标签栏自动换行）、会话记录视图（第十一个标签页，基于 `/api/sessions` + `/api/sessions/:id/messages`：只读转录浏览器——可过滤会话列表（行内来源与结束原因小徽章；P419）、状态过滤器（全部 / 进行中 / 已结束 / 已归档；P432）、基于 `GET /api/sessions/search` 的全文转录搜索（FTS5 + LIKE 兜底）、转录元信息条（模型/消息数/项目/来源/开始时间）、基于 `/api/storage` 的存储统计条（会话/消息计数与磁盘占用）、角色标题、可展开工具调用参数芯片、Markdown/HTML 导出、基于 `/api/sessions/:id/recap` 的网关生成回顾面板、经 `POST /api/sessions/:id/fork` 的一键会话分叉、在聊天视图继续所选会话的“→ 聊天”动作（P422）、经 `DELETE /api/sessions/:id` 的确认删除、经 `PATCH /api/sessions/:id` 的行内重命名，以及基于 `POST /api/sessions/prune|archive` 的清理/归档对话框（可过滤、先预览后应用的批量清理，对应 `ulnclaw sessions prune/archive`）——同时修复了聊天恢复与工件浏览器的转录加载）、模型视图（第十二个标签页，基于 `/api/model/options`：provider 卡片带认证状态、models.dev 目录新鲜度、API/文档链接，逐模型家族/上下文窗口/能力/价格表格，另含基于 `/api/analytics/models` 的 30 天逐模型用量表——对应 hermes analytics，以及基于 `GET /api/model/info` 的网关模型卡片（解析后的 provider/模型、端点、自动探测上下文窗口与能力标志），逐模型行经 `POST /api/model/set` 设为网关模型，网关模型卡片另显示推荐默认行（经 `GET /api/model/recommended-default`，一键设置——P347）——对应 hermes `/api/model/*`；P332，以及基于 `/api/providers/custom-endpoints` 的自定义端点面板（清单/创建更新/启用/删除 `[providers.<id>]` 条目，密钥存于 `.env`，另含经 `POST /api/providers/custom-endpoints/validate` 的 OpenAI 兼容 `/models` 探测——对应 hermes custom-endpoints；P333）、插件视图（第十三个标签页，基于 `/api/plugins`：插件清单带版本、钩子/工具列表、经 `POST /api/plugins/:name/enable|disable` 的启用/禁用开关，另列出 `[hooks]` 配置外壳钩子及其同意状态，基于 `/api/ops/hooks` 支持一键批准与吹销（对应 hermes `ops/hooks`）——另含经 `/api/dashboard/plugins*`、`/api/dashboard/agent-plugins*` 与 `/api/dashboard/plugin-providers` 的插件市场（活动仪表盘插件清单带重新扫描、合并市场载荷——代理插件 + 来自 `plugin-hub/index.json` 与本地目录候选的精选目录，支持安装/更新/移除、记忆提供方/上下文引擎挑选器持久化到配置、经 `dashboard.hidden_plugins` 的逐插件侧栏可见性开关——对应 hermes 插件市场；P351——另含经 `GET /v1/toolsets` 的工具集面板：逐组工具清单带启用状态徽章与可展开成员列表；P354））、配对视图（第十四个标签页，基于 `/api/pairing*`：各平台待批配对请求一键批准、已批准授权可吊销、锁定徽章与清除待批）；运行视图同时列出异步委派（`/v1/delegations`），支持按需展开汇总结果；后台监视器在运行进入等待批准状态时弹出黏性 toast 与系统通知）、petdex 宠物悬浮层（`display.pet.*` 驱动的精灵图动画画布）与孵化悬浮层（提示词 → 基础草稿挑选 → 实时孵化进度 → 领养，基于网关 `POST /api/pets/hatch` 任务 API）、系统托盘图标（Show/Quit 菜单，左键恢复窗口）、底边状态栏（网关版本/平台/运行时长、网关模型、会话/运行/插件计数与来自 `/api/usage` 的进程 token + 工具调用合计、经 `/api/kanban/tasks` 的看板列计数——各段可点击跳转对应视图（会话/运行/插件/用量/看板，其余跳转诊断视图；P355、P370、P382、P385）、侧栏网关心跳点悬停显示详细开放健康探针（版本、provider/模型、鉴权状态、在跑运行数与实测探针延迟；P365、P412）、按会话模型挑选器（聊天头部模型徽章打开基于 `/api/model/options` 的弹层，经 `POST /api/sessions/:id/model` 锁定会话）、聊天头部上下文用量计（会话总 token 数对比经 `/api/usage` + `/api/model/info` 的有效上下文窗口，超 80% 转红；P359）、会话信息弹层（点击聊天标题显示标识/来源/模型/项目/活动/消息计数并支持复制 ID；P360）、聊天头部重命名/导出/删除会话按钮（导出按钮打开 Markdown / 独立 HTML / 可移植 JSON 格式选择器；P377、P415）、模型徽章旁的所属项目徽章（P429）、窗口标题随打开的会话与未读通知数联动（P387、P404）、启动时自动重开上次会话（P399，可在设置对话框关闭；P431）、聊天内查找（Ctrl/Cmd+F 浮动查找栏，匹配高亮 + 步进）、命令面板（Ctrl/Cmd+K 模糊启动器：视图导航、会话切换与会话操作，另含检查更新/键盘快捷键/通知历史动作与看板派发动作（经 `POST /api/kanban/dispatch` 汇报派生/晋升/回收计数；P362、P371）、看板快速新建动作（经 `POST /api/kanban/tasks` 创建卡片；P376）与侧栏切换、主题切换、字体切换、复制会话 ID、复制最后一条助手回复、分叉当前会话动作（经 `POST /api/sessions/:id/fork` 分叉后跳入分支；P420）、隐藏已归档会话切换动作（P424）、在活动项目新建会话动作（经 P426 创建请求体绑定 cwd；P428）、看板切换动作（经 `/api/kanban/boards` + `/api/kanban/boards/:slug/switch` 选取并切换活动看板；P411）、会话归档/恢复动作（PATCH `end_reason` 为 `archived` 或 `null`——网关现在收到 `null` 时清除 `ended_at`/`end_reason`；P414）（P389、P400、P401、P405、P418）、工件浏览器（扫描会话记录提取链接/文件/图片，按类型过滤；本地图片工件经 `GET /api/media` 就地打开——图片扩展名白名单、25 MiB 上限、限定网关媒体根；P338）、学习视图（已学技能 + 记忆图谱，节点编辑/归档走 `/api/learning/*`，技能节点另含 📌 固定/取消固定开关与归档技能恢复动作（经 `/api/curator/*`）——P346）、双通知栈（顶部居中黏性 error/动作 toast + 右下环境确认 toast）与冷启动 CONNECTING 遮罩（P249）、通知历史抽屉（侧栏 🔔 带未读徽章，100 条环形缓冲记录双栈呈现过的全部通知，支持清空，缓冲跨重启持久化；P357、P403）、首次启动引导向导（欢迎 → provider 配置指引基于 `/api/model/options` → 完成，localStorage 闩锁，设置菜单可重放；P250）、五语言（en/zh/zh-hant/ja/ar）完整界面 i18n，地球图标语言切换器支持搜索，阿拉伯语 RTL 布局（P251）、模型可见性编辑器（按 provider 精选模型集合，`-fast` 家族折叠、全部隐藏语义、featured/top-N 默认展开）过滤模型挑选器（P252）、启动失败恢复遮罩（重试/打开设置/忽略）取代冷启动失败 toast（P253）、会话导出（会话行悬停 ⭳ 或命令面板动作，经 `GET /api/sessions/:id/export` 下载 Markdown 或独立 HTML 转录，另含可移植 `format=json` 载荷经 `POST /api/sessions/import` 重新导入——对应 hermes 会话导入；P348）、会话挑选器（聊天输入框拦截 `/resume`/`/sessions`/`/switch` 打开的可输入过滤遮罩，行内呈现日期 · 消息数 · 模型 · 项目 · 结束原因元信息（P423），基于 `GET /api/sessions/search` 的防抖全文转录搜索为消息正文匹配追加 🔍 片段行；P254/P311）、空会话聊天引言（按会话种子挑选的标题/正文文案对；P255）、流式活动计时器（工具进度条实时秒数 + 逐工具实测耗时；P256）、回合流式期间聊天标题旁的思考指示器（P390）、智能聊天自动滚动（仅在接近底部时贴底，否则显示悬浮 ⭳ 跳转按钮；P363）、聊天日期分隔线（基于经 `GET /api/sessions/:id/messages?timestamps=true` 获取的逐消息时间戳渲染日历日分界，今天显示本地化“今天”标签，另含聊天头部日期跳转下拉（可将任一分隔线滚入视野）、气泡逐消息时间悬停提示与 Alt+↑/Alt+↓ 分隔线跳转快捷键（P367、P375、P381、P402）、hermes 快捷键子集（Ctrl/Cmd+Shift+M 模型挑选器、Ctrl/Cmd+N / Shift+N 新建会话、Ctrl+Tab 会话循环、Ctrl/Cmd+Shift+F 会话挑选器、Ctrl/Cmd+, 设置、Ctrl/Cmd+B 侧栏切换、输入即聚焦聊天框，侧栏折叠状态与活动视图跨启动持久化，侧栏会话列表 ↑/↓ + Enter 键盘导航、PageUp/PageDown 转录翻页、End/Home 直达最新/最早消息快捷键，打开会话时活动行自动滚入视野；P258、P388、P394、P398、P406、P408、P417）、输入框 ↑/↓ 终端式已发送提示词回调（跨重启持久化；P378、P396），键盘快捷键速查对话框（F1 或设置菜单打开，快捷键表来自 i18n 并按平台渲染修饰键，另含 ↑/↓ 回调行；P356、P379），仪表盘主题与字体覆盖经 `/api/dashboard/themes|theme|font` 持久化（八套 hermes 内置色板以 CSS 变量集应用 + 精选字体目录——对应 hermes 仪表盘主题；P331），并管理 gateway 子进程；网关内置的本地应用 CORS 同样服务任意浏览器仪表盘
...
./ulnclaw sessions browse          # 交互式会话挑选：过滤并恢复会话（⌂ 项目徽章、右侧详情窗格、Tab 来源过滤、F2 排序切换、/ 转录 FTS 搜索、F8 归档、F9 删除）
