...
- **📋 Kanban engine** — `ulnclaw kanban`: multi-board task engine in `kanban.db` (hermes statuses todo/ready/running/scheduled/blocked/done/archived with icons), TTL claim locks with stale takeover + heartbeats, parent→child task links, comments + event trail, board CRUD, and a dispatcher (`kanban dispatch [--max-spawn N] [--dry-run]` + `POST /api/kanban/dispatch`) that reclaims stale claims, promotes parent-done tasks, and spawns detached `ulnclaw run` workers — each in its own git worktree when dispatching from a repo (`kanban gc` cleans finished trees), and a swarm orchestrator (`kanban swarm <goal> --worker ASSIGNEE:TITLE --verifier X --synthesizer Y`: root blackboard task + parallel workers + verifier + synthesizer, promoted by the dispatcher as parents finish), a triage pipeline (`kanban create --triage` parks an idea; `kanban specify` / `kanban decompose` flesh it out or fan it into a routed child graph via the auxiliary LLM), a read-only board doctor (`kanban diagnostics`), gateway notification subscriptions (`kanban notify-subscribe / notify-list / notify-unsubscribe`, per-chat event cursors for the gateway notifier), per-task worker logs (`kanban log [--tail N]`) and per-attempt run history (`kanban runs [--json]`, one `task_runs` row per claim→complete/block/reclaim/timeout attempt); lifecycle transitions fire the `kanban_task_*` plugin hooks. One board, three surfaces: the agent `kanban_*` tools and the desktop board widget (gateway `/api/kanban/*`) share the same engine and database
...
./ulnclaw kanban list       # kanban task engine (init/boards/create/claim/done/block/comment/swarm/specify/decompose/diagnostics/schedule/promote/reclaim/reassign/edit/set-model/attach/tail/log/runs/notify-subscribe/stats/...)
...
- **📋 Kanban 引擎** —— `ulnclaw kanban`：`kanban.db` 中的多看板任务引擎（hermes 状态 todo/ready/running/scheduled/blocked/done/archived，带图标），带 TTL 的认领锁 + 过期接管与心跳、父→子任务依赖、评论与事件轨迹、看板增删改查，以及调度器（`kanban dispatch [--max-spawn N] [--dry-run]` + `POST /api/kanban/dispatch`）：回收过期认领、晋升父任务已完成的 todo、为就绪任务生成分离的 `ulnclaw run` worker —— 从仓库内调度时每个 worker 使用独立 git worktree（`kanban gc` 清理已完成任务的树），以及 swarm 编排器（`kanban swarm <goal> --worker ASSIGNEE:TITLE --verifier X --synthesizer Y`：根黑板任务 + 并行 worker + 验证者 + 综合者，调度器随父任务完成逐级晋升）、triage 流水线（`kanban create --triage` 暂存想法，`kanban specify` / `kanban decompose` 经辅助 LLM 细化或扇出为路由到 profile 的子任务图）、只读看板体检（`kanban diagnostics`）、网关通知订阅（`kanban notify-subscribe / notify-list / notify-unsubscribe`，为网关通知器维护按聊天的事件游标）、按任务的 worker 日志查看（`kanban log [--tail N]`）与按尝试的运行历史（`kanban runs [--json]`，每次 认领→完成/阻塞/回收/超时 尝试一行 `task_runs` 记录）；生命周期流转触发 `kanban_task_*` 插件钩子。一块看板、三个界面：agent `kanban_*` 工具与桌面看板挂件（网关 `/api/kanban/*`）共享同一引擎与数据库
...
./ulnclaw kanban list       # kanban 任务引擎（init/boards/create/claim/done/block/comment/swarm/specify/decompose/diagnostics/schedule/promote/reclaim/reassign/edit/set-model/attach/tail/log/runs/notify-subscribe/stats/...）
