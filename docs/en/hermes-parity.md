...
| CLI (`hermes_cli/`) | ✅ core | chat REPL with slash commands (incl. `/rollback [N|hash] [file]`, `/rollback diff <N>`, `/diff` checkpoint commands, `/recap`), one-shot `run`, sessions/tools/skills/cron/checkpoints subcommands (incl. `sessions export` — SHA256-verified Markdown + manifest — and `sessions recap`), `init` |
| Delegation | ✅ | SubAgentRunner trait, depth limit, child sessions |
| HTTP gateway (`gateway/platforms/api_server.py`) | ✅ core | `ulnclaw gateway`: OpenAI-compatible `/v1/chat/completions` (session continuity via `X-Ulnclaw-Session-Id`, `stream: true` SSE token streaming with `hermes.tool.progress` events), `/v1/responses` (stateful via `previous_response_id`, `stream: true` Responses-API SSE events), `/v1/models`, `/api/model/options`, `/v1/capabilities`, `/v1/runs` (async runs + SSE events + stop + approval), `/api/sessions` CRUD + chat + chat/stream + `PATCH` (title/end_reason) + `fork` + per-session model lock (enforced on every turn), `/api/jobs` cron HTTP API (CRUD + pause/resume/run), `/v1/skills`, `/v1/toolsets`, bearer-token auth |
