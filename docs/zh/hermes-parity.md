...
- 插件：ulnclaw 插件是讲 hermes shell-hook JSON 协议的子进程（目录插件 +
  ...
  `--provider`。看板其余有意保留的差异：`create --project`（hermes 的项目
  登记簿未移植——看板 board 承担同样角色）、调度期 `default_assignee` 应用
  （ulnclaw 在默认 profile 上 spawn 未指派任务而不是跳过）。
