...
- 插件：ulnclaw 插件是讲 hermes shell-hook JSON 协议的子进程（目录插件 +
  ...
  无法解析的链接静默丢弃（hermes 丢弃悬空引用语义）。看板其余有意保留的
