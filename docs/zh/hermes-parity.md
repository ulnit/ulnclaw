...
- 插件：ulnclaw 插件是讲 hermes shell-hook JSON 协议的子进程（目录插件 +
  ...
  `cli-scan:v1` 策略键写入缓存；`project repos [--clear]` 列出/清空缓存。
  看板其余有意保留的
