//! `/learn` — build the standards-guided prompt that turns whatever the
//! user described into a reusable skill (port of hermes
//! `agent/learn_prompt.py`).
//!
//! `/learn` is open-ended: the user can point it at a directory of code,
//! an API doc URL, a workflow they just walked the agent through in this
//! conversation, or pasted notes. This module builds ONE prompt that
//! instructs the live agent to gather the named sources with the tools
//! it already has and author a single `SKILL.md` via `skill_manage`.
//! There is no separate distillation engine — every surface (gateway
//! session chat, platform chats) feeds the result to the agent as a
//! normal turn.

/// House-style skill-authoring rules embedded in the prompt so the
/// agent authors skills the way a maintainer would by hand (hermes
/// `_AUTHORING_STANDARDS` parity, adapted to the ulnclaw toolset).
const AUTHORING_STANDARDS: &str = "\
Follow the UlncLaw skill-authoring standards exactly. These are the same
rules a maintainer enforces in review:
...
4. \"## How to Run\" — the canonical invocation, framed through agent tools.
...
- Reference tools by name in backticks: `terminal`, `read_file`,
...
/// Build the agent prompt for an open-ended `/learn` request (hermes
/// `build_learn_prompt`). Empty input falls back to "the workflow we
...
mod tests {
    ...
    #[test]
    fn empty_request_falls_back_to_conversation_workflow() {
        let prompt = build_learn_prompt("   ");
        assert!(prompt.contains("[/learn]"), "{prompt}");
        assert!(
            prompt.contains("the workflow we just went through"),
            "{prompt}"
        );
    }

    #[test]
    fn request_is_embedded_with_standards() {
        let prompt = build_learn_prompt("docs/api.md focus on auth");
        assert!(prompt.contains("docs/api.md focus on auth"), "{prompt}");
        assert!(prompt.contains("THE REQUEST:"), "{prompt}");
        // Authoring standards ride along.
        assert!(prompt.contains("<=60 characters"), "{prompt}");
        assert!(prompt.contains("skill_manage"), "{prompt}");
